# Канал исполнителя — N4-priyom-zadach (один заход до конца)
...
```
cd /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot && python3 /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot/_generator/tools/orkestr.py zhurnal/2026-09-02_spetsmat-bot --rezhim progon --dvizhok opencode --rod instrumenty --model openrouter/z-ai/glm-5.2:free --zahody kod_N4-priyom-zadach.md < /dev/null 2>&1 | tee /tmp/zahod-N4-priyom-zadach.log
```
...
Приоритет владельца: разобрать инциденты важнее, потом закрыть долги — неразобранный инцидент это повторяющаяся ошибка, долг может подождать.
  ...
  4. пунктов очереди «ДОСТАВЛЕНО: нет»   : 210
...
ЦЕЛЬ: собрать сетку «ученик x задачи листка», где отметка ставится тапом через существующий сервис.
Приёмка — по ОТЧЁТУ, без построчной сверки. Стоп до цели: ты получишь экран приёма, но НЕ заведёшь второй журнал отметок — это объявлено провалом волны.
...
3. второго журнала отметок НЕ заводить — это названо провалом волны «сайт» и остаётся провалом здесь
4. marks — журнал СОБЫТИЙ: 15847 записей, из них часть retract. Кто посчитает сданное как count(*), покажет владельцу несуществующие решённые задачи

## КОНТРАКТ ЗОНЫ (обязателен — не удалять; вписан Cowork)
- **МЕСТО РАБОТЫ:** ветка `zahod/N4-priyom` в основной папке. 🔴 Она должна УЖЕ стоять. НЕ на ней — СТОП, НЕ делай `git checkout`: в общей папке он МОЛЧА откатывает дерево к состоянию ветки (цена 27→28.07: файл сильно откатился ночью, поймал владелец вручную; след в git НЕ остаётся). Тогда заход пересобрать с `--worktree`. Ветку не переключай, в другие НЕ коммить.
- **ЗОНА (можно менять):** `veb/priyom.py` `veb/static/priyom.css` `tests/veb/test_priyom.py` `zhurnal/2026-09-02_spetsmat-bot/kod_N4-priyom-zadach.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/bootstrap_zahod.py --zadanie-subagentu --zone veb/priyom.py --zone veb/static/priyom.css --zone tests/veb/test_priyom.py --kommitit 'каждые 20 минут, даже незаконченное' --zakryt 'ветку zahod/N4-priyom после переноса'
```
...
- Проверь ветку: `git branch --show-current` — обязано быть `zahod/N4-priyom`. Не она — СТОП, `git checkout` НЕ делай (§4 GIT-disciplina), нужен `--worktree`.
- Точка отката: `git add veb/priyom.py veb/static/priyom.css tests/veb/test_priyom.py zhurnal/2026-09-02_spetsmat-bot/kod_N4-priyom-zadach.md` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
Верификатор нужен, тип — **ПОСЛЕ-типа** — судит результат, стоит в конце, после задачи. Свежий субагент, ДРУГИМ методом (сверить проекцию клетки с core/services/progress.py на всех четырёх исходах живым прогоном, а не перечитыванием), не перечитывает свою же правку. Доля сплошной выборки: все четыре исхода EMPTY/SOLVED/RETRACTED/erratum, 4 из 4. Финальная строка ответа обязательна дословно: «выдано N позиций из M найденных» — без неё ответ считается усечённым и в отчёт не вставляется.
...
```
git --no-optional-locks add -- veb/priyom.py veb/static/priyom.css tests/veb/test_priyom.py zhurnal/2026-09-02_spetsmat-bot/kod_N4-priyom-zadach.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- veb/priyom.py veb/static/priyom.css tests/veb/test_priyom.py zhurnal/2026-09-02_spetsmat-bot/kod_N4-priyom-zadach.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `veb/priyom.py` `veb/static/priyom.css` `tests/veb/test_priyom.py` `zhurnal/2026-09-02_spetsmat-bot/kod_N4-priyom-zadach.md`

- **Г1. Зона доехала в git.** `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone veb/priyom.py` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone veb/static/priyom.css` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone tests/veb/test_priyom.py` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone zhurnal/2026-09-02_spetsmat-bot/kod_N4-priyom-zadach.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py vlit-v-osnovnuyu zahod/N4-priyom --zone <своя зона> \
