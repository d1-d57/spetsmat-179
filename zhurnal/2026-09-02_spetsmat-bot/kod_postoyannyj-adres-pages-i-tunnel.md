...
### ПРАВКА 1 · 2026-09-04 17:41 · причина, по которой часть 2 не пошла, устранена — `deploy/ADRES.txt` теперь В GIT

**Что было.** Ты написал в отчёте: «`deploy/ADRES.txt` отсутствует, туннель не запущен».
Это было ПРАВДОЙ ПРО ТВОЮ ПАПКУ и неправдой про репозиторий. Файл существовал в главной
рабочей папке, но НЕ БЫЛ В GIT — а неотслеженный файл в worktree не попадает. Ты
физически не мог его увидеть, и часть 2 была невыполнима не по твоей вине.

**Что изменилось.** Оркестратор забрал файл в git, коммит `6cfce9c`. Содержимое:

```
https://81aa074de780d0.lhr.life
```

Сделай `git --no-optional-locks pull` или `git merge main` в своей папке первым ходом,
и файл будет на месте. Проверь командой: `cat deploy/ADRES.txt`.

**Что делать.** Часть 1 ПРИНЯТА и переделке не подлежит — `c3d81ac` влит в main,
маркер `OUR_CONTENT_MARKER` проверен оркестратором в main грепом. Делай ТОЛЬКО ЧАСТЬ 2:
постоянный адрес. Критерий владельца дословно — адрес туннеля предъявлен ДВАЖДЫ с
ПЕРЕЗАПУСКОМ ПРОЦЕССА между предъявлениями. Три вещи в отчёт по порядку: первый `curl`
с адресом и кодом · команда убийства и повторного подъёма · второй `curl` с ТЕМ ЖЕ
адресом и кодом. Адрес во втором прогоне другой — критерий НЕ выполнен, так и напиши.

🔴 **И ОДНА ВЕЩЬ, КОТОРУЮ ВИДНО ТОЛЬКО СВЕРХУ — ПРОВЕРЬ ЕЁ.** Твой маркер
`"Распределение"` — это заголовок страницы, которая СЕГОДНЯ лежит на `/`. Позиция S1
прямо сейчас переносит распределение на `/raspredelenie`, а на `/` встанет главная.
Если адрес в `ADRES.txt` указывает на корень, после влития S1 твой починенный сторож
начнёт рапортовать `мёртв` о ЖИВОМ сайте — ложная тревога преподавателям, а владелец
сказал, что она дороже функционала. Реши это: либо сторож проверяет адрес с путём
`/raspredelenie`, либо маркер берётся такой, который есть на ЛЮБОЙ нашей странице.
Что выбрал — назови в `## ОТЧЁТ` строкой.

**Что откатить.** Ничего. Часть 1 остаётся как есть.
...
**ВЕРДИКТ:** `принято` — часть 1 проверена мной В MAIN грепом после влития, а не по отчёту; часть 2 позиция честно назвала несделанной, и мой собственный прогон показал, что она НЕВЫПОЛНИМА туннелем в принципе — это вопрос владельцу, а не недоработка позиции.

**ОХВАТ — что именно проверено и чем, прогоном рычага 17:39–17:46:**

| клауза критерия | чем снято | итог |
|---|---|---|
| сторож перестал зеленеть на баннере | прочитан дифф `c3d81ac`: заведён `OUR_CONTENT_MARKER`, 200 БЕЗ маркера → `мёртв: content_marker_MISSING (banner?)` | ✅ |
| правка доехала в main | `grep -c OUR_CONTENT_MARKER ops/storozh_sajta.py` в main → `2` | ✅ |
| сцепка с переездом распределения | коммит `23ed51d` — сторож проверяет `/raspredelenie`, а не корень; позиция закрыла её сама, прочитав ПРАВКУ 1 | ✅ |
| адрес туннеля отдаёт код | `curl https://598c6edac618b6.lhr.life` → `302` (ответ НАШЕГО сервера, не баннера) | ✅ |
| адрес переживает перезапуск САЙТА | убит pid 93172 → во время простоя `000`/«Empty reply», поднят заново → тот же адрес `302` | ✅ |
| 🔴 адрес переживает перезапуск ТУННЕЛЯ | убит ssh → `503`; поднят заново → адрес СТАЛ ДРУГИМ: `598c6edac618b6` → `f0a4bf9fecabd2` | ❌ НЕВЫПОЛНИМО |
| адрес Pages отдаёт 200 | не проверено: требует учётки владельца | ⏸ вопрос владельцу |

**ЧТО ЗАСЧИТАНО ПОЗИЦИИ.** Единственная вещь, за которую она отвечала и могла закрыть —
ложно-зелёный сторож — закрыта, влита и проверена мной в main. Плюс она честно назвала
несделанное списком вместо рапорта о готовности, и это ровно тот исход, который заход
объявлял законным.

**ЧТО НЕ ЕЁ ВИНА.** Первый прогон позиции был невыполним: `deploy/ADRES.txt` не был
в git, а неотслеженный файл в worktree не попадает — позиция физически не видела адреса.
Забрано в git оркестратором, коммит `6cfce9c`.

**ЧТО ОСТАЁТСЯ ВЛАДЕЛЬЦУ.** Постоянный адрес туннелем не достигается: `localhost.run`
на бесплатном тире выдаёт новый поддомен на каждое соединение. Нужен либо GitHub Pages,
либо резервированный поддомен. Оба требуют учётки владельца — вопрос `[V1]` мандата.
