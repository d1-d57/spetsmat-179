# Канал исполнителя — R2-slovar (один заход до конца)
...
> **Модель: Opus 5** — решение про gRPC-зависимость против точности распознавания фамилий — суждение, а не механика.
...
```
bash /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot/zhurnal/2026-09-02_spetsmat-bot/ZAPUSK-ZAHODA.sh R2-slovar opus
```
...
for l in open(sys.argv[1], encoding="utf-8"):
 ...
 if d.get("type")=="result":
  ...
  for m,v in (d.get("modelUsage") or {}).items():
   print("  %s: вх %s вых %s USD %s" % (m, v.get("inputTokens"), v.get("outputTokens"), v.get("costUSD")))' /tmp/zahod-R2-slovar.jsonl
```
...
Приоритет владельца: разобрать инциденты важнее, потом закрыть долги — неразобранный инцидент это повторяющаяся ошибка, долг может подождать.
  ...
  4. пунктов очереди «ДОСТАВЛЕНО: нет»   : 169
...
Голос — один из двух каналов, которыми преподаватель отмечает задачи на занятии.
Распознаёт его Yandex SpeechKit; ключ владельца заведён сегодня и проверен живым
вызовом. Ты чинишь то, из-за чего фамилии распознаются как обычная русская речь.

🔴 ЗАМЕР ОРКЕСТРАТОРА 02.09 19:2x, на живом каталоге:

    фамилий из каталога : 56
    меток задач         : 136
    ТЕРМИНОВ В СЛОВАРЕ  : 192
    строка живого старта: «speechkit, verbatim mode, 192 terms held for the phrase list»
...
То есть 192 термина собираются, передаются в распознаватель, хранятся в нём — и
в запрос НЕ ПОПАДАЮТ. «Санин» и «пятнадцать а» уезжают без подсказки.

🔴 И СТРОКА СТАРТА ВВОДИТ В ЗАБЛУЖДЕНИЕ. «192 terms held for the phrase list»
формально честна — «held» значит «удерживаются», — но человек читает её как
«словарь работает». Владелец прочёл именно так. Это тот же класс, что ловила
`P18`: текст, который не врёт буквально и обманывает по существу.

⚠ ВТОРОЙ КАНАЛ СОПОСТАВЛЕНИЯ ТОЖЕ ОТСУТСТВУЕТ, и это делает словарь важнее:
`core.services.golos.load_schema_extractor` ищет у `P7` текстовую фабрику схемы,
не находит (схема `P7` — про картинки) и говорит об этом вслух. Значит после
расшифровки работает ОДИН нечёткий канал, и всё, что он получит, — это то, что
услышал SpeechKit.
...
1. ВЛАДЕЛЕЦ ДОСЛОВНО, RYVOK-2025.md §1 строка D: «словарь SpeechKit через v3 gRPC — 192 термина собираются и не уезжают».
2. ВЛАДЕЛЕЦ ДОСЛОВНО про цену отсрочки: «чини найденное сразу, не откладывай в заявки: заявка это отсрочка, а у нас сегодня есть чем платить, и завтра не будет».

## КОНТРАКТ ЗОНЫ (обязателен — не удалять; вписан Cowork)
- **МЕСТО РАБОТЫ:** ветка `zahod/R2-slovar` в основной папке. 🔴 Она должна УЖЕ стоять. НЕ на ней — СТОП, НЕ делай `git checkout`: в общей папке он МОЛЧА откатывает дерево к состоянию ветки (цена 27→28.07: файл сильно откатился ночью, поймал владелец вручную; след в git НЕ остаётся). Тогда заход пересобрать с `--worktree`. Ветку не переключай, в другие НЕ коммить.
- **ЗОНА (можно менять):** `infra/asr.py` `tests/voice/` `zhurnal/2026-09-02_spetsmat-bot/kod_R2-slovar.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/bootstrap_zahod.py --zadanie-subagentu --zone infra/asr.py --zone tests/voice/ --kommitit 'починку и её проверку одним коммитом; отдельным — честную строку старта' --zakryt 'ветку zahod/R2-slovar влить в main последним ходом самому'
```
...
- Проверь ветку: `git branch --show-current` — обязано быть `zahod/R2-slovar`. Не она — СТОП, `git checkout` НЕ делай (§4 GIT-disciplina), нужен `--worktree`.
- Точка отката: `git add infra/asr.py tests/voice/ zhurnal/2026-09-02_spetsmat-bot/kod_R2-slovar.md` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
Реши по существу: доводить словарь до SpeechKit или объявить, что его нет.
Оба исхода законны, третий — оставить как есть — нет.

**ЧАСТЬ 1 · ЧЕСТНАЯ СТРОКА СТАРТА. Делай её ПЕРВОЙ и коммить отдельно.**
Она стоит десять минут и снимает ложное успокоение уже сегодня, чем бы ни
кончилась часть 2. Строка обязана говорить, что происходит на самом деле: словарь
из N терминов собран, синхронный эндпоинт его не принимает, распознавание идёт
без подсказки. Формулируй так, чтобы человек, читающий лог на занятии, понял с
первого раза.

**ЧАСТЬ 2 · СЛОВАРЬ ДО SPEECHKIT.** Разберись, чего стоит v3: это gRPC-поток,
то есть зависимость (`grpcio`, сгенерированные заглушки) и другой протокол.
Оцени ЧЕСТНО и напиши в отчёт: сколько строк кода, какие пакеты, что ломается,
если пакета нет на машине владельца. Если цена приемлема — сделай, с запасным
путём на v1 при отсутствии зависимости. Если нет — НЕ ДЕЛАЙ и объясни числами,
а не общими словами; тогда часть 1 остаётся единственным результатом, и это
законный исход.

🔴 ЗАПАСНОЙ ПУТЬ ОБЯЗАТЕЛЕН В ЛЮБОМ СЛУЧАЕ: если v3 недоступен — нет пакета, нет
сети, отказ сервиса, — голос обязан продолжать работать через v1 без словаря, а
не падать. Занятие важнее точности.

**ЧЕГО НЕ ДЕЛАТЬ.** Не трогать нечёткое сопоставление (`core/services/golos.py`):
оно принято позицией `P8` и работает. Не изобретать второй распознаватель. Не
полировать внутренние имена.

**КРИТЕРИЙ ГОТОВНОСТИ — что обязано провалиться, если работа не сделана:**

    🔴 ЖИВОЙ ВЫЗОВ, не фикстура. Ключ и каталог в `secrets/bot.env`
    (`SPETSMAT_ASR_KEY`, `SPETSMAT_ASR_FOLDER`), проверены оркестратором:
    секунда тишины в `stt:recognize` → `{"result":""}`.
    В отчёт — вывод вызова дословно.

    🔴 ЖИВОЙ ПРОГОН НА РЕАЛЬНЫХ ОБЪЕКТАХ РЕПОЗИТОРИЯ, не на фикстурах — без него
    критерий не закрыт. Живые объекты здесь: БОЕВАЯ БАЗА `data/spetsmat.db`
    (56 учеников, 136 меток задач — из них и собираются те самые 192 термина) и
    настоящий ключ SpeechKit в `secrets/bot.env`. Требование владельца: «всегда
    гонять на конкретной практике»; цена в этой волне — 486 зелёных тестов при
    боте, который не поднимался.

    python3 -m pytest tests/voice -q     → зелёный
    python3 -m pytest -q                 → 916 passed и больше, 0 failed
    (вернуть прежнюю строку старта)      → тест на честность строки КРАСНЕЕТ

Тест части 1 обязан требовать, чтобы строка НЕ утверждала работу словаря, пока
термины не попадают в запрос. Покажи его красным до починки — выводом команды.

## 3. ВЕРИФИКАТОР (если двигаем/теряем/жмём)

Верификатор нужен, тип — **ПОСЛЕ-типа** — судит результат, стоит в конце, после задачи. Свежий субагент, ДРУГИМ методом (живой вызов SpeechKit с синтетическим звуком и БЕЗ словаря / СО словарём, сравнение), не перечитывает свою же правку. Доля сплошной выборки: 2 прогона из 2: с подсказкой и без неё. Финальная строка ответа обязательна дословно: «выдано N позиций из M найденных» — без неё ответ считается усечённым и в отчёт не вставляется.
...
```
git --no-optional-locks add -- infra/asr.py tests/voice/ zhurnal/2026-09-02_spetsmat-bot/kod_R2-slovar.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- infra/asr.py tests/voice/ zhurnal/2026-09-02_spetsmat-bot/kod_R2-slovar.md   # отсекает всё чужое
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone infra/asr.py --zone tests/voice/   # из корня репо; должен быть ✅
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py commit --zone infra/asr.py --zone tests/voice/ \
...
**ЗОНА ГИГИЕНЫ:** `infra/asr.py` `tests/voice/` `zhurnal/2026-09-02_spetsmat-bot/kod_R2-slovar.md`

- **Г1. Зона доехала в git.** `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone infra/asr.py` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone tests/voice/` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone zhurnal/2026-09-02_spetsmat-bot/kod_R2-slovar.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
...
```
Своя зона — своими путями (`add` + `commit -- infra/asr.py tests/voice/`). Чужая содержательная работа — НЕ твоя:
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py vlit-v-osnovnuyu zahod/R2-slovar --zone <своя зона> \
...
**КОММИТ:** `<хэш>` — `<сообщение>` · `git_zona.py check --zone infra/asr.py --zone tests/voice/` → ✅
