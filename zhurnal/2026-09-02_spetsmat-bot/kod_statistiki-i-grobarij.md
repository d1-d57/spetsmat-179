...
**ВЕРДИКТ:** принято — 1205 строк, влито `50cf01e`, 35 passed. Клауза F6 закрыта БУКВАЛЬНО: сверка прямым SQL по снимку БОЕВОЙ базы через `Connection.backup` сквозь WAL (16188 событий, 595 задач), проверочный скрипт не импортирует ни строки проектного кода, числа рядом — «1а экран 16 SQL 16», «1б 15/15», «2 12/12», «3 10/10». Гробарий пустой вкладкой по решению владельца
