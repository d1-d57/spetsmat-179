# Канал исполнителя — R3-istochniki (один заход до конца)
...
> **Модель: Opus 5** — чтение четырёх содержательных документов и поиск расхождений с кодом — суждение; цена пропуска уже оплачена форматом текстового ввода.
...
```
bash /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot/zhurnal/2026-09-02_spetsmat-bot/ZAPUSK-ZAHODA.sh R3-istochniki opus
```
...
for l in open(sys.argv[1], encoding="utf-8"):
 ...
 if d.get("type")=="result":
  ...
  for m,v in (d.get("modelUsage") or {}).items():
   print("  %s: вх %s вых %s USD %s" % (m, v.get("inputTokens"), v.get("outputTokens"), v.get("costUSD")))' /tmp/zahod-R3-istochniki.jsonl
```
...
Приоритет владельца: разобрать инциденты важнее, потом закрыть долги — неразобранный инцидент это повторяющаяся ошибка, долг может подождать.
  ...
  4. пунктов очереди «ДОСТАВЛЕНО: нет»   : 172
...
Ты независимый судья волны. Девятнадцать позиций приняты, бот работает, завтра по
нему ведут занятие. Твоя задача — НЕ проверять, сделано ли то, что записано в
мандате. Твоя задача — проверить, сделано ли то, что хотел ВЛАДЕЛЕЦ.

🔴 РАЗНИЦА МЕЖДУ ЭТИМ УЖЕ ОПЛАЧЕНА СЕГОДНЯ, И ДОРОГО. В мандате финализировано
`[F7] text input carries the sheet in square brackets - [7] Petrov 3,5,7b`. Формат
этот придумал ОРКЕСТРАТОР, а не владелец. Настоящая запись владельца выглядит так:
...
Расхождений четыре, и одно из них молча теряло бы ПОЛОВИНУ задач: скобка стоит
ПОСЛЕ имени, листок по умолчанию текущий, задач около десяти с переносом строк,
прочерк — это явка без сдач. Позиция была написана по пересказу и переписана
целиком, когда владелец прислал настоящий формат. Мандат — пересказ. Ты идёшь к
источнику.

ИСТОЧНИКИ, все в `zhurnal/2026-09-02_spetsmat-bot/`:
`спецмат-бот-модель.md` (модель данных) · `спецмат-бот-архитектура.md` (стек,
схема SQL, экран приёма) · `спецмат-бот-механизм.md` (распознавание, эксплуатация,
персданные) · `спецмат-бот-ресёрч.md` (темпоральная модель, UX, педагогика).

🔴 ТЫ НИЧЕГО НЕ ПИШЕШЬ В КОД. Твоя зона — один файл, твой собственный. Нашёл
расхождение — назови его и поставь заявку; чинить будут другие.
...
1. ВЛАДЕЛЕЦ ДОСЛОВНО, RYVOK-2025.md §2: «сверяй с ИСХОДНЫМИ документами владельца, а не только с мандатом: спецмат-бот-модель.md, -архитектура.md, -механизм.md, -ресёрч.md. Мандат — пересказ; расхождение пересказа с источником сегодня уже стоило нам формата текстового ввода, который ты придумал сам».
2. ВЛАДЕЛЕЦ ДОСЛОВНО про независимость судьи: «где можешь, ставь второго, независимого верификатора сверх позиции: сегодня верификаторы нашли шесть дефектов в P1, шесть в P7, три в P12 — они окупались каждый раз».

## КОНТРАКТ ЗОНЫ (обязателен — не удалять; вписан Cowork)
- **МЕСТО РАБОТЫ:** ветка `zahod/R3-istochniki` в основной папке. 🔴 Она должна УЖЕ стоять. НЕ на ней — СТОП, НЕ делай `git checkout`: в общей папке он МОЛЧА откатывает дерево к состоянию ветки (цена 27→28.07: файл сильно откатился ночью, поймал владелец вручную; след в git НЕ остаётся). Тогда заход пересобрать с `--worktree`. Ветку не переключай, в другие НЕ коммить.
- **ЗОНА (можно менять):** `zhurnal/2026-09-02_spetsmat-bot/kod_R3-istochniki.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/bootstrap_zahod.py --zadanie-subagentu --zone zhurnal/2026-09-02_spetsmat-bot/kod_R3-istochniki.md --kommitit 'только свой файл-заход с отчётом; кода не трогать вовсе' --zakryt 'ветку zahod/R3-istochniki влить в main последним ходом самому'
```
...
- Проверь ветку: `git branch --show-current` — обязано быть `zahod/R3-istochniki`. Не она — СТОП, `git checkout` НЕ делай (§4 GIT-disciplina), нужен `--worktree`.
- Точка отката: `git add zhurnal/2026-09-02_spetsmat-bot/kod_R3-istochniki.md` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
Прочитай четыре исходных документа владельца и живой код, и назови РАСХОЖДЕНИЯ.

**ЧАСТЬ 1 · СОБЕРИ ТРЕБОВАНИЯ ИЗ ИСТОЧНИКОВ, А НЕ ИЗ МАНДАТА.** Выпиши из
четырёх документов всё, что является ТРЕБОВАНИЕМ к поведению бота — не рассуждения
и не примеры, а «должно быть так». Пронумеруй. В отчёт — их ЧИСЛО.

**ЧАСТЬ 2 · ПО КАЖДОМУ — ОДНА СТРОКА ПРИГОВОРА.** Формат строгий:

    N. <требование одной фразой>
       ИСТОЧНИК: <файл:строка>
       В КОДЕ:   <файл:строка, или «НЕ НАЙДЕНО»>
       ПРИГОВОР: сделано | сделано ИНАЧЕ, чем просили | НЕ сделано | не проверяемо
       ЧЕМ ПРОВЕРЕНО: <команда и её вывод>

🔴 «ПРИГОВОР» ОБЯЗАН ОПИРАТЬСЯ НА КОМАНДУ, А НЕ НА ЧТЕНИЕ. Волна сегодня трижды
принимала позиции с зелёными гейтами, которые не работали в бою: пустой
`BOT_TOKEN` при 486 зелёных тестах, зависимость `vision`, не проведённая в
`workflow_data`, при 18 гейтах из 18, и модель зрения, не державшая строгий JSON,
чей конвейер за всю волну ни разу не говорил с живой моделью. Общий корень один:
ТЕСТ ПОДСТАВЛЯЕТ ЗАВИСИМОСТЬ САМ и потому не видит, что в бою её не подставляет
никто. Читая тест, ты ничего не узнаёшь про бой.

**ЧАСТЬ 3 · ОСОБО — ЧЕТЫРЕ МЕСТА, ГДЕ ПЕРЕСКАЗ ВЕРОЯТНЕЕ ВСЕГО РАЗОШЁЛСЯ.**
Проверь их прицельно и первыми: (1) модель данных — что такое отметка, снятие,
явка и как они отличаются; (2) экран приёма задач — что видит преподаватель и в
каком порядке; (3) персональные данные — что именно покидает сервер и в каком
виде; (4) темпоральность — что значит «текущий преподаватель» и «текущий листок».

**ЧАСТЬ 4 · ЧЕГО НЕ ХВАТАЕТ ВОВСЕ.** Требование из источника, которого нет в коде
и нет в мандате, — самая ценная твоя находка: его никто не заметил дважды.

**КРИТЕРИЙ ГОТОВНОСТИ — что обязано провалиться, если работа не сделана:**

    🔴 ЖИВОЙ ПРОГОН НА РЕАЛЬНЫХ ОБЪЕКТАХ: приговоры опираются на команды,
    выполненные в этом репозитории на БОЕВОЙ базе `data/spetsmat.db`
    (56 учеников, 15 847 отметок) и на живом дереве `bot/ core/ infra/`.
    Приговор без вывода команды не считается приговором.

    требований выписано               → ЧИСЛО, не «много»
    приговоров вынесено               → столько же, ни одним меньше
    из них «сделано ИНАЧЕ» и «НЕ сделано» → перечислены поимённо
    заявок поставлено                 → по одной на каждое расхождение

В `## ОТЧЁТ` первой строкой — число требований и число расхождений.

## 3. ВЕРИФИКАТОР (если двигаем/теряем/жмём)

Верификатор не нужен: позиция САМА является независимым верификатором волны против первоисточников; верификатор верификатора удваивает стоимость, а не строгость.
...
```
git --no-optional-locks add -- zhurnal/2026-09-02_spetsmat-bot/kod_R3-istochniki.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- zhurnal/2026-09-02_spetsmat-bot/kod_R3-istochniki.md   # отсекает всё чужое
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone zhurnal/2026-09-02_spetsmat-bot/kod_R3-istochniki.md   # из корня репо; должен быть ✅
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py commit --zone zhurnal/2026-09-02_spetsmat-bot/kod_R3-istochniki.md \
...
**ЗОНА ГИГИЕНЫ:** `zhurnal/2026-09-02_spetsmat-bot/kod_R3-istochniki.md`

- **Г1. Зона доехала в git.** `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone zhurnal/2026-09-02_spetsmat-bot/kod_R3-istochniki.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
...
```
Своя зона — своими путями (`add` + `commit -- zhurnal/2026-09-02_spetsmat-bot/kod_R3-istochniki.md`). Чужая содержательная работа — НЕ твоя:
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py vlit-v-osnovnuyu zahod/R3-istochniki --zone <своя зона> \
...
**КОММИТ:** `<хэш>` — `<сообщение>` · `git_zona.py check --zone zhurnal/2026-09-02_spetsmat-bot/kod_R3-istochniki.md` → ✅
...
**ВЕТКА РАБОТЫ:** `zahod/R3-istochniki`
