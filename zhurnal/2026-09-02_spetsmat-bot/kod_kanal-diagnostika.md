...
**ВЕРДИКТ:** принято — ВТОРОЙ прогон. Артефакты `ops/diagnostika_kanala.py` (13042 б) и `ops/PISMO-HOSTERU.md` (6165 б) есть на диске и в git, влито `e7ee14b`; замер прогнан НА БОЕВОМ сервере — `/tmp/kanal-diagnostika-run1/` несёт 8 файлов и ровно 600 строк проб (три адреса × два протокола); письмо несёт настоящие числа и называет резолвер 198.18.18.18. Проверено оркестратором по ssh, а не по отчёту
