...
### ПРАВКА 1 (11.09 11:32, от оркестратора) — ПРЕПОДАВАТЕЛЬСКАЯ ПОЛОВИНА ЖУРНАЛА ТОЖЕ ТВОЯ

Я упустила это при сборке, дописываю отдельной правкой. Владелец про вкладку «Преподаватели»:

«С преподавателями всё здорово, но тоже сразу сделай вот это: „кто в какой день был, по нему
считается зарплата, цена ошибки в клетках“ — всё, выкидываем это, как и заголовок „журнал
преподавателей“. Выкидываем все комментарии, это ужасно, это нельзя людям показывать. Тоже
подпись „обычное“ убираем. Просто делаем шестнадцать вкладок на ближайшие восемь недель,
понедельники-четверги, только эти даты и всё, и там галочки ставим.»

То есть у ОБЕИХ половин журнала — школьников и преподавателей — одинаково:
* пояснительной прозы над таблицей нет вовсе;
* в шапке колонки только дата, без слова «обычное» и без рода занятия;
* колонки заведены на всю четверть вперёд (понедельники и четверги), пустые — под будущее;
* у преподавателей в клетке — галочка «был», и ничего сверх.

🔴 СОСЕДНЯЯ ПОЗИЦИЯ `otsutstvie-prepodavatelya` РАБОТАЕТ В ТОМ ЖЕ ФАЙЛЕ И ПРЯМО СЕЙЧАС: она
добавляет в клетки преподавательской половины ОТДЕЛЬНЫЙ ВИД «его не будет с … по …». Не
переписывай её клетку и не считай пустоту и отсутствие одним и тем же: пустая клетка значит
«не отмечено», отсутствие — отмечено особо. Если ваши правки столкнутся в одном месте, влей
её ветку к себе и продолжи поверх, а не поверх её работы.
