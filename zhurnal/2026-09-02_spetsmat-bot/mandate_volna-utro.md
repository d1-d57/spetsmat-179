...
ВОСЬМАЯ ПОЗИЦИЯ, `adres-bazy`, добавлена владельцем вечером через десктопный канал, под этот же
мандат: у базы появился АДРЕС вместо имени — отказ вместо фантома вне сервера, метка рода внутри
базы (миграция `011`), одна дверь на запись, ни одного файла базы в репозитории. Пять коммитов,
влиты ею самой (`8906b48`). Ради неё мандат был ВРЕМЕННО ВОЗВРАЩЁН в `OPEN`: он закрывался в
16:5x, но закрытый мандат при живой позиции — ложь в том единственном документе, который читает
закрывающая сессия.
...
- **kod_adres-bazy.md** — принято — все пять пар прогнаны мной, у каждой оба числа: rc=1 без переменной и rc=0 с ней; копия отказывает, боевая свежая своя работает; файлов базы в git 0, подложенный краснеет; pytest 1310 вход → 1328 с переменной (без неё 1268 при честных пропусках «источник не назван»); дверь печатает путь, род, свежесть, вердикт. Вшитых путей к базе в коде не осталось ни одного — перегнано мной
...
Список зоны в заходе не назвал `data/spetsmat.db`, хотя задача велела файл убрать: слияние потребовало объявить его зоной сверх контракта. ДОМ: `bootstrap_zahod.py` — зона обязана покрывать то, что задача велит удалить.
`register_doc.py` отказывается регистрировать `doc/ISTOCHNIK-BAZY.md`: у этого пути нет индекса в реестре корней. ДОМ: реестр корней документации.
