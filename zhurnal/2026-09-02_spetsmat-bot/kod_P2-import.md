...
### ПРАВКА 1 · 2026-09-02 10:55 · доработка ФОРМЫ шапки отчёта, работа принята по существу

Работа сделана, влита мержем `8c1bc79`, `make check` на `main` даёт 148 passed, и находки в
данных (недосчёт гробария по 16б*, дубль `12д` в листке `2д`, нулевой листок `4д`, третий
составной принимающий) — ценные. Доделать надо ровно ДВЕ СТРОКИ ФОРМЫ.

**Ты заменил жёсткие строки каркаса своим заголовком `### КОММИТ`.** Приёмка читает их машинно
и не находит:
...
- `❌ Г3` — воздержался, потому что Г2 не дал пути.

**Верни обе строки в каркасной форме, в начале `## ОТЧЁТ`:**

    **АРТЕФАКТ:** `<абсолютный путь>` — `<чем открывать>`
    **РОД АРТЕФАКТА:** `исходник`
    **КОММИТ:** `<хэш>` — `<сообщение>` · `git_zona.py check --zone …` → ✅

`РОД АРТЕФАКТА: исходник` здесь верен: продукт захода — КОД, он коммитится РАНЬШЕ отчёта,
и Г3 тогда сверяет не время, а «доехал ли артефакт в названный коммит». Артефактом назови
`tools/import_konduit.py` — то, что владелец открывает и запускает.

Свой раздел `### КОММИТ` с девятью коммитами оставь как есть, он полезен: каркасные строки
идут ДОПОЛНИТЕЛЬНО, в начале секции.

🔴 **НИЧЕГО, КРОМЕ ЭТОГО, НЕ ДЕЛАЙ.** Ни строки кода, ни теста, ни рефактора. Правка ФОРМЫ.
⚠ Твою правку `config.py` вне зоны оркестратор ПРИНЯЛ: §7 задания прямо велел положить туда
константу, а зону выдал слишком узкой оркестратор — это его ошибка, не твоя.
