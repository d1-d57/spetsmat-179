# Канал исполнителя — N6-stranica-pages-i-zaglushka (один заход до конца)
...
```
cd /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot && python3 /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot/_generator/tools/orkestr.py zhurnal/2026-09-02_spetsmat-bot --rezhim progon --dvizhok opencode --rod instrumenty --model openrouter/z-ai/glm-5.2:free --zahody kod_N6-stranica-pages-i-zaglushka.md < /dev/null 2>&1 | tee /tmp/zahod-N6-stranica-pages-i-zaglushka.log
```
...
Приоритет владельца: разобрать инциденты важнее, потом закрыть долги — неразобранный инцидент это повторяющаяся ошибка, долг может подождать.
  ...
  4. пунктов очереди «ДОСТАВЛЕНО: нет»   : 216
...
ЦЕЛЬ: собрать страницу Pages со снимком распределения и честной заглушкой на случай молчащего туннеля.
Приёмка — по ОТЧЁТУ, без построчной сверки. Стоп до цели: ты ПОДГОТОВИШЬ файлы, но НЕ опубликуешь их — spetsmat-179 сейчас не git-репозиторий вовсе, заводит его владелец в позиции N7.
...
3. постоянная ссылка, которую рассылают преподавателям — это адрес Pages, а НЕ адрес туннеля; туннель живёт за ней и его адрес переменная
4. падение динамической половины обязано быть видимым и объяснённым — это финализировано владельцем ранее; вместо ошибки браузера преподаватель видит нашу страницу

## КОНТРАКТ ЗОНЫ (обязателен — не удалять; вписан Cowork)
- **МЕСТО РАБОТЫ:** ветка `zahod/N6-pages` в основной папке. 🔴 Она должна УЖЕ стоять. НЕ на ней — СТОП, НЕ делай `git checkout`: в общей папке он МОЛЧА откатывает дерево к состоянию ветки (цена 27→28.07: файл сильно откатился ночью, поймал владелец вручную; след в git НЕ остаётся). Тогда заход пересобрать с `--worktree`. Ветку не переключай, в другие НЕ коммить.
- **ЗОНА (можно менять):** `../spetsmat-179/index.html` `../spetsmat-179/build.py` `../spetsmat-179/snimok.json` `tools/opublikovat_snimok.py` `tests/veb/test_snimok.py` `zhurnal/2026-09-02_spetsmat-bot/kod_N6-stranica-pages-i-zaglushka.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/bootstrap_zahod.py --zadanie-subagentu --zone ../spetsmat-179/index.html --zone ../spetsmat-179/build.py --zone ../spetsmat-179/snimok.json --zone tools/opublikovat_snimok.py --zone tests/veb/test_snimok.py --kommitit 'зона выходит в СОСЕДНИЙ репозиторий spetsmat-179: коммитить там ОТДЕЛЬНО, зону spetsmat-bot не смешивать' --zakryt 'ветку zahod/N6-pages после переноса'
```
...
- Проверь ветку: `git branch --show-current` — обязано быть `zahod/N6-pages`. Не она — СТОП, `git checkout` НЕ делай (§4 GIT-disciplina), нужен `--worktree`.
- Точка отката: `git add ../spetsmat-179/index.html ../spetsmat-179/build.py ../spetsmat-179/snimok.json tools/opublikovat_snimok.py tests/veb/test_snimok.py zhurnal/2026-09-02_spetsmat-bot/kod_N6-stranica-pages-i-zaglushka.md` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
Верификатор нужен, тип — **ПОСЛЕ-типа** — судит результат, стоит в конце, после задачи. Свежий субагент, ДРУГИМ методом (проверить заглушку ПОДМЕНОЙ адреса на заведомо недоступный и посмотреть глазами, что видит преподаватель, а не убедиться, что код заглушки написан), не перечитывает свою же правку. Доля сплошной выборки: обе половины страницы: снимок и заглушка, 2 из 2. Финальная строка ответа обязательна дословно: «выдано N позиций из M найденных» — без неё ответ считается усечённым и в отчёт не вставляется.
...
```
git --no-optional-locks add -- ../spetsmat-179/index.html ../spetsmat-179/build.py ../spetsmat-179/snimok.json tools/opublikovat_snimok.py tests/veb/test_snimok.py zhurnal/2026-09-02_spetsmat-bot/kod_N6-stranica-pages-i-zaglushka.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- ../spetsmat-179/index.html ../spetsmat-179/build.py ../spetsmat-179/snimok.json tools/opublikovat_snimok.py tests/veb/test_snimok.py zhurnal/2026-09-02_spetsmat-bot/kod_N6-stranica-pages-i-zaglushka.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `../spetsmat-179/index.html` `../spetsmat-179/build.py` `../spetsmat-179/snimok.json` `tools/opublikovat_snimok.py` `tests/veb/test_snimok.py` `zhurnal/2026-09-02_spetsmat-bot/kod_N6-stranica-pages-i-zaglushka.md`

- **Г1. Зона доехала в git.** `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone ../spetsmat-179/index.html` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone ../spetsmat-179/build.py` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone ../spetsmat-179/snimok.json` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone tools/opublikovat_snimok.py` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone tests/veb/test_snimok.py` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone zhurnal/2026-09-02_spetsmat-bot/kod_N6-stranica-pages-i-zaglushka.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py vlit-v-osnovnuyu zahod/N6-pages --zone <своя зона> \
