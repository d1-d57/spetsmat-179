...
### ЗАПИСЬ 4 · 2026-09-04 18:33 · СРОЧНАЯ ПРАВКА ПРО «НС» — ПРОВЕРЕНА И ОТКЛОНЕНА ПО МЕХАНИЗМУ, но нашлось настоящее

Аналитик передал: «страница отдаёт „Агаркова · НС · 203“, это `teachers.aka`, а выводить
обязано `teachers.name` («Наталья Яковлевна»). Правка на одно поле».
**Проверил прежде, чем править. Диагноз неверен, правка на одно поле не нужна.**

| что проверял | команда | что вышло |
|---|---|---|
| использует ли интерфейс `aka` | `grep -rn "aka" veb/` | **НИ РАЗУ** — ни в `server.py`, ни в шаблонах |
| чем сервер берёт имя | `grep -n "select id, name from teachers"` | `veb/server.py:143` — берёт **`name`**, а не `aka` |
| что показывает ЖИВАЯ страница | `curl <адрес>/api/view?slot=1` | 16 имён, **все полные**: «Александр Александрович Тертерян», «Наталья Яковлевна», «Елена Юрьевна», «Полина»… Инициалов НЕТ НИ ОДНОГО |

**Откуда взялось «НС».** Это не `aka`, а САМО ПОЛЕ `name` у преподавателя `id=19`:
`id=19 name='НС' aka='НС'`. То есть инициалы лежат в ДАННЫХ, а не подставляются кодом.
И «Наталья Яковлевна» — это ДРУГОЙ человек, `id=11`, `aka='НА'`. Подменить одно другим
было бы не исправлением, а припиской чужого имени.
Сейчас у `id=19` **ноль школьников**, поэтому на странице он и не появляется.
«Агаркова · НС · 203» — снимок ДО миграции; после неё у Агарковой преподаватель `id=14`, Полина.

🔴 **ЧТО НАСТОЯЩЕЕ И ЧТО НАДО РЕШИТЬ ВЛАДЕЛЬЦУ — вопрос [V6].**
В таблице `teachers` две мусорные строки, и обе попадут в список выбора у организатора:
* `id=18` · `name='отсутствует'` · `aka='x'` — заглушка, не человек;
* `id=19` · `name='НС'` · `aka='НС'` — инициалы вместо имени; кто это, я не знаю и
  **угадывать не буду**: рядом есть «Наталья Яковлевна» (11) и «Наталия Павлована» (12),
  и приписать не тому человеку хуже, чем оставить как есть.
* ⚠ Заодно: `id=12` записан как **«Наталия Павлована»** — похоже на опечатку в отчестве
  («Павловна»). Тоже не правлю сам: имя человека.
...
Пять позиций, собранных генератором и дозаполненных, все на бесплатных моделях.
**Запусков потребовалось 13 на 5 позиций** — семь отказов моделей:

| позиция | попытки | итог |
|---|---|---|
| S1 `slot-vmesto-weekday-i-routy` | z-ai (квота) · minimax-m3 (зависла на 35-й мин, 0.9% CPU) · deepseek (имя без слэша ушло в чужой движок) · **minimax-m2.7** | влита, добор починил регрессию |
| S2 `sostav-klassa-i-112-strok` | north-mini-code (23 мин правила свой файл-заход, работы ноль) · **dots** | инструмент сделан, добор идёт |
| S3 `glavnaya-i-stranicy-listkov` | laguna (квота) · gpt-oss-120b (лимит 8000 токенов при заходе 8515) · **dots** | влита, всё сделано |
| S4 `postoyannyj-adres-pages-i-tunnel` | nemotron-3-super (6 мин, пустой возврат) · devstral (недоступна) · **inkling** ×2 | влита |
| S5 `vhod-organizatora-dorabotka` | **inkling** | влита с первого раза |

**Работали:** inkling (3 позиции), dots (2), minimax-m2.7 (1).
**Не дожили до работы:** z-ai, laguna, devstral, nemotron-3-super, gpt-oss-120b, minimax-m3, deepseek.

### WHAT IT REPAIRED ITSELF AND WHY IT WAS BROKEN

**Волна чинила себя семь раз, и каждый раз ломалось УСТРОЙСТВО РАБОТЫ, а не предмет.**

1. **Хук молчал** — имена файлов-заходов разошлись с составом, плюс `VOLNA-POZICII` от прошлой волны вычёркивал все позиции. Надзора не было бы всю волну. Починено на 3-м круге прогоном хука.
2. **База невидима из worktree** — `.gitignore` исключает `data/` (правильно, там дети). Три позиции из пяти не могли работать с данными и не могли догадаться. Снят резерв, розданы копии.
3. **`ADRES.txt` вне git** — S4 честно доложила «адреса нет», не имея возможности его увидеть. Забран в git.
4. **Критерий S5 невыполним в её зоне** — шлагбаум входа жил в `veb/server.py`, которого в зоне не было. Зона расширена ДО запуска.
5. **Зона S1 накрывала 2 файла из 8**, где живёт `weekday`. Влитие уронило тесты `main` 18→9. Зона расширена, добор починил, снова 18 passed.
6. **`/glavnaya` давал 404** — шаблоны ссылались на роут, которого контракт не предусматривал. Добавлен синонимом корня.
7. **Постоянный адрес туннелем недостижим** — не починка, а опровержение допущения, прожившего с ночи.
...
* **GitHub Pages** — требует учётки владельца. Оформлено вопросом [V1], не «не успели».
* **Достройка 112 строк** — зависит от ответа на [V4]: пока не сказано, участвуют ли трое без строк, число 2N не определено. Волне ЗАПРЕЩЕНО выбирать за владельца — ровно на совпавшем числе ночью выключилась проверка.
* **Три позиции ночной волны** (вход преподавателя через Telegram, приём задач, кондуит) — граница, объявленная в составе.
* **Починка stop-хука** — живёт в `disciplina`, там своя волна.

### IRREVERSIBLE ACTIONS

**1. Миграция на ЖИВУЮ базу.** `enrollment.weekday` → `enrollment.slot`, применена штатным yoyo.
Резерв снят ДО: `data/REZERV-pered-volnoj-VECHER-1753.db`, 3 874 816 байт.
Было: `weekday`, 56 строк. Стало: `slot`, 56 строк, раскладка `{1: 53, 2: 3}`.
Позициям живая база НЕ отдавалась — им розданы копии, чтобы две параллельные записи не повредили данные класса.

**2. Правка живых данных при проверке — сделана и ВЕРНУТА.** Проверяя правку организатора, менял Агарковой преподавателя «Полина → Ваня». Возврат через интерфейс отдал `409`, вернул в базе руками: удалил строку `id=58`, вернул `valid_to='9999-12-31'` строке `id=2`. После возврата: 56 строк, «Агаркова · Полина · каб 203», раскладка `{1: 53, 2: 3}` — состояние в точности исходное.

**3. Туннель поднимался мной** и был остановлен. Наружу открывался адрес с данными класса на время проверки. Репозиторий публичный по решению владельца [F3].

**Ничего не удалено безвозвратно.** `VOLNA-POZICII` не удалён, а переименован в `VOLNA-POZICII.ustarel-volna-sajt`.
...
- [V4] **ТРОЕ ДЕТЕЙ БЕЗ ЕДИНОЙ СТРОКИ РАСПРЕДЕЛЕНИЯ — они в курсе или нет?**
  ...
  **у 3 нет ни одной**. Ваше число «53» в точности равно числу детей, у кого есть хоть
  одна строка. ТРОЕ, ПОИМЁННО, снято командой 18:22 — все `active`:
  * **Аракелова Дарья**, id 3 — 🔴 **КЛАСС НЕ УКАЗАН ВООБЩЕ** (это и есть «один без класса»);
  * **Гамаюнова Софья**, id 14, 9К;
  * **Сухов Даниил**, id 45, 9Л.
  На сайте все трое видны с пустым преподавателем и пустым кабинетом.
  ФОРМА ОТВЕТА, одной строкой: «эти трое в курсе не участвуют» · или «участвуют,
  распредели их» · или отдельно по каждому. Про Аракелову скажите ещё класс.

  ⚠ СВЯЗАННОЕ, не отдельный вопрос: изменения состава ещё НЕ ПРИМЕНЕНЫ — Емельянцев
  Всеволод (id 21, 9К) и Майоров Вячеслав (id 33, 9Л) по-прежнему `active`, Ишкаева
  Владислава в базе нет. Это остаток работы S2, она в доборе.

- [V6] **ДВЕ МУСОРНЫЕ СТРОКИ В СПИСКЕ ПРЕПОДАВАТЕЛЕЙ — назовите, что с ними.**
  ЧТО ЗАБЛОКИРОВАНО: ничего на странице (у обеих ноль школьников), но обе попадут
  в список выбора у организатора, когда он будет назначать.
  ЧТО ЗАМЕРЕНО: `id=18 name='отсутствует' aka='x'` · `id=19 name='НС' aka='НС'`.
  Рядом живут «Наталья Яковлевна» (11) и «Наталия Павлована» (12) — угадывать, кто из
  них «НС», я отказался: приписать чужое имя хуже, чем оставить инициалы.
  ФОРМА ОТВЕТА: «19 это <полное имя>, 18 убрать» · или «обе убрать из активных» ·
  или «оставь как есть».
  ⚠ Отдельно: у `id=12` в базе «Наталия **Павлована**» — похоже на опечатку в отчестве.
  Имя человека сам не правлю.
...
`none` — все 29 уроков судимы свежим заходом на `inkling` 18:22, ни один не остался без вердикта.
Раскладка: **ПРАВИЛО 25 · ДОЛГ 4 · ОТКЛОНЁН 0**. Из 25 правил у 23 назван носитель;
**У1 и У7 объявлены НАДЕЖДАМИ судьёй**, письменно, по нашему же закону — покраснеть при их
нарушении нечему.
...
- [F1] **ЧАСТИЧНО.** Распределение — **СДЕЛАНО и годно для отправки коллегам**: `/raspredelenie` → 200 без логина, «Агаркова Ирина · Полина · каб 203», 56 школьников, правка организатором работает и видна посторонним. Постоянный адрес — **НЕ СДЕЛАНО, и не по недоделке**: прогнал критерий владельца дословно и получил, что туннель его не даёт — перезапуск процесса меняет поддомен (`598c6edac618b6` → `f0a4bf9fecabd2`). Нужен Pages или платный поддомен, оба требуют учётки владельца. Вопрос [V1].
- [F2] **СДЕЛАНО.** Логин нужен ровно для одного: `POST /api/enrollment` без куки → 302 (отказ), с кукой организатора → 200 и правка видна. Все шесть страниц читаются БЕЗ логина: `/` `/glavnaya` `/raspredelenie` `/listki` `/listki-8` `/urovni` → 200. До волны все они отдавали 302 на форму входа.
- [F3] **СОБЛЮДЕНО.** Репозиторий публичный, приватным не делался. Пароли в истории git остались известным долгом, волну не держали — как и было решено. Про них ничего не предпринималось, чтобы не выдать долг за закрытый.
- [F4] **СОБЛЮДЕНО.** Фамилии на страницах остались: страница распределения показывает «Агаркова», «Аникина», «Аракелова» и так далее. Прежнее решение не пересматривалось.
- [F5] **СОБЛЮДЕНО, и трижды это решило спор в пользу надёжности.** (1) Роут отдаёт 200-заглушку, а не 500, когда шаблона ещё нет. (2) `/glavnaya` добавлен прощающим синонимом корня — 404 в шапке дороже лишней строки. (3) Живая база позициям НЕ отдана, только копии: две параллельные записи могли повредить данные класса. Плюс инструмент проверки состава НЕ подключён к автоматическому вызову — он краснел бы на каждом прогоне, пока числа не сойдутся, и приучил бы к красному.
- [F6] **ИСПОЛНЕНО В ЧАСТИ «ПРЕДЪЯВИТЬ», НЕ ИСПОЛНЕНО В ЧАСТИ «ПРИМЕНИТЬ».** Расхождение НАПЕЧАТАНО и не замазано: инструмент `tools/proverka_sostava.py` печатает все пять спорных чисел рядом (56 · 53 · 55 · 53 · 54), называет расхождение словами и краснеет кодом `1`. Волна за владельца не выбрала — это прямое требование пункта. Сами изменения состава на 18:24 НЕ применены: Емельянцев (id 21) и Майоров (id 33) по-прежнему `active`, Ишкаева в базе нет; строк 56 вместо 112. Причина не в нехватке времени: число 2N не определено, пока не сказано, участвуют ли трое без единой строки — **Аракелова Дарья** (id 3, **класс не указан**), **Гамаюнова Софья** (id 14, 9К), **Сухов Даниил** (id 45, 9Л). Вопрос [V4].
