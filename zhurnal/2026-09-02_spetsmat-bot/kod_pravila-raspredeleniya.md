...
**ВЕРДИКТ:** принято — `tests/enrollment/test_calendar_and_ceiling.py` 10 passed, имена покрывают обе клаузы дословно (F1 потолок пяти, F2 запрет дня неприхода), отдельным тестом закрыт стык с сервером. Влито `5cfb506`. Два живых нарушения НЕ тронуты намеренно — вынесены владельцу вопросом [V3]
