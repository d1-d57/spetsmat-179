...
**ВЕРДИКТ:** принято — 1132 строки, влито `ab72ceb`, `tests/veb/test_vnesenie.py` 12 passed. Клауза F10 закрыта по ВСЕМ ТРЁМ каналам, и это видно по именам тестов: `tekst_`, `foto_`, `golos_tri_vnesenia_pishut_rovno_podtverzhdyonnoe`. Запрет записи без подтверждения закрыт отдельно — `draft_dver_nichego_ne_pishet`
