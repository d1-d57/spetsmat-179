# Канал исполнителя — N5-konduit-proshlogo-goda (один заход до конца)
...
```
cd /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot && python3 /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot/_generator/tools/orkestr.py zhurnal/2026-09-02_spetsmat-bot --rezhim progon --dvizhok opencode --rod instrumenty --model openrouter/z-ai/glm-5.2:free --zahody kod_N5-konduit-proshlogo-goda.md < /dev/null 2>&1 | tee /tmp/zahod-N5-konduit-proshlogo-goda.log
```
...
Приоритет владельца: разобрать инциденты важнее, потом закрыть долги — неразобранный инцидент это повторяющаяся ошибка, долг может подождать.
  ...
  4. пунктов очереди «ДОСТАВЛЕНО: нет»   : 213
...
ЦЕЛЬ: открыть на чтение архив прошлого года: 18 листков, 56 учеников, 15847 событий.
Приёмка — по ОТЧЁТУ, без построчной сверки. Стоп до цели: ты получишь чтение архива, но НЕ тронешь каталог листков spetsmat-179 — это соседний репозиторий и предмет позиции N7.
...
3. число 15847 снято командой заново в сессии 04.09; в старом заходе стояло 15112 — это только assert, без retract
4. каталог листков spetsmat-179 НЕ трогать: это соседний репозиторий и предмет позиции N7

## КОНТРАКТ ЗОНЫ (обязателен — не удалять; вписан Cowork)
- **МЕСТО РАБОТЫ:** ветка `zahod/N5-konduit` в основной папке. 🔴 Она должна УЖЕ стоять. НЕ на ней — СТОП, НЕ делай `git checkout`: в общей папке он МОЛЧА откатывает дерево к состоянию ветки (цена 27→28.07: файл сильно откатился ночью, поймал владелец вручную; след в git НЕ остаётся). Тогда заход пересобрать с `--worktree`. Ветку не переключай, в другие НЕ коммить.
- **ЗОНА (можно менять):** `veb/konduit.py` `veb/static/konduit.css` `tests/veb/test_konduit.py` `zhurnal/2026-09-02_spetsmat-bot/kod_N5-konduit-proshlogo-goda.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/bootstrap_zahod.py --zadanie-subagentu --zone veb/konduit.py --zone veb/static/konduit.css --zone tests/veb/test_konduit.py --kommitit 'каждые 20 минут, даже незаконченное' --zakryt 'ветку zahod/N5-konduit после переноса'
```
...
- Проверь ветку: `git branch --show-current` — обязано быть `zahod/N5-konduit`. Не она — СТОП, `git checkout` НЕ делай (§4 GIT-disciplina), нужен `--worktree`.
- Точка отката: `git add veb/konduit.py veb/static/konduit.css tests/veb/test_konduit.py zhurnal/2026-09-02_spetsmat-bot/kod_N5-konduit-proshlogo-goda.md` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
Верификатор нужен, тип — **ПОСЛЕ-типа** — судит результат, стоит в конце, после задачи. Свежий субагент, ДРУГИМ методом (сверить число сданных на странице с проекцией progress.py, а не с count(*) по marks), не перечитывает свою же правку. Доля сплошной выборки: все 18 листков, 18 из 18. Финальная строка ответа обязательна дословно: «выдано N позиций из M найденных» — без неё ответ считается усечённым и в отчёт не вставляется.
...
```
git --no-optional-locks add -- veb/konduit.py veb/static/konduit.css tests/veb/test_konduit.py zhurnal/2026-09-02_spetsmat-bot/kod_N5-konduit-proshlogo-goda.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- veb/konduit.py veb/static/konduit.css tests/veb/test_konduit.py zhurnal/2026-09-02_spetsmat-bot/kod_N5-konduit-proshlogo-goda.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `veb/konduit.py` `veb/static/konduit.css` `tests/veb/test_konduit.py` `zhurnal/2026-09-02_spetsmat-bot/kod_N5-konduit-proshlogo-goda.md`

- **Г1. Зона доехала в git.** `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone veb/konduit.py` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone veb/static/konduit.css` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone tests/veb/test_konduit.py` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone zhurnal/2026-09-02_spetsmat-bot/kod_N5-konduit-proshlogo-goda.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py vlit-v-osnovnuyu zahod/N5-konduit --zone <своя зона> \
