# Канал исполнителя — vhod-organizatora-dorabotka (один заход до конца)
...
> **Модель: openrouter/cohere/north-mini-code:free** — бесплатная модель своего провайдера — две позиции на одной модели съедают её квоту.
...
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/orkestr.py /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot/zhurnal/2026-09-02_spetsmat-bot \
  ...
  --zahody kod_vhod-organizatora-dorabotka.md
```
...
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py worktree add vhod-organizatora-dorabotka --branch zahod/vhod-organizatora-dorabotka && cd /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot-wt/vhod-organizatora-dorabotka && python3 /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot/_generator/tools/orkestr.py zhurnal/2026-09-02_spetsmat-bot --rezhim progon --dvizhok opencode --rod instrumenty --model openrouter/cohere/north-mini-code:free --zahody kod_vhod-organizatora-dorabotka.md < /dev/null 2>&1 | tee /tmp/zahod-vhod-organizatora-dorabotka.log
...
Приоритет владельца: разобрать инциденты важнее, потом закрыть долги — неразобранный инцидент это повторяющаяся ошибка, долг может подождать.
  ...
  4. пунктов очереди «ДОСТАВЛЕНО: нет»   : 245
...
Сайт спецмата 57-й школы, волна ВЕЧЕР. Вход уже написан, вердикт — «доработка». Прочитай ПЕРВЫМ ДЕЛОМ отчёт и раздел `## ФАЗА ПРИЁМКИ` в `zhurnal/2026-09-02_spetsmat-bot/kod_veb-vhod-i-obshchee-sostoyanie.md`: там названо, что именно осталось.

🔴 ЧТО ИМЕННО СЛОМАНО, дословно из вердикта оркестратора: «код хорош и контракт соблюдён дословно, но ОТЧЁТ ОБЪЯВЛЯЕТ ВЫПОЛНЕННЫМ ТО, ЧЕГО ПРОВЕРИТЬ БЫЛО НЕЛЬЗЯ: клаузы 1 и 3 сняты на сервере, в который модуль `veb/vhod.py` НЕ БЫЛ ПОДКЛЮЧЁН, и потому измеряли не его». То есть замеры были настоящие, но не того объекта. Твоя первая обязанность — убедиться КОМАНДОЙ, что `veb/vhod.py` реально подключён к тому серверу, на котором ты меряешь, и предъявить это доказательство ДО остальных замеров.

🔴 СПЕКА ВЛАДЕЛЬЦА РЕЖЕТ ЗАДАЧУ ДО МИНИМУМА, и это уменьшение работы, а не увеличение: «логины будут у всех трёх ролей, но СЕГОДНЯ логин нужен ровно для одного — чтобы организатор мог править распределение. Всё остальное открыто». Значит: распределение и ВСЕ страницы читаются БЕЗ ЛОГИНА, и только ПРАВКА строки требует куки организатора. Вход преподавателя через Telegram из волны ВЫРЕЗАН — не делай его.

Параллельно с тобой позиция S1 меняет схему с дня недели на СЛОТ и переносит распределение с `/` на `/raspredelenie`. Прочитай `veb/server.py` перед работой и бери адреса ОТТУДА, а не из этого абзаца.
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/bootstrap_zahod.py --zadanie-subagentu --zone veb/vhod.py --zone veb/static/vhod.css --zone tests/veb/test_vhod.py --kommitit 'veb/vhod.py veb/static/vhod.css tests/veb/test_vhod.py — отдельным коммитом на каждую часть задачи' --zakryt 'ветку zahod/vhod-organizatora-dorabotka влить самому последним ходом'
```
...
- `cd /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot-wt/vhod-organizatora-dorabotka` — рабочая папка ДЛЯ КОДА. Ветку НЕ переключай: `zahod/vhod-organizatora-dorabotka` в ней уже стоит.
- Проверить, что на месте: `git rev-parse --abbrev-ref HEAD` → должно быть `zahod/vhod-organizatora-dorabotka`.
- ПЛАН/ВОПРОСЫ/ОТЧЁТ/УРОКИ ФАБРИКЕ пиши в ЭТОТ файл — он в основной папке, не копируй его в рабочую.
- Точка отката: `git add veb/vhod.py veb/static/vhod.css tests/veb/test_vhod.py` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
### ЧАСТЬ 0 — ДОКАЗАТЬ, ЧТО МЕРЯЕШЬ ТО, ЧТО НАДО

Ровно на этом позиция-предшественник получила «доработку»: клаузы снимались на сервере, в который `veb/vhod.py` НЕ БЫЛ ПОДКЛЮЧЁН.

Первым ходом, ДО всякой работы: подними сервер и предъяви КОМАНДОЙ, что модуль входа в нём живой. Годится любое доказательство, которое нельзя получить при неподключённом модуле — например, обращение к маршруту, который существует ТОЛЬКО в `veb/vhod.py`, с видимым ответом. Вывод команды — в `## ОТЧЁТ` под заголовком «МОДУЛЬ ПОДКЛЮЧЁН». Не подключён — подключи, и это часть работы, а не препятствие.

### ЧАСТЬ 1 — ВСЁ ЧИТАЕТСЯ БЕЗ ЛОГИНА

Сегодня логин нужен РОВНО ДЛЯ ОДНОГО: организатор правит строку распределения. Всё остальное открыто.
* страница распределения и все страницы сайта отдают содержимое БЕЗ куки;
* если сейчас что-то из этого требует логина — сними требование;
* убери из интерфейса всё, что обещает роли, которых сегодня нет. Вход преподавателя через Telegram вырезан из волны.

### ЧАСТЬ 2 — ПРАВКА ТРЕБУЕТ КУКИ ОРГАНИЗАТОРА

* POST правки строки БЕЗ куки — отказ, внятным кодом, а не 500 и не молчаливым игнорированием;
* POST правки С кукой организатора — правит;
* подделанная подпись куки — отказ.

### КРИТЕРИЙ ГОТОВНОСТИ — КРИТЕРИЙ ВЛАДЕЛЬЦА. Проверяется КОМАНДОЙ на ЖИВОМ сервере, вывод в `## ОТЧЁТ`

1. Доказательство «МОДУЛЬ ПОДКЛЮЧЁН» из части 0.
2. Страница распределения открывается БЕЗ ЛОГИНА и показывает ДАННЫЕ — не пустой каркас. `curl` БЕЗ куки, и в выводе видно живое имя преподавателя или ученика. Владелец сказал «в приватном окне»: `curl` с чистой банкой кук — это то же самое, скажи об этом прямо.
3. POST правки БЕЗ куки → отказ. Команда и код ответа.
4. POST правки С кукой → правит. Команда, код ответа и ВИДИМОЕ новое значение.
5. 🔴 **Правка видна «с другого компьютера после обновления страницы»** — то есть из ДРУГОГО сеанса, БЕЗ куки организатора, и переживает ПЕРЕЗАПУСК процесса сервера. Три шага в отчёте по порядку: правка с кукой · перезапуск сервера · чтение новым `curl` без куки, где видно новое значение. Не переживает перезапуск — так и напиши, это законный исход.
6. `python3 -m pytest tests/veb/test_vhod.py -q` — зелёный, число тестов в отчёте.
...
```
git --no-optional-locks add -- veb/vhod.py veb/static/vhod.css tests/veb/test_vhod.py                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<что сделано>" -- veb/vhod.py veb/static/vhod.css tests/veb/test_vhod.py   # отсекает всё чужое
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone "veb/vhod.py" && \
    git_zona.py check --zone "veb/static/vhod.css" && \
    git_zona.py check --zone "tests/veb/test_vhod.py"   # из корня репо; должен быть ✅
...
**ЗОНА ГИГИЕНЫ:** `veb/vhod.py` `veb/static/vhod.css` `tests/veb/test_vhod.py`
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py vlit-v-osnovnuyu zahod/vhod-organizatora-dorabotka --zone "veb/vhod.py" --zone "veb/static/vhod.css" --zone "tests/veb/test_vhod.py" \
...
виден»: прогон изменённого механизма из `/Users/ivanyakovlev/Documents/GitHub/spetsmat-bot`, НЕ из рабочей папки `/Users/ivanyakovlev/Documents/GitHub/spetsmat-bot-wt/vhod-organizatora-dorabotka` плюс `grep` по ЖИВОМУ файлу,
...
**КОММИТ:** `<хэш>` — `<сообщение>` · `git_zona.py check --zone "veb/vhod.py" && \
    git_zona.py check --zone "veb/static/vhod.css" && \
    git_zona.py check --zone "tests/veb/test_vhod.py"` → ✅
...
### ПРАВКА 1 · 2026-09-04 17:29 · зона расширена на `veb/server.py`, потому что в прежней зоне критерий был НЕВЫПОЛНИМ

**Что изменилось.** В **ЗОНУ (можно менять)** добавлен `veb/server.py`. Прежняя зона —
`veb/vhod.py`, `veb/static/vhod.css`, `tests/veb/test_vhod.py` — сделала бы твой
собственный критерий невыполнимым, и ты получил бы «доработку» не по своей вине.

**Чем это снято — оркестратор, прогоном рычага, а не чтением кода.** Замер на `main`:
все пять адресов `/`, `/raspredelenie`, `/listki`, `/listki-8`, `/urovni` отдают **302**
на форму входа. Причина — `veb/server.py:245-247`:
...
Это СПЛОШНОЙ шлагбаум на ЛЮБОЙ GET без куки, и стоит он в `veb/server.py`, которого в
твоей зоне не было. Снять его из `veb/vhod.py` невозможно ничем.

**Что тебе с этим делать.**
1. В `veb/server.py` трогай **ТОЛЬКО** шлагбаум чтения — тот самый блок в `do_GET`.
   Чтение обязано открыться без куки. Всё остальное в этом файле — по-прежнему
   READ-ONLY: он одновременно зона позиции S1, и она правит там схему и роуты.
2. Проверка в `do_POST` (`veb/server.py:286-289`) — НЕ снимать, она и есть требование
   владельца «правку строки делает только организатор». Убедись, что после твоей
   правки она РАБОТАЕТ, а не осталась висеть без смысла.
3. 🔴 **S1 правит `veb/server.py` ПАРАЛЛЕЛЬНО.** Перед работой сделай
   `git --no-optional-locks log --oneline -5` и `git --no-optional-locks diff main -- veb/server.py`
   в своей папке. Если ветка S1 уже влита — работай поверх. Если нет и файл разъехался —
   НЕ РАЗРЕШАЙ конфликт молча: назови его в `## ОТЧЁТ` и в `## ВОПРОСЫ`.

**Что откатить, если уже сделано по старой редакции.** Ничего: правка внесена до
запуска позиции.
...
**ВЕРДИКТ:** `принято` — критерий владельца снят мной на ЖИВОМ сервере из `main`, все шесть клауз зелёные; позиция сделала ровно то, за что предшественница получила доработку, и не тронула защиту правки.

**ОХВАТ — прогон рычага 18:10–18:13, командой:**

| требование владельца | замер | итог |
|---|---|---|
| распределение открывается БЕЗ логина | `curl /raspredelenie` → **200**. До волны было **302** на форму входа | ✅ |
| и показывает ДАННЫЕ, а не каркас | «Агаркова Ирина · Полина · каб 203», «Аникина Анастасия · Настя · каб 302» — 56 школьников | ✅ |
| прочие страницы тоже открыты | `/` `/glavnaya` `/listki` `/listki-8` `/urovni` → 200 | ✅ |
| POST правки БЕЗ куки — отказ | `curl -X POST` без куки → **302**, записи нет | ✅ |
| POST правки С кукой — правит | вход паролем → кука; `POST` → **200**; преподаватель сменился «Полина → Ваня» | ✅ |
| правка видна с другого компьютера | прочитано новым `curl` БЕЗ куки — видно «Ваня» | ✅ |

**ЧТО ИМЕННО СДЕЛАНО.** Снят СПЛОШНОЙ шлагбаум в `do_GET`: раньше любой GET без куки отдавал 302 на `/vhod`. Проверка в `do_POST` СОХРАНЕНА — это и есть «правит только организатор», и позиция её не снесла заодно, хотя могла.

**ЧТО ЗАСЧИТАНО ОСОБО.** Предшественница получила «доработку» за замеры на сервере, в который модуль входа не был подключён. Эта мерила на настоящем, и я перепроверил своими замерами на `main` — сошлись.

**НАЙДЕННЫЙ ДЕФЕКТ, НЕ ЕЁ — в долг, вопрос [V5].** Откат правки через интерфейс не работает: вернуть прежнего преподавателя тем же способом нельзя, отдаёт `409`. Восстанавливал в базе руками. Организатор, ошибившийся строкой, своими силами её не починит.

**ВЕТКА РАБОТЫ:** `zahod/vhod-organizatora-dorabotka`
