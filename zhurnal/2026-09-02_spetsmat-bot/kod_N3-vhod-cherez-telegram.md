# Канал исполнителя — N3-vhod-cherez-telegram (один заход до конца)
...
```
cd /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot && python3 /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot/_generator/tools/orkestr.py zhurnal/2026-09-02_spetsmat-bot --rezhim progon --dvizhok opencode --rod instrumenty --model openrouter/z-ai/glm-5.2:free --zahody kod_N3-vhod-cherez-telegram.md < /dev/null 2>&1 | tee /tmp/zahod-N3-vhod-cherez-telegram.log
```
...
Приоритет владельца: разобрать инциденты важнее, потом закрыть долги — неразобранный инцидент это повторяющаяся ошибка, долг может подождать.
  ...
  4. пунктов очереди «ДОСТАВЛЕНО: нет»   : 207
...
ЦЕЛЬ: сделать вход преподавателя по одноразовой ссылке от бота, без преподавательского пароля.
Приёмка — по ОТЧЁТУ, без построчной сверки. Стоп до цели: ты получишь работающий МЕХАНИЗМ на фикстурах, но НЕ привяжешь живые аккаунты — в базе 0 tg_id из 19, список даёт владелец.
...
3. владелец дословно: «В Telegram у меня есть аккаунты всех преподавателей. Я могу легко выдать их этому боту или тебе — просто со списком имён. Это делается за пять минут. Коммуницировать с ними иначе сложнее»
4. реальных tg_id у исполнителя НЕТ и не будет: в базе 0 из 19. Позиция строит и доказывает МЕХАНИЗМ на поддельных данных; привязка живых аккаунтов — отдельный ход владельца

## КОНТРАКТ ЗОНЫ (обязателен — не удалять; вписан Cowork)
- **МЕСТО РАБОТЫ:** ветка `zahod/N3-vhod` в основной папке. 🔴 Она должна УЖЕ стоять. НЕ на ней — СТОП, НЕ делай `git checkout`: в общей папке он МОЛЧА откатывает дерево к состоянию ветки (цена 27→28.07: файл сильно откатился ночью, поймал владелец вручную; след в git НЕ остаётся). Тогда заход пересобрать с `--worktree`. Ветку не переключай, в другие НЕ коммить.
- **ЗОНА (можно менять):** `bot/` `veb/vhod.py` `migrations/004_privyazka.sql` `tests/veb/test_vhod.py` `tests/privyazka/` `zhurnal/2026-09-02_spetsmat-bot/kod_N3-vhod-cherez-telegram.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/bootstrap_zahod.py --zadanie-subagentu --zone bot/ --zone veb/vhod.py --zone migrations/004_privyazka.sql --zone tests/veb/test_vhod.py --zone tests/privyazka/ --kommitit 'миграцию 004 отдельно от кода бота' --kommitit 'каждые 20 минут, даже незаконченное' --zakryt 'ветку zahod/N3-vhod после переноса'
```
...
- Проверь ветку: `git branch --show-current` — обязано быть `zahod/N3-vhod`. Не она — СТОП, `git checkout` НЕ делай (§4 GIT-disciplina), нужен `--worktree`.
- Точка отката: `git add bot/ veb/vhod.py migrations/004_privyazka.sql tests/veb/test_vhod.py tests/privyazka/ zhurnal/2026-09-02_spetsmat-bot/kod_N3-vhod-cherez-telegram.md` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
Верификатор нужен, тип — **ПОСЛЕ-типа** — судит результат, стоит в конце, после задачи. Свежий субагент, ДРУГИМ методом (провести преподавателя весь путь ЖИВЫМ прогоном на фикстурах, а не чтением кода: выпуск токена, погашение, повторная попытка тем же токеном, отзыв, старая кука после отзыва), не перечитывает свою же правку. Доля сплошной выборки: все пять переходов состояния, 5 из 5. Финальная строка ответа обязательна дословно: «выдано N позиций из M найденных» — без неё ответ считается усечённым и в отчёт не вставляется.
...
```
git --no-optional-locks add -- bot/ veb/vhod.py migrations/004_privyazka.sql tests/veb/test_vhod.py tests/privyazka/ zhurnal/2026-09-02_spetsmat-bot/kod_N3-vhod-cherez-telegram.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- bot/ veb/vhod.py migrations/004_privyazka.sql tests/veb/test_vhod.py tests/privyazka/ zhurnal/2026-09-02_spetsmat-bot/kod_N3-vhod-cherez-telegram.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `bot/` `veb/vhod.py` `migrations/004_privyazka.sql` `tests/veb/test_vhod.py` `tests/privyazka/` `zhurnal/2026-09-02_spetsmat-bot/kod_N3-vhod-cherez-telegram.md`

- **Г1. Зона доехала в git.** `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone bot/` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone veb/vhod.py` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone migrations/004_privyazka.sql` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone tests/veb/test_vhod.py` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone tests/privyazka/` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone zhurnal/2026-09-02_spetsmat-bot/kod_N3-vhod-cherez-telegram.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py vlit-v-osnovnuyu zahod/N3-vhod --zone <своя зона> \
