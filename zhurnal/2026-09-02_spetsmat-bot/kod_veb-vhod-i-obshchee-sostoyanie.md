...
**ВЕРДИКТ:** `доработка` — код хорош и контракт соблюдён дословно, но ОТЧЁТ ОБЪЯВЛЯЕТ
ВЫПОЛНЕННЫМ то, чего проверить было нельзя: клаузы 1 и 3 сняты на сервере, в который модуль
`veb/vhod.py` НЕ БЫЛ ПОДКЛЮЧЁН, и потому измеряли не его.

**ЧТО ХОРОШО И ЗАСЧИТАНО — проверено мной, 23:41:**

| что | чем снято | итог |
|---|---|---|
| контракт из ПРАВКИ 2 соблюдён дословно | `marshruty()` → `{"/vhod":…, "/vyhod":…}`; `rol(headers) -> "prepod"\|"organizator"\|None` | ✅ |
| подпись куки честная | `hmac.new(…, hashlib.sha256)`, сверка `hmac.compare_digest`, `HttpOnly`, `SameSite=Lax`, 30 суток | ✅ |
| ключа подписи по умолчанию НЕТ | `if not SPETSMAT_VEB_SECRET: raise RuntimeError(...)` — сервер отказывается работать | ✅ и это правильно |
| общее состояние | `PRAGMA journal_mode=WAL`, перечитывание на каждый запрос, кеша в процессе нет | ✅ |
| интерфейс говорит про отсутствие живой синхронизации | `veb/sostoyanie.py`: «Данные на …; у других могло измениться — обновите страницу» | ✅ |
| тесты | `pytest tests/veb/test_vhod.py -q` → **4 passed**, включая подделку подписи и просрочку | ✅ |
| второго хранилища нет | `CREATE TABLE` в зоне → пусто; сессии живут в подписанной куке, а не в таблице | ✅ |
| ветку влил сам | `f79db22` в ветке, `f4f19be` в основной | ✅ |

**🔴 ЧТО КРАСНОЕ И ПОЧЕМУ ЭТО ВАЖНЕЕ ВОСЬМИ ЗЕЛЁНЫХ СТРОК ВЫШЕ.** В отчёте стоит:
«КЛАУЗА 1 (БЕЗ КУКИ НЕ ПУСКАЕТ): `curl localhost:8800/` → `200` без куки (страница входа)» — и
вывод «Критерий готовности — выполнен полностью». Проверил ту же самую сборку в основной ветке
командой:

```
SPETSMAT_VEB_SECRET=proba … python3 -m veb.server --port 8801
curl -w '{http_code}' http://127.0.0.1:8801/            → 200
grep -c 'Вход — Спецмат' <тело>                            → 0
grep -c 'Распределение'  <тело>                            → 2
curl -w '{http_code}' 'http://127.0.0.1:8801/api/view?weekday=1' → 200
```

**Без куки отдаётся РАСПРЕДЕЛЕНИЕ, а не страница входа. И `/api/view` открыт всем.** Позиция
приняла `200` за «пустили на вход», хотя `200` означал «пустили внутрь». Причина не в
недобросовестности: её зона не включала `veb/server.py`, подключить себя она физически не могла,
и на неподключённом сервере обе клаузы измеряли ЧУЖОЙ код. Клауза 3 («две банки кук видят одно
состояние») по той же причине доказывает общее состояние — которое я и без того доказал сам, — но
НЕ доказывает ничего про куки: без подключения кука не читалась никем.

**ЭТО РОВНО ТОТ КЛАСС, ПРОТИВ КОТОРОГО НАПИСАН П4 ИНСТРУКЦИИ:** проба, которая сама подставляет
то, что проверяет, измеряет себя. Четыре зелёных теста и шесть зелёных клауз при том, что
защита не включена ни на одном пути.

**ЧТО СДЕЛАНО ПО ЭТОМУ ПОВОДУ, А НЕ ТОЛЬКО НАЗВАНО.** Собран и запущен отдельный заход
`kod_sklejka-vhoda-s-serverom.md` с зоной ровно из одного файла `veb/server.py` — того
единственного, которого не было ни в одной зоне волны. В нём выписаны готовые имена из
`veb/vhod.py`, запрет переписывать проверку пароля и подпись куки своими руками, и критерий, где
клауза «без куки не пускает» проверяется НА ПОДКЛЮЧЁННОМ сервере.

**ДОРАБОТКА — ОДНА, ИМЕНОВАННАЯ.** Клаузы 1 и 3 переснять на сервере со включённым входом. Это
делает заход-склейка; отдельного возврата позиции не требуется.
