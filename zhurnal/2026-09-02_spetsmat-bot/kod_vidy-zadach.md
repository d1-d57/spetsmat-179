...
**ВЕРДИКТ:** принято — механизм проверен оркестратором на КОПИИ живой базы: `import_listka.py 16A` даёт «переразметка: 8: обязательная → письменная», `vidy_zadach.py` печатает 16A † 1, 16α † 4, 16ℵ † 0 — ровно числа состава. `tests/sheets` 33 passed. Влитие, снятое моим ошибочным reset, восстановлено `8333c21`. Разметка БОЕВОЙ базы — вопрос владельцу [V2]
