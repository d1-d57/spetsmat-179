# Канал исполнителя — N1-slot-vmesto-weekday (один заход до конца)
...
```
cd /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot && python3 /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot/_generator/tools/orkestr.py zhurnal/2026-09-02_spetsmat-bot --rezhim progon --dvizhok opencode --rod instrumenty --model openrouter/z-ai/glm-5.2:free --zahody kod_N1-slot-vmesto-weekday.md < /dev/null 2>&1 | tee /tmp/zahod-N1-slot-vmesto-weekday.log
```
...
Приоритет владельца: разобрать инциденты важнее, потом закрыть долги — неразобранный инцидент это повторяющаяся ошибка, долг может подождать.
  ...
  4. пунктов очереди «ДОСТАВЛЕНО: нет»   : 204
...
🔴 ДВЕРЬ НЕЗАКОММИЧЕННЫХ `kod_*.md` ОТКРЫТА КЛАПАНОМ: в арке `zhurnal/2026-09-02_spetsmat-bot` есть 1 незакоммиченных файлов-заходов `kod_N2-chelovek-bez-klassa.md` — второй заход в такую арку обычно не собирается (долг 4 `disciplina-git`), но АНАЛИТИК открыл клапан, причина дословно: «заходы волны ОСНОВАНИЕ собираются ПАКЕТОМ в одной ночной сессии перед закрытием: вся пачка уезжает владельцу одной командой коммита сразу после сборки. Держать их порознь дороже — сборка по одному потребовала бы семи круговых обменов с владельцем среди ночи». Отключение видно здесь, в артефакте, а не осталось решением в голове аналитика.

КОНТЕКСТ. spetsmat-bot — телеграм-бот учёта сданных задач спецмата 179-й школы;
рядом строится веб-страница распределения, которую надо разослать преподавателям.
Прошлый этап: волна «сайт» закрыта, её сверка с реальностью — SVERKA-2026-09-04.md;
принятые решения с числами — RESHENIA-2026-09-05.md; мандат этой волны —
mandate_volna-OSNOVANIE.md. Читай их, а не весь проект.
ЦЕЛЬ: перевести enrollment с weekday на slot 1|2 и вынести room в отдельную таблицу.
Приёмка — по ОТЧЁТУ, без построчной сверки. Стоп до цели: ты получишь верную СХЕМУ и верный импорт, но НЕ получишь настоящий состав класса на 2026/27 — его знает только владелец, это предмет позиции N2.
...
1. владелец снял с себя технические вопросы дословно: «тебе нужно решать технические вопросы… мне от тебя нужна умная, глубокая идея и глубокие решения». Технические развилки внутри зоны решает исполнитель, к владельцу с ними не ходит
2. волна называется ОСНОВАНИЕ и её мандат — mandate_volna-OSNOVANIE.md; провалом объявлено любое из трёх: ссылка разослана при пустом втором дне у детей, адрес снова одноразовый, завёлся второй журнал отметок
3. владелец описал умолчание дословно: «по умолчанию в оба дня у детей одни и те же преподаватели… по умолчанию может быть две одинаковые строки, а в сложных ситуациях в эти два места записываются разные параметры»
4. владелец про расписание дословно: «два раза в неделю гарантированно, во все учебные недели. Конкретные дни, время и кабинеты мы пока не знаем — кабинеты привязаны ко дню, и это может поменяться». Отдельный календарь года НЕ заводить

## КОНТРАКТ ЗОНЫ (обязателен — не удалять; вписан Cowork)
- **МЕСТО РАБОТЫ:** ветка `zahod/N1-slot` в основной папке. 🔴 Она должна УЖЕ стоять. НЕ на ней — СТОП, НЕ делай `git checkout`: в общей папке он МОЛЧА откатывает дерево к состоянию ветки (цена 27→28.07: файл сильно откатился ночью, поймал владелец вручную; след в git НЕ остаётся). Тогда заход пересобрать с `--worktree`. Ветку не переключай, в другие НЕ коммить.
- **ЗОНА (можно менять):** `migrations/003_slot.sql` `core/services/enrollment.py` `core/services/room.py` `infra/enrollment_repo.py` `tools/import_raspredelenie.py` `tests/enrollment/` `tests/room/` `zhurnal/2026-09-02_spetsmat-bot/kod_N1-slot-vmesto-weekday.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/bootstrap_zahod.py --zadanie-subagentu --zone migrations/003_slot.sql --zone core/services/enrollment.py --zone core/services/room.py --zone infra/enrollment_repo.py --zone tools/import_raspredelenie.py --zone tests/enrollment/ --zone tests/room/ --kommitit 'миграцию 003 отдельным коммитом ДО правки сервисов — чтобы откат схемы был отделим от отката кода' --kommitit 'каждые 20 минут, даже незаконченное: урок волны «сайт» — 453 строки были спасены ЧУЖИМ коммитом' --zakryt 'ветку zahod/N1-slot после переноса работы; чужие ветки не трогать'
```
...
- Проверь ветку: `git branch --show-current` — обязано быть `zahod/N1-slot`. Не она — СТОП, `git checkout` НЕ делай (§4 GIT-disciplina), нужен `--worktree`.
- Точка отката: `git add migrations/003_slot.sql core/services/enrollment.py core/services/room.py infra/enrollment_repo.py tools/import_raspredelenie.py tests/enrollment/ tests/room/ zhurnal/2026-09-02_spetsmat-bot/kod_N1-slot-vmesto-weekday.md` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
Верификатор нужен, тип — **ДО-типа** — судит СОСТОЯНИЕ, которое работа уничтожит, и идёт ПЕРВЫМ ходом, до самой работы: после неё он неисполним. Свежий субагент, ДРУГИМ методом (на КОПИИ боевой базы выполнить `select weekday, count(*) from enrollment group by weekday` и записать вывод в файл до всякой правки; та же команда после миграции неисполнима, поэтому верификатор стоит первым ходом. Метод именно прогон команды, а не перечитывание своей же миграции), не перечитывает свою же правку. Доля сплошной выборки: сплошная выборка: все 56 детей, обе строки каждого — 112 из 112. Финальная строка ответа обязательна дословно: «выдано N позиций из M найденных» — без неё ответ считается усечённым и в отчёт не вставляется.
...
```
git --no-optional-locks add -- migrations/003_slot.sql core/services/enrollment.py core/services/room.py infra/enrollment_repo.py tools/import_raspredelenie.py tests/enrollment/ tests/room/ zhurnal/2026-09-02_spetsmat-bot/kod_N1-slot-vmesto-weekday.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- migrations/003_slot.sql core/services/enrollment.py core/services/room.py infra/enrollment_repo.py tools/import_raspredelenie.py tests/enrollment/ tests/room/ zhurnal/2026-09-02_spetsmat-bot/kod_N1-slot-vmesto-weekday.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `migrations/003_slot.sql` `core/services/enrollment.py` `core/services/room.py` `infra/enrollment_repo.py` `tools/import_raspredelenie.py` `tests/enrollment/` `tests/room/` `zhurnal/2026-09-02_spetsmat-bot/kod_N1-slot-vmesto-weekday.md`

- **Г1. Зона доехала в git.** `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone migrations/003_slot.sql` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone core/services/enrollment.py` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone core/services/room.py` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone infra/enrollment_repo.py` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone tools/import_raspredelenie.py` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone tests/enrollment/` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone tests/room/` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone zhurnal/2026-09-02_spetsmat-bot/kod_N1-slot-vmesto-weekday.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py vlit-v-osnovnuyu zahod/N1-slot --zone <своя зона> \
