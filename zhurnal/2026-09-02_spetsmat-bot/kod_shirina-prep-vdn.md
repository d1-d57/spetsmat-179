# Канал исполнителя — shirina-prep-vdn (один заход до конца)
...
/Users/ivanyakovlev/Documents/GitHub/spetsmat-bot/zhurnal/2026-09-02_spetsmat-bot/kod_shirina-prep-vdn.md
...
Первым ходом: python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py worktree add shirina-prep-vdn --branch zahod/shirina-prep-vdn, затем чтение захода, затем секция
...
Приоритет владельца: разобрать инциденты важнее, потом закрыть долги — неразобранный инцидент это повторяющаяся ошибка, долг может подождать.
  ...
  4. пунктов очереди «ДОСТАВЛЕНО: нет»   : 570
...
1. менять ТОЛЬКО вкладки групп В Д Н и ТОЛЬКО у вошедшего; у гостя и на вкладке школьников уже всё хорошо
2. обе версии: распределение на занятие и постоянное распределение
3. левый край НЕ трогать, правый отодвинуть по максимуму; сдвиг левого влево — опционально
4. больше НИЧЕГО в дизайне не менять — жёстко

## КОНТРАКТ ЗОНЫ (обязателен — не удалять; вписан Cowork)
- **МЕСТО РАБОТЫ:** **рабочая папка `../spetsmat-bot-wt/shirina-prep-vdn`** (рядом с репозиторием) — ТОЛЬКО ДЛЯ КОДА, worktree захода на ветке `zahod/shirina-prep-vdn`. 🔴 **ДАЛЬШЕ — ТОЛЬКО ПУТИ ОТНОСИТЕЛЬНО ЭТОЙ ПАПКИ** (или `cd` в неё безусловно, каждым ходом): абсолютный путь в главную папку репозитория здесь — типичная ошибка, правка утекает МИМО worktree и найдётся только на коммите («вне git» в `git_zona.py check --zone` из рабочей папки, на файле, который уже правил, — цена, оплаченная живьём: 5 файлов, ручное копирование и откат главной папки). 🔴 **Заводишь её САМ — ВТОРЫМ ходом, СРАЗУ ПОСЛЕ пункта 1 блока §0.1 ГИТ-КОНТУР ниже (не первым ходом захода целиком), из корня репозитория:**
  ```
  python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py worktree add shirina-prep-vdn --branch zahod/shirina-prep-vdn
  cd ../spetsmat-bot-wt/shirina-prep-vdn
  ```
...
- **ЗОНА (можно менять):** `veb/obshchee/karkas.py`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/bootstrap_zahod.py --zadanie-subagentu --zone veb/obshchee/karkas.py --kommitit 'свою зону по ходу' --zakryt 'ветку zahod/shirina-prep-vdn через poteri после влития; рабочую папку снять'
```
...
- Проверить, что на месте: `git rev-parse --abbrev-ref HEAD` → должно быть `zahod/shirina-prep-vdn`. Не оно — ты не в своей папке.
- ПЛАН/ВОПРОСЫ/ОТЧЁТ/УРОКИ ФАБРИКЕ пиши в ЭТОТ файл — он в основной папке, не копируй его в рабочую.
- Точка отката: `git add veb/obshchee/karkas.py` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
### 🔴 МИКРО-ЗАХОД. ОДНО МЕСТО. ГРАНИЦЫ НАЗВАНЫ ВЛАДЕЛЬЦЕМ ЖЁСТКО — НЕ РАСШИРЯЙ ИХ.

**ЧТО НЕ ТАК, СЛОВАМИ ВЛАДЕЛЬЦА.** «Есть строчки, в которых слева написано имя и фамилия
школьника, а справа в одной или двух колонках упоминаются преподаватели. Там левый край
правильно выставлен, а правый слишком узкий. Непонятно почему. Просто сдвинуть его правее.
Больше ничего не менять. Жёстко.»

**ГДЕ ИМЕННО — АДРЕСА СНЯТЫ АНАЛИТИКОМ, ПЕРЕПРОВЕРЬ И ПРАВЬ ТАМ.**
`veb/obshchee/karkas.py:1583` — `.para .kto{flex:0 1 auto;min-width:0;overflow:hidden;
text-overflow:ellipsis;white-space:nowrap}`; `:1585` — `.para .komu{margin-left:auto;
text-align:right;...}`; `:1572` — `#v-В .para,#v-Д .para,#v-Н .para{font-size:1.6rem;...}`.
Обрезается ПОЛЕ ВЫБОРА принимающего в строке школьника: на боевом видно «Дима Елисе◇»,
«Наталия Стр◇», «Александр Т◇».

**ГРАНИЦЫ, ЗА КОТОРЫЕ ВЫХОДИТЬ НЕЛЬЗЯ:**
* только вкладки групп **В · Д · Н** (`#v-В`, `#v-Д`, `#v-Н`);
* только **у вошедшего**. У гостя и на вкладке «школьникам» владелец сказал дословно: «уже всё
  хорошо» — там НЕ трогать вовсе;
* **обе версии**: распределение на занятие и постоянное распределение (`/raspredelenie` и
  `/raspredelenie/postoyannoe`);
* **левый край НЕ двигать.** Правый — отодвинуть по максимуму. Сдвиг левого влево владелец
  назвал опциональным: «может быть, где-то можно левый подвинуть ещё левее, но это опционально».
  Не делай опциональное без нужды.

🔴 **ПРАВИЛО, ПО КОТОРОМУ ВЫБИРАЕТСЯ ШИРИНА, — БАЗОВОЕ ТРЕБОВАНИЕ ВЛАДЕЛЬЦА, А НЕ ВКУС:**
«ширина полей, в которые что-то вводится, должна выбираться по САМОМУ ШИРОКОМУ тексту, который
может быть введён». Значит поле принимающего обязано вмещать самое длинное имя из списка
преподавателей ЦЕЛИКОМ, а не среднее. Возьми самое длинное имя командой, а не на глаз.

### 🔴 ЧЕГО НЕ ДЕЛАТЬ — ЭТО ПОЛОВИНА ЗАДАНИЯ

Больше в дизайне НЕ МЕНЯТЬ НИЧЕГО. Ни шрифтов, ни отступов, ни высоты строк, ни карточек
принимающих справа, ни вкладки «школьникам», ни гостевого вида, ни главной. Владелец: «всё
остальное в дизайне не меняем… только это место поправить, только это. Жёстко.»
Увидел рядом другой дефект — назови его в `## ОТЧЁТ` и НЕ чини. Комментарии в
`karkas.py:1577` и `:1590` объясняют, почему имя сжимается, а карточке вернули высоту, — это
оплаченные решения, их не отменять.

**КРИТЕРИЙ ГОТОВНОСТИ (может ПРОВАЛИТЬСЯ):** живой прогон на реальном объекте — на живом сайте
обеими ролями, не на фикстуре. Пары чисел, у каждой печатаются ОБА:
* на вкладках В, Д, Н у ВОШЕДШЕГО обрезанных полей принимающего было N → стало **0**
  (`python3 tools/gejt_verstki.py`, обе роли, оба адреса — и на занятие, и постоянное);
* самое длинное имя преподавателя (назвать его и его ширину) помещается в поле ЦЕЛИКОМ;
* левый край строки: координата до правки и после — **совпадают**;
* у ГОСТЯ и на вкладке «школьникам»: снимок до и после **побайтово равен** — доказательство,
  что ничего лишнего не тронуто;
* горизонтальной прокрутки нет: `scrollWidth` не больше окна на 1440×900.
...
Верификатор не нужен: правка в одну строку CSS, судится гейтом вёрстки и одним снимком; верификатор дороже работы.
...
```
git --no-optional-locks add -- veb/obshchee/karkas.py                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- veb/obshchee/karkas.py   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `veb/obshchee/karkas.py`

- **Г1. Зона доехала в git.** `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone veb/obshchee/karkas.py` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py vlit-v-osnovnuyu zahod/shirina-prep-vdn --zone <своя зона> \
...
виден»: прогон изменённого механизма из `/Users/ivanyakovlev/Documents/GitHub/spetsmat-bot`, НЕ из рабочей папки `/Users/ivanyakovlev/Documents/GitHub/spetsmat-bot-wt/shirina-prep-vdn` плюс `grep` по ЖИВОМУ файлу,
...
### ПРАВКА 1 (2026-09-11 00:38, от владельца через оркестратора) — ЛОКАЛЬНО МЕРЬ, БОЕВОЕ ЗА ОРКЕСТРАТОРОМ

Проверять ширину поля принимающего **локально МОЖНО** — рендер честен. Принимать ГЛАЗАМИ
владелец будет на боевом, и боевой прогон делает оркестратор последним ходом волны.
От тебя: число **с охватом** («поле принимающего проверено на X вкладках из Y, в обеих
версиях — по занятию и постоянное») и ОДНА строка в `## ОТЧЁТ` о том, как это увидеть:
какой адрес открыть, под кем войти, куда смотреть. «Стало шире» без охвата не принимается.

Границы владельца (F6) не двигаются: левый край на месте, правый вправо, единственное
место дизайна в волне. Ничего рядом не трогай.
