...
WT = "/Users/ivanyakovlev/Documents/GitHub/spetsmat-bot-wt"
INSTR = "/Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools"


def promt(tema: str) -> str:
    """Промпт бесплатному исполнителю. Один дом на все позиции волны.

    🔴 Отмена §0.1 стоит ПЕРВОЙ и адресована прямо: субагент гит-контура убил
    четыре захода из десяти у соседней волны, и работы у него не было.
    """
    return (
        "🔴 ОТМЕНА ОДНОГО ПУНКТА ТВОЕГО ЗАХОДА, ЧИТАЙ ЭТО ПЕРВЫМ. СУБАГЕНТА ГИТ-КОНТУРА §0.1 "
        "НЕ ЗАПУСКАЙ — пункт отменён оркестратором, данное указание сильнее текста захода; "
        "причина замерена соседней волной: четыре захода из десяти умерли ровно на этом вызове. "
        "Вместо всего блока §0.1 выполни САМ одну команду и вставь её вывод в ## ОТЧЁТ: "
        "git --no-optional-locks branch --no-merged main | grep -c zahod/ . "
        "Заполни в ## ГИГИЕНА ВХОДА строку **ВСЕ ДОЛГИ ВХОДА ЗАКРЫТЫ:** словом да или нет и вставь "
        "туда вывод той единственной команды — без этой строки приёмка краснеет гейтом Г12, и краснеет "
        "по вине отмены, а не по твоей. "
        f"| Дальше: твой заход — файл {ARKA}/kod_{tema}.md . "
        "Прочитай ТОЛЬКО его и то, что он называет; остальной проект не изучай. "
        "План/вопросы/отчёт пиши в этот же файл внизу (## ПЛАН / ## ВОПРОСЫ / ## ОТЧЁТ), "
        "НА АНГЛИЙСКОМ. Ничего сверх задачи не трогай. Субагентов не зови ни на что, кроме "
        "верификатора §3; коммиты делай САМ, по ходу работы, а не одним последним ходом. "
        "Ветку в конце вливаешь САМ, последним ходом, после коммита зоны. "
        "🔴 Частей в задании несколько: делай ПО ПОРЯДКУ, коммить КАЖДУЮ отдельно; не успел — "
        "назови несделанные списком в ## ОТЧЁТ, это законный исход."
    )
...
def main(argv=None) -> int:
    ...
    staraya = m.group(0)
    if besplatnaya:
      # ── БЕСПЛАТНАЯ: прямой вызов движка, надзор СНАРУЖИ ────────────────────
      # 🔴 Строка генератора для `--dvizhok opencode` нерабочая ТРЕМЯ способами
      # сразу, и все три снаружи выглядят одинаково — «модель упала», лог 0 байт
      # (замер 02.09 09:57–10:00, шесть сожжённых прогонов):
      #   1. путь к инструменту не существует — генератор подставил объявленный
      #      корень владельца в путь ЧУЖОГО инструмента и вписал
      #      `spetsmat-bot/_generator/tools/orkestr.py`, а каталога `_generator/`
      #      у этого репозитория нет вовсе;
      #   2. строка РЕКУРСИВНА: она зовёт `orkestr.py --rezhim progon`, а тот
      #      берёт стартовую команду ИЗ ЭТОГО ЖЕ ФАЙЛА и исполняет её —
      #      надзиратель запускает надзиратель;
      #   3. цепочка склеена через `&&` после `worktree add`, а тот на
      #      СУЩЕСТВУЮЩЕЙ рабочей папке возвращает rc=1 при живом состоянии,
      #      и на повторе (добор, перевыбор модели) всё умирает ДО движка.
      # Форма ниже — ровно то, чего ждёт `orkestr.py progon`: он читает строку,
      # подменяет В НЕЙ одно имя модели и надзирает снаружи, различая четыре
      # исхода, включая ЛОЖНЫЙ УСПЕХ.
      t = t.replace(staraya, (
        "```\n"
        f"python3 {INSTR}/git_zona.py worktree add {a.tema} --branch zahod/{a.tema} ; "
        f"cd {WT}/{a.tema} && "
        f"opencode run --auto --model {a.model} '{promt(a.tema)}' "
        f"< /dev/null 2>&1 | tee /tmp/zahod-{a.tema}.log\n"
        "```\n"
        ...
        "```\n"
        f"python3 {INSTR}/orkestr.py {ARKA} \\\n"
        f"  --rezhim progon --rod instrumenty --molchanie 1800 --potolok 14400 --popytok 3 \\\n"
        f"  --zahody kod_{a.tema}.md\n"
        "```\n"
    ...
    chto = ("стартовая строка → прямой opencode, надзор снаружи orkestr.py" if besplatnaya
            else "стартовая строка → ZAPUSK-ZAHODA.sh, мимо мёртвого ARN Bedrock")
