...
def main(argv=None) -> int:
    ...
    # Печатать надо ТО, ЧТО СДЕЛАНО, а не то, что обычно делается: строка
    # «стартовая строка заменена» на бесплатной позиции была бы прямым враньём
    # собственного вывода — тот самый класс, что весь вечер стоил времени.
    chto = ("стартовая строка ОСТАВЛЕНА (надзиратель orkestr.py)" if staraya is None
            else "стартовая строка заменена (мимо мёртвого ARN)")
    print(f"✅ {fajl.name}: {chto}, контекст и задача вписаны, "
