# Канал исполнителя — N7-repozitorii-pages-tunnel (один заход до конца)
...
```
cd /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot && python3 /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot/_generator/tools/orkestr.py zhurnal/2026-09-02_spetsmat-bot --rezhim progon --dvizhok opencode --rod instrumenty --model openrouter/z-ai/glm-5.2:free --zahody kod_N7-repozitorii-pages-tunnel.md < /dev/null 2>&1 | tee /tmp/zahod-N7-repozitorii-pages-tunnel.log
```
...
Приоритет владельца: разобрать инциденты важнее, потом закрыть долги — неразобранный инцидент это повторяющаяся ошибка, долг может подождать.
  ...
  4. пунктов очереди «ДОСТАВЛЕНО: нет»   : 219
...
🔴 ДВЕРЬ НЕЗАКОММИЧЕННЫХ `kod_*.md` ОТКРЫТА КЛАПАНОМ: в арке `zhurnal/2026-09-02_spetsmat-bot` есть 6 незакоммиченных файлов-заходов `kod_N1-slot-vmesto-weekday.md`, `kod_N2-chelovek-bez-klassa.md`, `kod_N3-vhod-cherez-telegram.md`, `kod_N4-priyom-zadach.md`, `kod_N5-konduit-proshlogo-goda.md`, `kod_N6-stranica-pages-i-zaglushka.md` — второй заход в такую арку обычно не собирается (долг 4 `disciplina-git`), но АНАЛИТИК открыл клапан, причина дословно: «заходы волны ОСНОВАНИЕ собираются ПАКЕТОМ в одной ночной сессии перед закрытием; вся пачка уезжает владельцу одной командой коммита сразу после сборки». Отключение видно здесь, в артефакте, а не осталось решением в голове аналитика.
...
ЦЕЛЬ: завести spetsmat-179 репозиторием и Pages, дать spetsmat-bot origin, поднять постоянный туннель и автозапуск через launchd.
Приёмка — по ОТЧЁТУ, без построчной сверки. 🔴 СТОП ДО ЦЕЛИ И ЗАПРЕТ: origin НЕ заводить, пока не перевыпущены пароля организатора и преподавателя — они лежат открытым текстом в истории git (SOSTOYANIE-VOLNY-sajt.md, коммит прошлой волны). Пока репозиторий локальный, история с диска не уходила; origin делает её выездной. Перевыпуск дешевле переписывания истории.
...
1. владелец вернул эту работу В волну на интервью 04.09, отменив прежнюю договорённость: дословно «создание репозиториев и туннеля нужно сделать в этой же волне». В волне «сайт» это была позиция P7 — она стояла в составе и не запускалась НИ РАЗУ, а на ней стояло всё остальное
2. владелец снял с себя технические вопросы дословно: «тебе нужно решать технические вопросы… мне от тебя нужна умная, глубокая идея и глубокие решения». Выбор туннеля сделан за него: Tailscale Funnel
3. 🔴 ЭТОТ ЗАХОД ИСПОЛНЯЕТСЯ НА МАШИНЕ ВЛАДЕЛЬЦА, ПРИ НЁМ, под его учёткой gh. Ночному исполнителю его не отдавать: авторизоваться под чужой учёткой он не может и умрёт на первом шаге

## КОНТРАКТ ЗОНЫ (обязателен — не удалять; вписан Cowork)
- **МЕСТО РАБОТЫ:** ветка `zahod/N7-osnovanie` в основной папке. 🔴 Она должна УЖЕ стоять. НЕ на ней — СТОП, НЕ делай `git checkout`: в общей папке он МОЛЧА откатывает дерево к состоянию ветки (цена 27→28.07: файл сильно откатился ночью, поймал владелец вручную; след в git НЕ остаётся). Тогда заход пересобрать с `--worktree`. Ветку не переключай, в другие НЕ коммить.
- **ЗОНА (можно менять):** `../spetsmat-179/` `deploy/` `ops/` `zhurnal/2026-09-02_spetsmat-bot/kod_N7-repozitorii-pages-tunnel.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/bootstrap_zahod.py --zadanie-subagentu --zone ../spetsmat-179/ --zone deploy/ --zone ops/ --kommitit 'spetsmat-179 коммитить в ЕГО СОБСТВЕННОМ репозитории отдельно — зону spetsmat-bot с ним не смешивать' --kommitit 'каждые 20 минут, даже незаконченное' --zakryt 'ветку zahod/N7-osnovanie после переноса; чужие ветки не трогать'
```
...
- задеплоить: spetsmat-179 на GitHub Pages; сервер под launchd на машине владельца

- Проверь ветку: `git branch --show-current` — обязано быть `zahod/N7-osnovanie`. Не она — СТОП, `git checkout` НЕ делай (§4 GIT-disciplina), нужен `--worktree`.
- Точка отката: `git add ../spetsmat-179/ deploy/ ops/ zhurnal/2026-09-02_spetsmat-bot/kod_N7-repozitorii-pages-tunnel.md` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
Верификатор нужен, тип — **ПОСЛЕ-типа** — судит результат, стоит в конце, после задачи. Свежий субагент, ДРУГИМ методом (предъявить адреса ЖИВЫМИ запросами извне, а не из браузера владельца: адрес Pages и адрес туннеля отдельными curl. Адрес туннеля предъявить ДВАЖДЫ с перезапуском процесса между предъявлениями — именно это отличает постоянный адрес от одноразового, и именно это не было проверено в прошлой волне), не перечитывает свою же правку. Доля сплошной выборки: все четыре дела позиции: Pages, origin, туннель, автозапуск — 4 из 4. Финальная строка ответа обязательна дословно: «выдано N позиций из M найденных» — без неё ответ считается усечённым и в отчёт не вставляется.
...
```
git --no-optional-locks add -- ../spetsmat-179/ deploy/ ops/ zhurnal/2026-09-02_spetsmat-bot/kod_N7-repozitorii-pages-tunnel.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- ../spetsmat-179/ deploy/ ops/ zhurnal/2026-09-02_spetsmat-bot/kod_N7-repozitorii-pages-tunnel.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `../spetsmat-179/` `deploy/` `ops/` `zhurnal/2026-09-02_spetsmat-bot/kod_N7-repozitorii-pages-tunnel.md`

- **Г1. Зона доехала в git.** `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone ../spetsmat-179/` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone deploy/` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone ops/` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone zhurnal/2026-09-02_spetsmat-bot/kod_N7-repozitorii-pages-tunnel.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py vlit-v-osnovnuyu zahod/N7-osnovanie --zone <своя зона> \
...
**ВЕТКА РАБОТЫ:** `zahod/N7-osnovanie`
