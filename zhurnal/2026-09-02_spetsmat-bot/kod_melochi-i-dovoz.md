# Канал исполнителя — melochi-i-dovoz (один заход до конца)
...
> **Модель: openrouter/nvidia/nemotron-3-ultra-550b-a55b:free** — чистая механика: два грепа с заменой и довоз трёх документов в git — суждения здесь нет вовсе.
...
```
Модель: openrouter/nvidia/nemotron-3-ultra-550b-a55b:free — см. обоснование в шапке захода, §0.

Ты исполнитель в репозитории /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot-wt/melochi-i-dovoz.

Твой единственный вход — файл-заход:
/Users/ivanyakovlev/Documents/GitHub/spetsmat-bot/zhurnal/2026-09-02_spetsmat-bot/kod_melochi-i-dovoz.md
...
Первым ходом: python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py worktree add melochi-i-dovoz --branch zahod/melochi-i-dovoz, затем чтение захода, затем секция
...
Приоритет владельца: разобрать инциденты важнее, потом закрыть долги — неразобранный инцидент это повторяющаяся ошибка, долг может подождать.
  ...
  4. пунктов очереди «ДОСТАВЛЕНО: нет»   : 561
...
1. мелочи висят с 04.09 и раз за разом объявляются сделанными
2. три документа doc/ лежат вне git с 06.09

## КОНТРАКТ ЗОНЫ (обязателен — не удалять; вписан Cowork)
- **МЕСТО РАБОТЫ:** **рабочая папка `../spetsmat-bot-wt/melochi-i-dovoz`** (рядом с репозиторием) — ТОЛЬКО ДЛЯ КОДА, worktree захода на ветке `zahod/melochi-i-dovoz`. 🔴 **ДАЛЬШЕ — ТОЛЬКО ПУТИ ОТНОСИТЕЛЬНО ЭТОЙ ПАПКИ** (или `cd` в неё безусловно, каждым ходом): абсолютный путь в главную папку репозитория здесь — типичная ошибка, правка утекает МИМО worktree и найдётся только на коммите («вне git» в `git_zona.py check --zone` из рабочей папки, на файле, который уже правил, — цена, оплаченная живьём: 5 файлов, ручное копирование и откат главной папки). 🔴 **Заводишь её САМ — ВТОРЫМ ходом, СРАЗУ ПОСЛЕ пункта 1 блока §0.1 ГИТ-КОНТУР ниже (не первым ходом захода целиком), из корня репозитория:**
  ```
  python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py worktree add melochi-i-dovoz --branch zahod/melochi-i-dovoz
  cd ../spetsmat-bot-wt/melochi-i-dovoz
  ```
...
- **ЗОНА (можно менять):** `seed/teachers.csv` `veb/razdely/prepodavateli.py` `veb/obshchee/karkas.py` `veb/razdely/shkolniki.py` `doc/`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/bootstrap_zahod.py --zadanie-subagentu --zone seed/teachers.csv --zone veb/razdely/prepodavateli.py --zone veb/obshchee/karkas.py --zone veb/razdely/shkolniki.py --zone doc/ --kommitit 'свою зону по ходу; отдельным коммитом довоз трёх документов doc/' --zakryt 'ветку zahod/melochi-i-dovoz через poteri после влития; рабочую папку снять'
```
...
- Проверить, что на месте: `git rev-parse --abbrev-ref HEAD` → должно быть `zahod/melochi-i-dovoz`. Не оно — ты не в своей папке.
- ПЛАН/ВОПРОСЫ/ОТЧЁТ/УРОКИ ФАБРИКЕ пиши в ЭТОТ файл — он в основной папке, не копируй его в рабочую.
- Точка отката: `git add seed/teachers.csv veb/razdely/prepodavateli.py veb/obshchee/karkas.py veb/razdely/shkolniki.py doc/` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
### ТРИ МЕЛОЧИ, КОТОРЫЕ ВИСЯТ С 04.09 И ДВАЖДЫ ОБЪЯВЛЯЛИСЬ СДЕЛАННЫМИ

**1. «Лена», а не «Елена» (C1 ТЗ).** Греп «Елена» по `seed/` и `veb/` обязан дать НОЛЬ. Позиция
P10 числила это сделанным; по данным — не тронуто.

**2. Слово «ПРИХОДИТ» (C2 ТЗ).** Одно вхождение в `veb/razdely/prepodavateli.py`. Может оказаться
комментарием — проверь и убери, только если это ВИДИМЫЙ человеку текст. Комментарий не трогай.

**3. 🔴 ТРИ ДОКУМЕНТА ЛЕЖАТ ВНЕ GIT С 06.09 И НИКОГДА В НЁМ НЕ БЫЛИ:**
`doc/PLAN-veb-2026-09.md` (20 КБ, план веб-части с открытыми вопросами и составом волны),
`doc/TZ-raspredelenie-dizajn-i-dva-sloya.md` (15 КБ), `doc/TZ-sloj-zanyatia.md` (17 КБ).
Пятьдесят два килобайта решений владельца, которые исчезнут при первой чистке рабочего дерева.
Забрать в git отдельным коммитом. **Ничего в них не править** — только сохранить.

### ЧЕГО НЕ ДЕЛАТЬ
Ничего сверх этих трёх пунктов. Увидел рядом другой дефект — назови в `## ОТЧЁТ`, не чини.

**КРИТЕРИЙ ГОТОВНОСТИ (может ПРОВАЛИТЬСЯ):** живой прогон на реальном объекте репозитория, не
на фикстуре. Три пары чисел: «Елена» было N → стало 0 · «ПРИХОДИТ» было 1 → стало 0 или назвать,
почему осталось · `git ls-files doc/ | wc -l` было N → стало N+3.
...
Верификатор не нужен: работа проверяется двумя грепами и одной командой git — верификатор дороже самой работы.
...
```
git --no-optional-locks add -- seed/teachers.csv veb/razdely/prepodavateli.py veb/obshchee/karkas.py veb/razdely/shkolniki.py doc/                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- seed/teachers.csv veb/razdely/prepodavateli.py veb/obshchee/karkas.py veb/razdely/shkolniki.py doc/   # отсекает всё чужое
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py vlit-v-osnovnuyu zahod/melochi-i-dovoz --zone <своя зона> \
...
виден»: прогон изменённого механизма из `/Users/ivanyakovlev/Documents/GitHub/spetsmat-bot`, НЕ из рабочей папки `/Users/ivanyakovlev/Documents/GitHub/spetsmat-bot-wt/melochi-i-dovoz` плюс `grep` по ЖИВОМУ файлу,
