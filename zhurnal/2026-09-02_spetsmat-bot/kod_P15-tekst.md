...
**ВСЕ ДОЛГИ ВХОДА ЗАКРЫТЫ:** `нет` — невлитых `zahod/*`-веток 1: `zahod/P18-teksty`.
> ⚠ Привёл ОРКЕСТРАТОР на приёмке, и это не подлог. На ВХОДЕ галочка была верна: `P18`
> тогда не существовала, её завели в 17:56, через четверть часа после закрытия этого
> захода. Гейт Г12 перепроверяет входную гигиену прогоном в момент ПРИЁМКИ, то есть
> судит вход по состоянию на выход, и краснеет от чужой ветки, заведённой позже.
> Влить `P18` нельзя — она живая. Из двух исходов, которые гейт разрешает, доступен
> один. Тот же случай уже разобран у `P14`; это дефект ГЕЙТА, а не двух заходов.
...
- 2026-09-02T1742-p15-tekst-dfe3d28-main-12-bot — pravka-koda — ВЫХОД ЗА ЗОНУ, объявленный самим заходом: 12 строк в `bot/app.py` (включение своего роутера). 🔴 ДОПОЛНЕНИЕ К ВЕРДИКТУ, ВПИСАНО ПОЗЖЕ: мой первый вердикт этого НЕ РАЗОБРАЛ — 18 гейтов были зелёные, и я принял позицию, не открыв её заявку. Разобрал в 18:0x и **выход за зону признаю законным**: без этих 12 строк влитие делало `main` КРАСНЫМ на классовом гейте P20 («роутеры-экраны: найдено 7, включено 6»), а предписанный откат влития был уже невозможен — поверх лёг `zahod/P9-listok`, и `reset` снёс бы работу соседа. Заход выбрал меньшее и ОБРАТИМОЕ действие и раскрыл его сам, заявкой, до приёмки. Место роутера проверено по существу: после `uvedomlenia` и до `stale_router` — иначе тапы таблицы отвечали бы «экран устарел». Замер сошёлся: до правки 1 failed / 899 passed, после — 910 passed.
- 2026-09-02T1802-callback-voiceconfirm-bot-routers-voice-py — pravka-koda, blokiruet — НАЙДЕНО ПРИ РАЗБОРЕ ЭТОЙ ЗАЯВКИ, к P15 отношения не имеет: `VoiceConfirm` и `ViewYear` делят префикс `vy`, голосовой черновик не может записать никто. Гейт P20 этот дефект ПЕЧАТАЕТ и пропускает (6 passed) — надежда вместо проверки.
