...
Одна строка текста → черновик → ТА ЖЕ таблица подтверждения → журнал. Никогда прямой записи.

### 1 · Форма строки

    [7] Петров 3,5,7б

- `[7]` — номер листка, в КВАДРАТНЫХ скобках, решение владельца. Без него строка неоднозначна:
  номера задач у разных листков совпадают.
- дальше фамилия и номера задач через запятую или пробел; `7б` — метка, а не число.
- 🔴 Строка может нести НЕСКОЛЬКО учеников подряд, если преподаватель так пишет. Реши, поддерживаешь
  ли, и напиши решение в отчёт — молчаливое «разберём только первого» это тихая потеря отметок.

### 2 · 🔴 ТОТ ЖЕ НЕЧЁТКИЙ ИНДЕКС, ЧТО У ФОТО — импортируй, не пиши второй

P7 уже развернула 56 фамилий по падежам (350–670 форм) и считает `rapidfuzz.fuzz.ratio`, порог
около 0,7, **не `token_set_ratio`** (он даёт 100 на вложенности: «Иванов И.» против «Иванов»).
Это её код, он принят и влит.

**Второй, свой индекс — это два дома для одной правды, и они разойдутся молча.** Импортируй.
Чего-то не хватает — скажи в `## ВОПРОСЫ`, не форкай.

### 3 · Неоднозначность НЕ УГАДЫВАЕТСЯ

Два Пети, опечатка, обрезанная фамилия — бот показывает КНОПКИ с вариантами, а не выбирает сам.
Это то же правило, что у фото и голоса: заставлять угадывать нельзя, потому что угадает.
Тест на неоднозначную фамилию обязан проверять, что вариантов показано ДВА, а отметка не записана.

### 4 · Подтверждение — то же самое, не своё

Разобранное показывается ТОЙ ЖЕ таблицей «ученик × галочки», что у P7; тап снимает и ставит; запись
идёт через путь отметок P4. **Ты не строишь ни второй таблицы, ни второго пути записи.**

Идемпотентность: та же строка, посланная дважды, не задваивает отметки.

### 5 · Что не трогать

`core/services/raspoznavanie.py` и `infra/llm.py` (P7), `core/services/marking.py` (P1),
`core/ports.py`, `infra/repositories.py`, `config.py`, `migrations/` — read-only.
Роутер подключается ДОБАВЛЕНИЕМ в `bot/app.py`; правку вне зоны объяви отдельным коммитом.

⚠ **Осторожно с catch-all.** P4 подключила catch-all роутер ПОСЛЕДНИМ, и он отвечает «экран
устарел» на всё неопознанное. Твой обработчик обычного текста обязан встать ДО него и не
перехватывать то, что принадлежит другим — например ввод фамилии при регистрации (P3) или ответ
на вопрос о явке (P14). Проверь порядок роутеров грепом по живому `bot/app.py` и напиши в отчёт,
куда именно встал твой.
...
**КРИТЕРИЙ ГОТОВНОСТИ (может ПРОВАЛИТЬСЯ), каждая команда печатает ЧИСЛО:**
    ...
    python3 -m pytest tests/text -q
        # rc=0; печатает охват: 20 строк ввода, из них 3 неоднозначные;
        # ошибок 0 И УГАДАННЫХ 0 — угаданное считается ошибкой
    python3 -m pytest tests/text -q -k "petrov"
        # rc=0; ИМЕНОВАННЫЙ тест: «[7] Петров 3,5,7б» → листок 7, Петров, задачи 3, 5, 7б
    grep -c "token_set_ratio" core/services/bystryj_tekst.py ; echo "должно быть 0"
    grep -c "rapidfuzz" core/services/bystryj_tekst.py ; echo "0 тоже верно, если индекс ИМПОРТИРОВАН из P7 — тогда назови в отчёте, откуда"

🔴 Ноль разобранных строк при непустом наборе — КРАСНЫЙ, а не зелёный. Отрицательный вердикт несёт
охват В СЕБЕ: «ошибок 0, разобрано 20 из 20, угадано 0».
...
Верификатор нужен, тип — **ПОСЛЕ-типа** — судит результат, стоит в конце, после задачи. Свежий субагент, ДРУГИМ методом («[7] Петров 3,5,7б» разрешается в листок 7, ученика Петров и задачи 3, 5, 7б через ТОТ ЖЕ нечёткий индекс, что у фото; неоднозначная фамилия уходит в кнопки, а не угадывается), не перечитывает свою же правку. Доля сплошной выборки: 20 строк ввода, включая 3 неоднозначные, ошибок 0 и угаданных 0. Финальная строка ответа обязательна дословно: «выдано N позиций из M найденных» — без неё ответ считается усечённым и в отчёт не вставляется.
