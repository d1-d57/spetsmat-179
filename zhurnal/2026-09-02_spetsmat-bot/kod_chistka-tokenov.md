# Канал исполнителя — chistka-tokenov (один заход до конца)
...
> **Модель: Opus 5** — переписывание истории публичного репозитория с force-push: цена ошибки необратима, решения рождаются по ходу разведки.
...
Ты исполнитель в репозитории /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot-wt/chistka-tokenov.

Твой единственный вход — файл-заход:
/Users/ivanyakovlev/Documents/GitHub/spetsmat-bot/zhurnal/2026-09-02_spetsmat-bot/kod_chistka-tokenov.md
...
Первым ходом: python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py worktree add chistka-tokenov --branch zahod/chistka-tokenov, затем чтение захода, затем секция
...
Приоритет владельца: разобрать инциденты важнее, потом закрыть долги — неразобранный инцидент это повторяющаяся ошибка, долг может подождать.
  ...
  4. пунктов очереди «ДОСТАВЛЕНО: нет»   : 352
...
🔴 ДВЕРЬ НЕЗАКОММИЧЕННЫХ `kod_*.md` ОТКРЫТА КЛАПАНОМ: в арке `zhurnal/2026-09-02_spetsmat-bot` есть 3 незакоммиченных файлов-заходов `kod_storozh-vstal.md`, `kod_bekap-v-papku.md`, `kod_pages-i-materialy.md` — второй заход в такую арку обычно не собирается (долг 4 `disciplina-git`), но АНАЛИТИК открыл клапан, причина дословно: «в арке один незакоммиченный kod_*.md — kod_storozh-vstal.md, он ждёт приёмки аналитика в этой же сессии». Отключение видно здесь, в артефакте, а не осталось решением в голове аналитика.

КОНТЕКСТ. Репозиторий `d1-d57/spetsmat-179` ПУБЛИЧНЫЙ и таким остаётся: он публикует GitHub Pages, ссылка на который роздана детям. Решение владельца, записано, пересмотру не подлежит.

В четырёх отслеживаемых файлах лежат строки вида «токен телеграм-бота». Замер аналитика 08.09, командами:

```
git --no-optional-locks grep -lE "[0-9]{8,10}:[A-Za-z0-9_-]{35}" HEAD -- .        → 4 файла
git --no-optional-locks log --oneline --all -G"[0-9]{8,10}:[A-Za-z0-9_-]{35}"     → 2 коммита
```
Файлы: `zhurnal/2026-09-02_spetsmat-bot/VYGRUZKA-2026-09-02.md`, `VYGRUZKA-2026-09-03.md`, `VYGRUZKA-2026-09-03-2.md`, `VYGRUZKA-2026-09-03-3.md` — это выгрузки транскриптов сессий, в которые токен попал вместе с репликами.

🔴 **ЭТО ГИГИЕНА, А НЕ АВАРИЯ, И ПУТАТЬ НЕЛЬЗЯ.** Токен **отозван владельцем в @BotFather** — байты мертвы, воспользоваться ими нельзя. Значит спешить некуда, и ни одна причина не оправдывает риска сломать публичный репозиторий ради удаления мёртвых байт. Если по ходу выяснится, что чистка стоит дороже, чем грязь, — **сказать это и не делать** — законный и хороший исход захода.

ЦЕЛЬ: убрать строки из рабочих файлов и, если это безопасно, из истории.

🔴 **СТОП ДО ЦЕЛИ — и он здесь главный.** Ты НЕ получаешь права на `push --force` по своему решению. Разведка, план, оценка риска и правка ФАЙЛОВ — твои. Переписывание истории и force-push — **только после отдельного «да» владельца в чате, полученного ПОСЛЕ того, как ты покажешь ему план и цену**. Нет ответа — заход кончается на плане, и это полноценный успех.
...
1. Токен бота ОТОЗВАН владельцем в @BotFather — байты в истории мертвы, работа гигиеническая, а не аварийная
2. Репозиторий остаётся публичным, ссылка на Pages роздана детям и должна продолжать работать
3. Аналитик вынес эту работу в ОТДЕЛЬНЫЙ заход: переписывание истории и force-push — другой класс риска, чем правка двух файлов
4. Замер аналитика 08.09: строки лежат в 4 файлах в HEAD и затрагивают 2 коммита — объём малый, риск нет

## КОНТРАКТ ЗОНЫ (обязателен — не удалять; вписан Cowork)
- **МЕСТО РАБОТЫ:** **рабочая папка `../spetsmat-bot-wt/chistka-tokenov`** (рядом с репозиторием) — ТОЛЬКО ДЛЯ КОДА, worktree захода на ветке `zahod/chistka-tokenov`. 🔴 **ДАЛЬШЕ — ТОЛЬКО ПУТИ ОТНОСИТЕЛЬНО ЭТОЙ ПАПКИ** (или `cd` в неё безусловно, каждым ходом): абсолютный путь в главную папку репозитория здесь — типичная ошибка, правка утекает МИМО worktree и найдётся только на коммите («вне git» в `git_zona.py check --zone` из рабочей папки, на файле, который уже правил, — цена, оплаченная живьём: 5 файлов, ручное копирование и откат главной папки). 🔴 **Заводишь её САМ — ВТОРЫМ ходом, СРАЗУ ПОСЛЕ пункта 1 блока §0.1 ГИТ-КОНТУР ниже (не первым ходом захода целиком), из корня репозитория:**
  ```
  python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py worktree add chistka-tokenov --branch zahod/chistka-tokenov
  cd ../spetsmat-bot-wt/chistka-tokenov
  ```
...
- **ЗОНА (можно менять):** `zhurnal/2026-09-02_spetsmat-bot/`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/bootstrap_zahod.py --zadanie-subagentu --zone zhurnal/2026-09-02_spetsmat-bot/ --kommitit 'свою зону zhurnal/2026-09-02_spetsmat-bot/ по ходу работы' --zakryt 'ветку НЕ закрывать и в main НЕ вливать — влитие есть ход приёмки'
```
...
- Проверить, что на месте: `git rev-parse --abbrev-ref HEAD` → должно быть `zahod/chistka-tokenov`. Не оно — ты не в своей папке.
- ПЛАН/ВОПРОСЫ/ОТЧЁТ/УРОКИ ФАБРИКЕ пиши в ЭТОТ файл — он в основной папке, не копируй его в рабочую.
- Точка отката: `git add zhurnal/2026-09-02_spetsmat-bot/` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
- Прочитать ТОЛЬКО ЭТО. Проект сверх названного не изучай.
  1. Четыре файла зоны: `zhurnal/2026-09-02_spetsmat-bot/VYGRUZKA-2026-09-02.md`, `VYGRUZKA-2026-09-03.md`, `VYGRUZKA-2026-09-03-2.md`, `VYGRUZKA-2026-09-03-3.md` — только чтобы найти строки, не для изучения содержания.
  2. `.github/workflows/` — чем и с какой ветки публикуется Pages. 🔴 Ветка публикации знается в ДВУХ местах, и второе — настройки окружения `github-pages` в GitHub, которых в дереве не видно; сборка может проходить при отвергнутом деплое, и снаружи это выглядит как «просто не обновилось».
  3. `zhurnal/_INFRA-git/zayavki/na-zahod/2026-09-02T1431-02-09-14-2x-conduit179-bot.md` — заявка, которая этот долг и завела: чем именно установлено, что репозиторий публичный.
...
**A. ДО-верификатор снимает слепок — первым ходом, до всего.** Список всех веток с хэшами вершин, число коммитов, состояние каждой рабочей папки `spetsmat-bot-wt/*` (чисто/грязно), надгробия `mogila/*`, хэш `origin/main`. После переписывания истории этот слепок неисполним, а он — единственный способ доказать, что ничего не потеряно.

**B. Чистка ФАЙЛОВ — делается всегда, риска нет.** В четырёх названных файлах строка токена заменяется на заглушку вида `<токен вырезан 2026-09-08, отозван в @BotFather>`. Это обычный коммит, обычная история, откат тривиален. Здесь же — грепом проверить, не появились ли новые вхождения сверх четырёх известных файлов, и напечатать число.

**C. Разведка перед историей — цена, а не намерение.** Ответь КОМАНДАМИ, а не рассуждением:
   1. Сколько коммитов реально придётся переписать и какие ветки они затрагивают.
   2. Есть ли форки у репозитория (GitHub API). Форк унесёт старые объекты с собой, и чистка станет частичной — назвать это числом.
   3. Живы ли сейчас рабочие папки других заходов и их ветки: `push --force` по переписанной истории оставит их на осиротевших коммитах.
   4. Что произойдёт с публикацией Pages: с какой ветки она собирается и переживёт ли переписывание.
   5. Установлен ли `git-filter-repo`; если нет — как его ставить и чем это грозит.
🔴 **Ответ «риска нет» без этих пяти чисел не принимается.**

**D. План — владельцу, в чат, до всякого force-push.** Короткий: что переписываем, что теряется, как откатываемся, сколько это стоит по времени. Дальше — ЖДАТЬ. Владелец не читает файлы-заходы по устройству, поэтому спрашивать надо в чате, а не строкой в `## ВОПРОСЫ`.

**E. Исполнение — только по «да».** Бэкап `.git` целиком ДО, проверка после, восстановление проверено на копии ПРЕЖДЕ, чем трогается оригинал.

**КРИТЕРИЙ ГОТОВНОСТИ (может ПРОВАЛИТЬСЯ):**

1. **Живой прогон на реальном репозитории, не на фикстуре:** после чистки файлов `git --no-optional-locks grep -lE "[0-9]{8,10}:[A-Za-z0-9_-]{35}" HEAD -- zhurnal/2026-09-02_spetsmat-bot/ | wc -l` → **0** (на входе было 4). Путь ограничен зоной намеренно: греп по всему дереву дал бы ноль и по причине «нигде нет», и по причине «искал не там», а различить их потом нечем. Оба числа, входное и выходное, печатаются рядом.
2. Слепок ДО снят и приложен: ветки с хэшами, число коммитов, состояние всех рабочих папок. Печатается охват «веток N из N».
3. Пять чисел разведки §C получены командами, каждое с командой рядом. Нет числа — клауза красная.
4. План показан владельцу в чате, и в отчёте записано, что он ответил. **«Не ответил» — законный исход, и тогда клаузы 5–6 не применяются, а заход считается закрытым на плане.**
5. Если «да» получено: бэкап `.git` снят, восстановление из него проверено НА КОПИИ до правки оригинала, оба факта с командами.
6. Если история переписана: `origin/main` и локальный `main` совпадают, все рабочие папки соседних заходов проверены на осиротевшие коммиты, Pages после публикации отдаёт `200` — все числа печатаются.
7. `git_zona.py check --zone zhurnal/2026-09-02_spetsmat-bot/` → ✅.

**ОХВАТ:** финальная строка «проверено X из 7 клауз; клаузы 5–6 применимы/неприменимы — почему».

**Отрицательный вердикт несёт ОХВАТ В СЕБЕ.**

🔴 **ЧЕГО НЕ ДЕЛАТЬ.**
- **Никакого `push --force` без явного «да» владельца в чате.** Это главный запрет захода. Молчание — не согласие.
- Не делать репозиторий приватным и не удалять Pages: ссылка роздана детям.
- Не трогать ветки и рабочие папки соседних заходов.
- Не трогать `veb/`, `ops/`, `deploy/`, `docs/` — вне зоны. Заход про историю и четыре файла журнала.
- Не чинить `veb/vhod.py` и nginx: запрос на проверку в Search Console владелец уже отправил, любая правка там может её провалить.
- Токен в вывод, в логи и в отчёт не печатать — ни целиком, ни куском. Заменяешь его, а не цитируешь.
...
Верификатор нужен, тип — **ДО-типа** — судит СОСТОЯНИЕ, которое работа уничтожит, и идёт ПЕРВЫМ ходом, до самой работы: после неё он неисполним. Свежий субагент, ДРУГИМ методом (свежий субагент ДО работы снимает полный слепок состояния: список веток с хэшами, число коммитов, содержимое рабочих папок соседних заходов — после переписывания истории это состояние неисполнимо), не перечитывает свою же правку. Доля сплошной выборки: сплошная выборка 100 процентов: все ветки из всех, все рабочие папки из всех. Финальная строка ответа обязательна дословно: «выдано N позиций из M найденных» — без неё ответ считается усечённым и в отчёт не вставляется.
...
```
git --no-optional-locks add -- zhurnal/2026-09-02_spetsmat-bot/                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- zhurnal/2026-09-02_spetsmat-bot/   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `zhurnal/2026-09-02_spetsmat-bot/`

- **Г1. Зона доехала в git.** `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone zhurnal/2026-09-02_spetsmat-bot/` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py vlit-v-osnovnuyu zahod/chistka-tokenov --zone <своя зона> \
...
виден»: прогон изменённого механизма из `/Users/ivanyakovlev/Documents/GitHub/spetsmat-bot`, НЕ из рабочей папки `/Users/ivanyakovlev/Documents/GitHub/spetsmat-bot-wt/chistka-tokenov` плюс `grep` по ЖИВОМУ файлу,
...
**ВЕТКА РАБОТЫ:** `zahod/chistka-tokenov`
