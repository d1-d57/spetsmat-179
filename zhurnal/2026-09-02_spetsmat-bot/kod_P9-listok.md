...
**ВЕРДИКТ:** `принято` — ПРЕЖНИЙ ВЕРДИКТ БЫЛ «ДОРАБОТКА» И ОСТАЁТСЯ ВЕРНЫМ ДЛЯ ТОГО МОМЕНТА: окно висело мёртвым 75 минут после `rc=0` и умерло, не написав отчёта (Г1 Г2 Г3 Г4 красные). Добор дописал отчёт И сделал больше, чем просили: 12 тестов доведены до 22, а сверка перестала оценивать сама себя — коммит «the kind comes off the glyphs, so the reconciliation stops grading itself» убирает тавтологию в собственном оракуле, тот же класс, что закрывала P2. ПРОВЕРЕНО МНОЙ, А НЕ ОТЧЁТОМ: сверка печатает охват сама — «сверено 544 задач из 544, расхождений 0», и это число НЕЗАВИСИМО сошлось с замером P4 («кнопок задач 544 из 544»), то есть две позиции пришли к одному счёту разными путями. Красноту доказал экспериментом: убрал одну букву из класса букв в метке (`core/services/sheets.py`) — 22 passed стали 22 errors, вернул — снова 22 passed. Человеческие ворота на месте: `test_teacher_is_refused_at_confirm_gate` — разборщик отдаёт черновик старшему, а в базу сам не пишет.
...
заявок нет: влитие сделано самим заходом, коммит `4cf605d` на месте, вывоз непроверяем (удалённых нет), деплой вне зоны (P10), гашение зависшего окна сделано приёмкой руками по номеру процесса и в очередь не просится.
