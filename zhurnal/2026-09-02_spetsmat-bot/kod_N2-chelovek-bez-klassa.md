# Канал исполнителя — N2-chelovek-bez-klassa (один заход до конца)
...
```
cd /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot && python3 /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot/_generator/tools/orkestr.py zhurnal/2026-09-02_spetsmat-bot --rezhim progon --dvizhok opencode --rod instrumenty --model openrouter/z-ai/glm-5.2:free --zahody kod_N2-chelovek-bez-klassa.md < /dev/null 2>&1 | tee /tmp/zahod-N2-chelovek-bez-klassa.log
```
...
ЦЕЛЬ: дать инструмент, называющий подозрительные строки состава и НЕ удаляющий ничего сам.
Приёмка — по ОТЧЁТУ, без построчной сверки. Стоп до цели: ты получишь ИНСТРУМЕНТ и его ловушки, но НЕ будешь править состав — кто ушёл и кто пришёл, решает владелец.
...
3. владелец про Аракелову дословно: «это вообще не ребенок, это наш преподаватель, который просто был записан как тестовый слушатель»
4. владелец про состав дословно: «у нас несколько детей ушло, несколько добавилось, произошли изменения в классе, и с ними нужно работать и работать». Значит задача НЕ угадать состав, а предъявить подозрительные строки

## КОНТРАКТ ЗОНЫ (обязателен — не удалять; вписан Cowork)
- **МЕСТО РАБОТЫ:** ветка `zahod/N2-sostav` в основной папке. 🔴 Она должна УЖЕ стоять. НЕ на ней — СТОП, НЕ делай `git checkout`: в общей папке он МОЛЧА откатывает дерево к состоянию ветки (цена 27→28.07: файл сильно откатился ночью, поймал владелец вручную; след в git НЕ остаётся). Тогда заход пересобрать с `--worktree`. Ветку не переключай, в другие НЕ коммить.
- **ЗОНА (можно менять):** `tools/proverka_sostava.py` `tests/import/test_proverka_sostava.py` `seed/` `zhurnal/2026-09-02_spetsmat-bot/kod_N2-chelovek-bez-klassa.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/bootstrap_zahod.py --zadanie-subagentu --zone tools/proverka_sostava.py --zone tests/import/test_proverka_sostava.py --zone seed/ --kommitit 'инструмент и его тесты одним коммитом — инструмент без ловушек не принимается' --zakryt 'ветку zahod/N2-sostav после переноса'
```
...
- Проверь ветку: `git branch --show-current` — обязано быть `zahod/N2-sostav`. Не она — СТОП, `git checkout` НЕ делай (§4 GIT-disciplina), нужен `--worktree`.
- Точка отката: `git add tools/proverka_sostava.py tests/import/test_proverka_sostava.py seed/ zhurnal/2026-09-02_spetsmat-bot/kod_N2-chelovek-bez-klassa.md` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
Верификатор нужен, тип — **ПОСЛЕ-типа** — судит результат, стоит в конце, после задачи. Свежий субагент, ДРУГИМ методом (прогнать инструмент на КОПИИ боевой базы и сверить его вывод со списком, снятым независимым SQL, а не перечитыванием своего же кода), не перечитывает свою же правку. Доля сплошной выборки: сплошная: все 56 строк students и все 19 teachers. Финальная строка ответа обязательна дословно: «выдано N позиций из M найденных» — без неё ответ считается усечённым и в отчёт не вставляется.
...
```
git --no-optional-locks add -- tools/proverka_sostava.py tests/import/test_proverka_sostava.py seed/ zhurnal/2026-09-02_spetsmat-bot/kod_N2-chelovek-bez-klassa.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- tools/proverka_sostava.py tests/import/test_proverka_sostava.py seed/ zhurnal/2026-09-02_spetsmat-bot/kod_N2-chelovek-bez-klassa.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `tools/proverka_sostava.py` `tests/import/test_proverka_sostava.py` `seed/` `zhurnal/2026-09-02_spetsmat-bot/kod_N2-chelovek-bez-klassa.md`

- **Г1. Зона доехала в git.** `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone tools/proverka_sostava.py` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone tests/import/test_proverka_sostava.py` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone seed/` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone zhurnal/2026-09-02_spetsmat-bot/kod_N2-chelovek-bez-klassa.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py vlit-v-osnovnuyu zahod/N2-sostav --zone <своя зона> \
