...
**ВЕРДИКТ:** принято — доказано числом на БОЕВОМ сайте: у гостя на `/raspredelenie` было 48 обрезанных фамилий, стало 0, скролла нет. Продукт `veb/razdely/verstka_stili.py` — общая сетка колонок вместо замороженного `flex:0 0 9.6rem`, 512 строк. Заход снят потолком фоновой задачи, не дописав отчёт; влитие `e1aa86c` и проверку сделал оркестратор
