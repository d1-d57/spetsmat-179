# Канал исполнителя — dannye-07-09 (один заход до конца)
...
> **Модель: openrouter/thinkingmachines/inkling:free** — дверь внесения уже написана и сама отказывает на несовпадении — исполнителю остаётся прогнать её и назвать числа.
...
```
Модель: openrouter/thinkingmachines/inkling:free — см. обоснование в шапке захода, §0.

Ты исполнитель в репозитории /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot-wt/dannye-07-09.

Твой единственный вход — файл-заход:
/Users/ivanyakovlev/Documents/GitHub/spetsmat-bot/zhurnal/2026-09-02_spetsmat-bot/kod_dannye-07-09.md
...
Первым ходом: python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py worktree add dannye-07-09 --branch zahod/dannye-07-09, затем чтение захода, затем секция
...
Приоритет владельца: разобрать инциденты важнее, потом закрыть долги — неразобранный инцидент это повторяющаяся ошибка, долг может подождать.
  ...
  4. пунктов очереди «ДОСТАВЛЕНО: нет»   : 567
...
🔴 ДВЕРЬ НЕЗАКОММИЧЕННЫХ `kod_*.md` ОТКРЫТА КЛАПАНОМ: в арке `zhurnal/2026-09-02_spetsmat-bot` есть 7 незакоммиченных файлов-заходов `kod_dobivka-vecher.md`, `kod_gejt-vse-ekrany.md`, `kod_konduit-i-raspredelenie.md`, `kod_melochi-i-dovoz.md`, `kod_nadzor-zvonit.md`, `kod_offlajn-ochered.md`, `kod_zhurnaly-i-kabinet.md` — второй заход в такую арку обычно не собирается (долг 4 `disciplina-git`), но АНАЛИТИК открыл клапан, причина дословно: «ночная волна поверх незакоммиченных заходов дня». Отключение видно здесь, в артефакте, а не осталось решением в голове аналитика.
...
1. Романчук за 07.09: листок 16A, задачи 1а и 1б — ответ владельца
2. владелец был на занятии 10.09, а в полосе кабинета оно не отмечено — починить причину, а не покраску
3. ничего не угадывать: не сошлось — отказать и напечатать, чего не нашла дверь

## КОНТРАКТ ЗОНЫ (обязателен — не удалять; вписан Cowork)
- **МЕСТО РАБОТЫ:** **рабочая папка `../spetsmat-bot-wt/dannye-07-09`** (рядом с репозиторием) — ТОЛЬКО ДЛЯ КОДА, worktree захода на ветке `zahod/dannye-07-09`. 🔴 **ДАЛЬШЕ — ТОЛЬКО ПУТИ ОТНОСИТЕЛЬНО ЭТОЙ ПАПКИ** (или `cd` в неё безусловно, каждым ходом): абсолютный путь в главную папку репозитория здесь — типичная ошибка, правка утекает МИМО worktree и найдётся только на коммите («вне git» в `git_zona.py check --zone` из рабочей папки, на файле, который уже правил, — цена, оплаченная живьём: 5 файлов, ручное копирование и откат главной папки). 🔴 **Заводишь её САМ — ВТОРЫМ ходом, СРАЗУ ПОСЛЕ пункта 1 блока §0.1 ГИТ-КОНТУР ниже (не первым ходом захода целиком), из корня репозитория:**
  ```
  python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py worktree add dannye-07-09 --branch zahod/dannye-07-09
  cd ../spetsmat-bot-wt/dannye-07-09
  ```
...
- **ЗОНА (можно менять):** `tools/vnesti_s_bumagi.py` `tests/veb/`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/bootstrap_zahod.py --zadanie-subagentu --zone tools/vnesti_s_bumagi.py --zone tests/veb/ --kommitit 'свою зону по ходу; отметки в базе коммита не оставляют — печатать свидетеля' --zakryt 'ветку zahod/dannye-07-09 через poteri после влития; рабочую папку снять'
```
...
- Проверить, что на месте: `git rev-parse --abbrev-ref HEAD` → должно быть `zahod/dannye-07-09`. Не оно — ты не в своей папке.
- ПЛАН/ВОПРОСЫ/ОТЧЁТ/УРОКИ ФАБРИКЕ пиши в ЭТОТ файл — он в основной папке, не копируй его в рабочую.
- Точка отката: `git add tools/vnesti_s_bumagi.py tests/veb/` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
### ДАННЫЕ ЗА 07.09 И 10.09. ДВЕРЬ УЖЕ НАПИСАНА — ТВОЁ ДЕЛО ПРОГНАТЬ ЕЁ И НАЗВАТЬ ЧИСЛА

🔴 **ПЕРВЫМ ШАГОМ — КОПИЯ БОЕВОЙ БАЗЫ штатной дверью.** И помни: база вне git, коммита эта
работа НЕ оставит. Свидетелем служит печать `SvidetelRaboty` — дата последней записи ДО и ПОСЛЕ
и число затронутых строк. Без неё работа неотличима от отсутствия работы: 10.09 аналитик уже
объявил внесённые отметки невнесёнными, потому что искал копию не там.

**ПЕРВОЕ. РОМАНЧУК ТИМОФЕЙ ЗА 07.09 — единственный из одиннадцати с нулём.** Ответ владельца: листок
**16A**, задачи **1а** и **1б**. Основание: `1а` и `1б` есть только в 16A, в 16α первая задача —
просто `1`. Команда:
```
python3 tools/vnesti_s_bumagi.py --den 2026-09-07 --shkolnik Романчук --listok 16A --zadachi 1а,1б --prepodavatel "Наталия Стрелкова" --pochemu "бумажный кондуит 07.09, ответ владельца 10.09" --da
```
Без `--da` она делает пробу и ничего не пишет — прогони сначала пробу.
🔴 **Метки проверит сама дверь на БОЕВОЙ базе. Если `1а` или `1б` там не окажется — она откажет.
НЕ обходи отказ, покажи его владельцу:** второе чтение рукописи («16ℵ, задачи −1а и −1в»)
сходится не хуже, и различает их только бумага.

**ВТОРОЕ. ЯВКА ВЛАДЕЛЬЦА НА 10.09.** Он был на занятии, а в полосе кабинета оно не зелёное.
🔴 **Сначала выясни, ПОЧЕМУ явка не доехала, и только потом ставь.** Если причина в механизме —
назови её и не заметай отметкой руками: покрашенный симптом вернётся в следующий четверг.

**ТРЕТЬЕ. ЮСУФОВ АРОН В ЧЕТВЕРГ НИ К КОМУ НЕ ПРИКРЕПЛЁН** — решение владельца 10.09, правка данных.

**ЧЕТВЁРТОЕ. 🔴 И ГЛАВНОЕ ТРЕБОВАНИЕ ВЛАДЕЛЬЦА К САМОЙ ДВЕРИ:** «должен быть понятный инструмент, чтобы
из Cowork можно было вносить в базу плюсики кому-то. Это важно». Прогони дверь и посмотри на её
вывод ГЛАЗАМИ ВЛАДЕЛЬЦА: непонятная строка, неудобный аргумент, невнятный отказ — правь ДВЕРЬ,
она в зоне. Дверь, которой пользуется человек раз в неделю, обязана объяснять себя сама.

### ЧЕГО НЕ ДЕЛАТЬ
Ничего не угадывать: не сошлось — отказать и напечатать, чего не нашла. Не писать `insert` в
`marks` руками: журнал append-only, «поставить галочку» не равно «вставить строку». Не отвечать
за владельца на четыре открытых вопроса (приписка «2 пункт» у Верхошинского · листок «16ба» у
Виляева · отсутствие Юсуфова на листе Шнитке · править ли слой занятия Виляеву) — они его.

**КРИТЕРИЙ ГОТОВНОСТИ (может ПРОВАЛИТЬСЯ):** живой прогон на реальном объекте — на боевой базе
через дверь, не на фикстуре. Пары чисел: Романчук было 0 клеток → стало 2, номеров не найденных
в листке 0 · явка 10.09 не зелёная → зелёная, и причина названа · свидетель напечатал дату
последней записи ДО и ПОСЛЕ и число строк. **Отрицательный вердикт несёт ОХВАТ В СЕБЕ.**
...
Верификатор не нужен: дверь сама печатает свидетеля — дату последней записи ДО и ПОСЛЕ и число строк; это и есть проверка.
...
```
git --no-optional-locks add -- tools/vnesti_s_bumagi.py tests/veb/                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- tools/vnesti_s_bumagi.py tests/veb/   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `tools/vnesti_s_bumagi.py` `tests/veb/`
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py vlit-v-osnovnuyu zahod/dannye-07-09 --zone <своя зона> \
...
виден»: прогон изменённого механизма из `/Users/ivanyakovlev/Documents/GitHub/spetsmat-bot`, НЕ из рабочей папки `/Users/ivanyakovlev/Documents/GitHub/spetsmat-bot-wt/dannye-07-09` плюс `grep` по ЖИВОМУ файлу,
...
**АРТЕФАКТ (ДОПИСАНО ОРКЕСТРАТОРОМ НА ПРИЁМКЕ, исполнитель вышел без отчёта):** `/Users/ivanyakovlev/Documents/GitHub/spetsmat-bot/scratchpad/dannye-07-09/ready_commands.md` — готовые команды внесения для утреннего прогона на сервере; рядом `paper_07-09.md` — разбор бумаги с источником у строки. Правка двери — `tools/vnesti_s_bumagi.py` (коммит 9a43f89).

**ВЕРДИКТ:** `доработка` — РАБОТА ЕСТЬ И ГЛАВНЫЙ ЗАПРЕТ СОБЛЮДЁН: data/spetsmat.db не менялась ночью (mtime 10.09 17:00), в локальную базу не внесено ничего, F8 закрыта правкой человекочитаемых отказов в tools/vnesti_s_bumagi.py (9a43f89), черновик бумаги и готовые команды лежат в scratchpad/dannye-07-09/. ДОРАБОТКА ЗА ДВА: (1) секция ## ОТЧЁТ осталась плейсхолдерами — артефакт, коммит и род не названы, позиция вышла без отчёта; (2) готовых команд ДВЕ (16A: 1а, 1б), тогда как владелец говорит про одиннадцать отметок Романчука — перечень бумаги неполон и его надо добрать с бумаги утром

**ВЕТКА РАБОТЫ:** `zahod/dannye-07-09`
...
### ПРАВКА 1 (2026-09-11 00:38, от владельца через оркестратора) — 🔴 БОЕВОЙ БАЗЫ ЗДЕСЬ НЕТ. В КОПИЮ НЕ ВНОСИТЬ

ЗАМЕРЕНО ОРКЕСТРАТОРОМ 11.09 00:36, одним ходом:
* `ls -d /srv/spetsmat/data` → `No such file or directory`
* `env | grep -i spetsmat` → переменной `SPETSMAT_BAZA` нет
* `~/.ssh/config` → ни одного `Host` кроме `Host *`; живого туннеля в процессах нет
* `python3 core/istochnik.py` → отказ «ИСТОЧНИК НЕ НАЗВАН»

Владелец 11.09 дословно: отметки обязаны лечь в БОЕВУЮ базу, она на сервере; локальная
копия мёртвая. **Внесение в копию — второй источник правды, худший исход из возможных.**

ЧТО ЭТО ЗНАЧИТ ДЛЯ ТЕБЯ:
1. 🔴 Ни в какую локальную базу НИЧЕГО не вноси. `--da` не запускай ВООБЩЕ. Не заводи
   копию, не выставляй `SPETSMAT_BAZA` на локальный файл ради того, чтобы дверь прошла.
2. Прогони дверь В РЕЖИМЕ ПРОБЫ (без `--da`) и вставь её вывод в `## ОТЧЁТ` ДОСЛОВНО —
   включая отказ, если дверь откажет. Отказ двери на мёртвой базе это ПРАВИЛЬНОЕ
   поведение и законный результат замера, а не твоя неудача.
3. Сделай то, что от боевой базы НЕ зависит, и в этом вся твоя ценность этой ночью:
   а) разбери бумажный кондуит 07.09 (Романчук, явка владельца 10.09, Юсуфов) в ЧЕРНОВИК
      — файл в репозитории, человекочитаемый, с источником у каждой строки;
   б) выпиши ГОТОВЫЕ КОМАНДЫ внесения, строка в строку, по одной на отметку, с полными
      `--den --shkolnik --listok --zadachi --prepodavatel --pochemu` — так, чтобы утром
      их можно было выполнить на сервере ОДНОЙ вставкой и без раздумий;
   в) закрой F8 — дверь внесения должна быть ПОНЯТНА ЧЕЛОВЕКУ: её отказы и подсказки
      читает не программист. Это правка текстов двери, она локальна и не трогает данные.
4. В `## ОТЧЁТ` ОТДЕЛЬНОЙ первой строкой напиши: «ВНЕСЕНИЕ НЕ ВЫПОЛНЕНО: боевой базы на
   машине нет» — и приведи замер выше. Молчание об этом хуже невнесения.
5. Свидетелем зови `SvidetelRaboty` (`core/istochnik.py:314`): дата последней записи ДО и
   ПОСЛЕ и число строк. Если базы нет вовсе — так и напиши, числом «н/д» с причиной.
   Без свидетеля 10.09 аналитик объявил внесённые отметки невнесёнными.
