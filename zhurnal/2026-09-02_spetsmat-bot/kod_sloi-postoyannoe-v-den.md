...
1. Прогон `python3 -m pytest tests/veb` ПЕРЕПИСЫВАЕТ отслеживаемый в git файл `data/spetsmat.db`, и зелёные тесты оставляют после себя грязное дерево вне зоны любого захода. Механизм: обработчик правки зовёт `_peresobrat()` → `tools/sobrat_stranicu.sobrat()` → `infra.db.connect()` БЕЗ пути, то есть `config.DB_PATH`, а не временную базу теста; дальше `infra.prepodavatel_den_repo.obespechit` заводит в ней таблицу `prepodavatel_ne_prihodit`. Замерено на ЧИСТОМ дереве, до всякой правки этого захода (`git archive HEAD~2`): sha1 базы 377276a3 → 92c40d34, размер 3928064 → 3936256, появилась ровно одна таблица. Файл забирается в git НАМЕРЕННО (`.gitignore:14` — `!data/spetsmat.db`), поэтому чинить надо не `.gitignore`, а адрес базы в пересборке. Правка невелика — передать путь базы в `sobrat()`, — но живёт в `tools/` и `veb/obshchee/`, вне зоны этого захода.
   ...
   ДОСТАВЛЕНО: нет
2. Разметку кнопки «применить постоянное к этому дню» этот заход НЕ ставил — она вне его зоны (`veb/obshchee/karkas.py`, `veb/razdely/shkolniki.py`). Ставить её и не пришлось: пока шла работа, соседняя позиция влила её в `main` сама (`karkas.py:1156` — кнопка, `karkas.PRIMENIT_SKRIPT` — её скрипт), и обращается она ровно к этим двум дверям. Сквозной прогон на копии боевой базы показал, что связка работает целиком: у организатора кнопка есть, у гостя нет, при нуле перекрытий прячется сама (числа — в `## ОТЧЁТ`). Одно расхождение стыка — подсказка читала список как строки, а дверь отдавала объекты — починено в СВОЕЙ зоне коммитом `7413c03`. ЧЕЛОВЕКУ ОСТАЁТСЯ одно: на боевом сайте эту кнопку ещё никто не нажимал (пункт 3).
   ДОМ: владелец
   ДОСТАВЛЕНО: нет
...
4. На боевой базе 10.09 слой занятия несёт восемь ручных правок по принимающему при нуле отметок отсутствия, и на экране это число не видно нигде. Показывать ли его постоянно («на этом дне восемь ручных правок»), а не только в подтверждении кнопки, — вопрос оформления, и он за владельцем: именно невидимость этого числа и превратила исправную работу двух слоёв в жалобу «правка постоянного не доезжает».
   ДОМ: владелец
   ДОСТАВЛЕНО: нет
5. Отметка присутствия из бота СТИРАЕТ ручную правку распределения на день. `bot/routers/text_input.py:582` зовёт `sessions.mark_attendance(lesson.id, student_id, tekst.PRESENT)` без `teacher_id`, а `infra/sessions_repo.py:133` выполняет `update attendance set status = ?, teacher_id = ?` — то есть кладёт `NULL` поверх переопределения. Ребёнок, отданный на один раз другому принимающему, возвращается к постоянному сам, как только кто-нибудь отметит его пришедшим, и число на кнопке «применить постоянное» молча уменьшается по ходу занятия. Найдено верификатором прогоном (`до mark(): teacher_id=4 → после: None`), проверено мной по коду отдельно. Соседний писатель той же таблицы, `core/services/room.set_present`, делает это ПРАВИЛЬНО — переносит `member.today_teacher_id`, и в его комментарии прямо стоит «marking a child present must not silently undo a move made a minute ago». Оба файла вне зоны этого захода.
   ДОМ: владелец
   ДОСТАВЛЕНО: нет
...
**3. `tests/sostav/test_primenit_postoyannoe.py` — четырнадцать тестов** на настоящей схеме и
настоящей веб-двери: воспроизведённый симптом, три сценария критерия, инвариант отметок
отсутствия, тождество показанного числа и снятого, и три теста на находки верификатора.
...
после     : $ python3 -m pytest tests/sostav -q  →  30 passed in 10.78s
```
Шире зоны ничего не сломано, и это тоже измерено, а не обещано: на ЧИСТОМ дереве до всякой
правки (`git archive HEAD~2` во временную папку) `pytest tests/veb tests/room` даёт
«15 failed, 152 passed, 1 xfailed, 43 errors»; с этой работой
`pytest tests/sostav tests/veb tests/room` даёт «15 failed, 182 passed, 1 xfailed,
43 errors». Те же 15 падений и те же 43 ошибки — унаследованный чужой долг, не мой;
прибавились ровно 30 моих зелёных (182 − 152).
...
1. **`git checkout -- data/spetsmat.db` в рабочей папке — ДВАЖДЫ** (файл отслеживается
   намеренно, `.gitignore:14`). Что: каждый раз снята МОЯ же случайная модификация — её
   делает прогон `pytest tests/veb`, см. пункт 1 `## ВОПРОСЫ`. Содержимое при этом не
   менялось ни разу: `sessions 0 · attendance 0 · enrollment 108` до и после, разница —
   одна служебная таблица `prepodavatel_ne_prihodit`, которую репозиторий заводит сам.
   Восстанавливается из коммита `bdf2dd6` (откуда и восстановлено оба раза).
   Файл в зону этого захода не входит, и своей работы в нём нет ни байта.
...
**КОММИТ:** `898fcd9` — `sostav+veb: the three findings of this заход's verifier, closed in the zone` · `git_zona.py check --zone veb/server.py` → ✅ · `--zone core/services/sostav_na_den.py` → ✅ · `--zone tests/sostav/` → ✅

Коммитов у захода четыре, по ходу работы, каждая часть отдельно — как требует §4:

| хэш | что |
|---|---|
| `8048925` | служба: `Mesto.perekryt_prepodavatelem` и `perekrytiya_k_snyatiyu` с тремя отказами |
| `958e31f` | две двери кнопки + одиннадцать тестов |
| `7413c03` | подсказка кнопки получает имена, а не объекты (живой дефект на стыке с чужой разметкой) |
| `898fcd9` | три находки верификатора, закрытые в зоне, + три теста (итого 30) |

Влито в `main`: `2eb79d9`, `af3c53d` (и `c65be4d` — не мной, см. ГИТ-ГИГИЕНУ).
Рабочая папка `git status --porcelain` → 0.
...
**ВЕРДИКТ:** принято — диагноз верен и подтверждён владельцем на живом сайте («все нормально, кнопка есть»). Гейт 0 по зоне `core/services/sostav_na_den.py`, `tests/sostav` зелёный; ветка влита; `tests/sostav` 30 passed — ровно её число (16 → 30). Обе двери API прогнаны МНОЙ: `GET` вернул `n:3` с именами, `POST` — `snyato:3`, «с преподавателем» 3 → 0, повторный `GET` — `n:0`. ВТОРУЮ ПОЛОВИНУ (разметку кнопки) ДОДЕЛАЛ ОРКЕСТРАТОР: коммит назывался «the button», но трогал только `server.py` и тесты — это первым увидел ВЛАДЕЛЕЦ, не я и не позиция. Разметка легла в `veb/obshchee/karkas.py` (зона уже закончившей `karkas-menyu-kabinet`), кнопка есть на пяти вкладках дня, на «Постоянном» и у гостя её нет. В ПЛЮС позиции: сама нашла два дефекта сверх задачи, включая свой стык с моей разметкой (её дверь отдавала объекты, чужой скрипт ждал строки — человек прочитал бы `[object Object]`), и сама назвала, что осталось человеку. РАСХОЖДЕНИЕ ЧИСЕЛ ОБЪЯСНЕНО: её «8» — все ручные строки дня, моё «5» — из них расходящиеся с постоянным; на 12:14 стало 7 и 4, потому что базу правят прямо сейчас. Оба замера верны и отвечают на разные вопросы
