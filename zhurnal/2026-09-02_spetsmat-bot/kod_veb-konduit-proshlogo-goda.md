# Канал исполнителя — veb-konduit-proshlogo-goda (один заход до конца)
...
```
cd /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot-wt/veb-konduit-proshlogo-goda && GIT_ZONA_REPO=/Users/ivanyakovlev/Documents/GitHub/spetsmat-bot opencode run --auto --model openrouter/minimax/minimax-m3:free 'Модель: openrouter/minimax/minimax-m3:free — решение владельца 03.09: бесплатная везде, платная только эскалацией после провала.

Ты исполнитель в репозитории /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot-wt/veb-konduit-proshlogo-goda.

Твой единственный вход — файл-заход:
/Users/ivanyakovlev/Documents/GitHub/spetsmat-bot/zhurnal/2026-09-02_spetsmat-bot/kod_veb-konduit-proshlogo-goda.md
...
работы; git-контур §0.1 — законное исключение и исполняется целиком, включая
влитие названной там чужой ветки ПЕРВЫМ ходом.
...
твой контекст впустую.

Первым ходом — git-контур §0.1, затем чтение захода, затем ## ПЛАН.
Рабочая папка у тебя уже заведена: /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot-wt/veb-konduit-proshlogo-goda.' < /dev/null 2>&1 | tee /tmp/zahod-veb-konduit-proshlogo-goda.log
```
...
Приоритет владельца: разобрать инциденты важнее, потом закрыть долги — неразобранный инцидент это повторяющаяся ошибка, долг может подождать.
  ...
  4. пунктов очереди «ДОСТАВЛЕНО: нет»   : 191
...
🔴 ДВЕРЬ НЕЗАКОММИЧЕННЫХ `kod_*.md` ОТКРЫТА КЛАПАНОМ: в арке `zhurnal/2026-09-02_spetsmat-bot` есть 2 незакоммиченных файлов-заходов `kod_veb-raspredelenie-mvp.md`, `kod_vykatka-tunnel-i-storozh.md` — второй заход в такую арку обычно не собирается (долг 4 `disciplina-git`), но АНАЛИТИК открыл клапан, причина дословно: «грязь — живые исполнители P1 и P5, пишущие ПЛАН и ОТЧЁТ в свои файлы-заходы прямо сейчас; ждать их чистоты значит не собрать веер за всю волну». Отключение видно здесь, в артефакте, а не осталось решением в голове аналитика.

КОНТЕКСТ. Телеграм-бот кондуита спецмата 179-й школы. Кондуит прошлого года УЖЕ импортирован
в `data/spetsmat.db` — импортировать нечего, всё лежит. Соседний заход `veb-raspredelenie-mvp`
построил над этой базой первую веб-страницу (папка `veb/`, сервер на стандартной библиотеке); его
ветка влита в твою первым ходом.
ЦЕЛЬ: дать этому архиву ЧИТАЮЩУЮ страницу — кто что сдал за прошлый год.
Приёмка — по ОТЧЁТУ, без построчной сверки. Если стоп до цели: получишь сводку по листкам, но НЕ
получишь разреза по ученику.
...
- `cd /Users/ivanyakovlev/Documents/GitHub/spetsmat-bot-wt/veb-konduit-proshlogo-goda` — рабочая папка ДЛЯ КОДА. Ветку НЕ переключай: `veb-konduit-proshlogo-goda` в ней уже стоит.
- Проверить, что на месте: `git rev-parse --abbrev-ref HEAD` → должно быть `veb-konduit-proshlogo-goda`.
- ПЛАН/ВОПРОСЫ/ОТЧЁТ/УРОКИ ФАБРИКЕ пиши в ЭТОТ файл — он в основной папке, не копируй его в рабочую.
- Точка отката: `git add veb/konduit.py veb/static/konduit.css tests/veb/test_konduit.py` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
| файл | зачем |
|---|---|
| `veb/server.py` и остальное в `veb/` | работа соседа, влитая тебе первым ходом: как устроен сервер и куда встраивать свою страницу |
| `core/services/progress.py` | проекция «сдано». Считать сданное — через неё, руками события не складывать |
| `core/models.py`, `config.py` | модели и пути |

**НЕ ТРОГАЙ каталог листков.** Он собран и выложен отдельным репозиторием `spetsmat-179`,
эта волна его не пересобирает и не перекрашивает.

### 🔴 СХЕМА И ЧИСЛА, СНЯТЫЕ ОРКЕСТРАТОРОМ С ЖИВОЙ БАЗЫ 03.09 — НЕ ПЕРЕСЧИТЫВАЙ, СВЕРЯЙСЯ

```
students  56    (id, tg_id, surname, name, class, status, first_sheet_id)
sheets    18    (id, number, title, issued_at, ord)
problems  544   (id, sheet_id, label, kind, ord)
teachers  19    (id, tg_id, name, aka, is_owner)
marks     15847 (id, student_id, problem_id, session_id, event, reverses_id,
                 teacher_id, valid_at, recorded_at, source, note, idempotency_key)
```

🔴 **`marks` — ЭТО ЖУРНАЛ СОБЫТИЙ, А НЕ ТАБЛИЦА «СДАНО».** В нём `event='assert'` — 15 112 строк
и `event='retract'` — 735 строк, и `retract` СНИМАЕТ более раннюю отметку (`reverses_id`).
Кто посчитает сданное как `count(*)` по `marks`, получит 15 847 вместо правильного числа и
покажет владельцу 735 несуществующих решённых задач. Считать — через `core/services/progress.py`,
она для этого и написана.
...
проверь числом: `python3 -c "import sqlite3;print(sqlite3.connect('data/spetsmat.db').execute('select count(*) from marks').fetchone())"`
→ обязано напечатать `(15847,)`. Напечатало `(0,)` — СТОП, ты на пустой копии.

### ЗАДАЧА — ТРИ ФАЙЛА, СТРАНИЦА ТОЛЬКО НА ЧТЕНИЕ

**A. `veb/konduit.py` — два разреза одного архива.**

- **Разрез «листки × ученики»** — решётка 18 столбцов на 56 строк: строка на ученика, столбец на
  листок, в клетке — сколько задач листка он сдал из скольких. Это главный вид: владелец смотрит
  на него, чтобы понять, кто где встал.
- **Разрез «ученик»** — по щелчку на фамилии: его листки и его задачи, что сдано и когда
  (`valid_at`).
- Сводка сверху тремя числами: учеников, листков, сданных задач всего.
- 🔴 **Страница ТОЛЬКО ЧИТАЕТ.** Ни одной формы, ни одного `POST`, ни одного `INSERT`. Приём
  задач — соседняя позиция волны, у неё своя зона.
- Открывается по паролю: механизм входа делает соседний заход `veb-vhod-i-obshchee-sostoyanie`.
  Если его кода на диске ещё нет — НЕ пиши свой вход и НЕ жди: оставь страницу под тем же
  правилом, что и остальные, одной строкой в отчёте назови, чем именно она сейчас защищена.

**B. `veb/static/konduit.css`** — решётка на 18 столбцов обязана читаться на ноутбуке. Без
внешних загрузок: страница обязана открываться там, где интернета нет, а сайт есть.

**C. `tests/veb/test_konduit.py`** — и главный тест здесь один: **`retract` уменьшает число
сданного**. Фикстура: ученик, задача, `assert`, затем `retract` на неё же → страница обязана
показать НОЛЬ сданных, а не одну и не две.

🔴 **ВТОРОГО ЖУРНАЛА ОТМЕТОК НЕ ЗАВОДИТЬ. Это объявлено ПРОВАЛОМ ВОЛНЫ в мандате.**
Ни своей таблицы, ни своего кеша сданного в файле. Читаешь — через сервисы.

### Готовность

**КРИТЕРИЙ ГОТОВНОСТИ (может ПРОВАЛИТЬСЯ)** — четыре клаузы, все предъявляются выводом команд:

1. **Живой прогон на реальном объекте — страница открывается на живой базе.** Подними сервер,
   `curl` по адресу страницы кондуита → `200`, первые 40 строк тела в отчёт.
2. **Три числа сводки сверены с базой.** Сними те же три числа прямым запросом к
   `data/spetsmat.db` и предъяви ОБЕ пары рядом. Расходятся — клауза красная.
   🔴 Число сданных задач НЕ равно 15847 и НЕ равно 15112: `retract` снимает более ранние
   `assert`. Если у тебя вышло ровно одно из этих двух чисел — ты считаешь события, а не сданное.
3. **`retract` уменьшает.** Прогон теста из пункта C с выводом.
4. **`python3 -m pytest tests/veb -q`** → зелёный, с числом тестов, и ни один тест соседа не сломан.
...
Верификатор нужен, тип — **ПОСЛЕ-типа** — судит результат, стоит в конце, после задачи. Свежий субагент, ДРУГИМ методом (живой прогон по живой базе: сверить три числа страницы с тремя числами, снятыми прямыми запросами sqlite к data/spetsmat.db, и предъявить обе пары), не перечитывает свою же правку. Доля сплошной выборки: 3 из 3 числа сводки, 2 из 2 разреза. Финальная строка ответа обязательна дословно: «выдано N позиций из M найденных» — без неё ответ считается усечённым и в отчёт не вставляется.
...
```
git --no-optional-locks add -- veb/konduit.py veb/static/konduit.css tests/veb/test_konduit.py                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- veb/konduit.py veb/static/konduit.css tests/veb/test_konduit.py   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `veb/konduit.py` `veb/static/konduit.css` `tests/veb/test_konduit.py`

- **Г1. Зона доехала в git.** `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone veb/konduit.py` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone veb/static/konduit.css` → ✅; `python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py check --zone tests/veb/test_konduit.py` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
...
```
python3 /Users/ivanyakovlev/Documents/GitHub/disciplina/_generator/tools/git_zona.py vlit-v-osnovnuyu veb-konduit-proshlogo-goda --zone <своя зона> \
...
виден»: прогон изменённого механизма из `/Users/ivanyakovlev/Documents/GitHub/spetsmat-bot`, НЕ из рабочей папки `/Users/ivanyakovlev/Documents/GitHub/spetsmat-bot-wt/veb-konduit-proshlogo-goda` плюс `grep` по ЖИВОМУ файлу,
...
**ВЕТКА РАБОТЫ:** `veb-konduit-proshlogo-goda`
