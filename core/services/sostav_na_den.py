...
def perekrytiya_k_snyatiyu(
    sostav: SostavDnya,
    otsutstvuyushchie_prepodavateli: Sequence[int] = (),
) -> tuple:
    ...
    🔴 ОТКАЗ 2 — ПОСТОЯННЫЙ ПРЕПОДАВАТЕЛЬ ЭТОГО ШКОЛЬНИКА СЕГОДНЯ ОТСУТСТВУЕТ.
    ...
    сегодня нет в здании, — то есть сделать ровно то, ради предотвращения чего
    правку и внесли. Отсутствие принимающего живёт в `teacher_attendance` и
    приезжает сюда списком: этот модуль в базу не ходит.
