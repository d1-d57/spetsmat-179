...
# 🔴 ДНИ ЗАНЯТИЙ ЖИВУТ В ОДНОМ МЕСТЕ, И ЭТО ОНО. Занятия по ПОНЕДЕЛЬНИКАМ и
# ЧЕТВЕРГАМ; `date.weekday()` считает от понедельника с нуля, поэтому пн = 0, чт = 3.
#
# 🔴 ЭТО ПРАВКА ПОДПИСЕЙ, А НЕ ДАННЫХ, И ОНА ЛИКВИДИРУЕТ РАСХОЖДЕНИЕ, А НЕ ВВОДИТ ЕГО.
# `migrations/003_slot_vmesto_weekday.sql` кладёт weekday=1 → slot=1 и weekday=4 →
# slot=2 (ISO, понедельник = 1), то есть слот 1 — понедельник, слот 2 — четверг, и
# так уже давно. `ops/raspisanie.py` согласен: `LESSON_WEEKDAYS = (1, 4)`, и по нему
# же стоят таймеры бэкапов в `deploy/`. Отставало ровно это место — оно одно звало
# дни четвергом и субботой, и через него неверные имена уезжали на страницу.
# Расписание на 2026-09-07 продиктовано владельцем 06.09: пн 14:15–15:55, чт 13:10–15:00.
DNI_ZANYATIJ = {0: "понедельник", 3: "четверг"}

# 🔴 СОКРАЩЕНИЯ ДНЕЙ — СТАНДАРТНЫЕ, И ЭТО ПРОВЕРЕНО, А НЕ ВЫВЕДЕНО ИЗ СЛОВА.
# Русская норма — двухбуквенные `пн вт ср чт пт сб вс` (Грамота.ру; правил ГОСТа
# на этот счёт нет, написание закреплено словарями сокращений). Раньше сокращение
# бралось первыми двумя буквами названия — и получалось «по» и «че», чего в языке
# нет вовсе. Владелец: «не „понь“ и „че“, это точно».
SOKR_DNYA = {0: "пн", 1: "вт", 2: "ср", 3: "чт", 4: "пт", 5: "сб", 6: "вс"}
