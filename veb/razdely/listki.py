...
#
# 🔴 СРОК НАЗВАН СОБЫТИЕМ, А НЕ ДАТОЙ, И ЭТО НЕ СТИЛЬ — ЭТО ЕДИНСТВЕННАЯ
# ФОРМУЛИРОВКА, КОТОРАЯ НЕ ВРЁТ НИ ОДНОГО ДНЯ. Напрашивается «сдать на этом
# занятии» (или, как просил заход, «до следующего спецмата» с датой ближайшего
# занятия) — и обе эти строки ложны в день выдачи листка:
#   `blizhajshij_den()` (`veb/sobrat_fajl.py:80`) считает ближайший день
#   «начиная с сегодняшнего», поэтому в день занятия `kt.blizh` — это СЕГОДНЯ.
#   Как только новый листок попадает в `L9` и его PDF ложится на диск, карточка
#   показывает уже ЕГО, и обе строки велят сдать его на том самом занятии, на
#   котором он выдан. Сдавать его надо к СЛЕДУЮЩЕМУ. Листок 16 выдан 2026-09-03,
#   в день занятия, — то есть это штатный день выдачи, а не редкий угол.
# Отличить «сегодня срок» от «сегодня выдача» можно только по дате выдачи
# листка, а её здесь нет: `L9` дат не хранит, и в базу карточка не ходит вовсе
# (`sheets.issued_at` существует, но чтение из базы — работа ветки «А», §С4).
# Поэтому строка называет то, что истинно всегда и записано в модели дословно:
# `doc/model.md:38` — «Дедлайн листка наступает в момент выдачи следующего».
# Дата ближайшего занятия при этом уже стоит первой строкой карточки, и второй
# раз здесь не называется: «второе упоминание — то самое повторение, от которого
# мы избавляемся по всему сайту» (`glavnaya.py`, про список принимающих).
SROK_TEKST = "сдать до следующего листка"
