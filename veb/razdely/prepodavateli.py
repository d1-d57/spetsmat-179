...
def galochka_dnya(kt, x, kl):
    ...
    """
    # 🔴 ПОДПИСИ «пн»/«чт» ВНУТРИ ГАЛКИ БОЛЬШЕ НЕТ (G6.1, владелец 10.09).
    # Столбец УЖЕ озаглавлен ПН или ЧТ, и повторять день внутри него — дословно
    # «это глупость, это не надо». День остаётся в `title` (его читает наведение)
    # и в заголовке колонки; сам орган цел, ушла только вторая подпись.
    #
    # 🔴 ЧЕГО ЗДЕСЬ НАМЕРЕННО НЕ СДЕЛАНО: G6.2 требует убрать эти галки из
    # постоянного распределения ВОВСЕ. Не сделано и вынесено владельцу вопросом,
    # потому что галка — ЕДИНСТВЕННЫЙ орган, которым задаётся «в четверг прихожу»:
    # она пишет в `prepodavatel_ne_prihodit` (`infra/prepodavatel_den_repo.py:60`),
    # а эта же таблица сужает список принимающих по дню — пункт A2, закрытый этой
    # же волной по прямому требованию владельца. Убрать орган здесь значит отнять
    # единственный способ сказать «Ольга Рыжая по четвергам не приходит» и молча
    # обрушить A2.
    sl = kt.DNI[kl][1]
...
def metka_dnya(kt, x, kl):
    ...
    # Подписи дня нет по той же причине, что у галки: столбец уже озаглавлен (G6.1).
    # Гостю признак остаётся ФОРМОЙ — закрашена или бледная, — а день говорит `title`.
    est = prihodit(kt, x, kl)
    return ('<span class="den-metka%s" data-tolko-gost title="%s"></span>'
            % ("" if est else " pusto", e(kt.DNI[kl][0])))
