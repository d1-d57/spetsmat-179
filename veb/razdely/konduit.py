...
/* ── СЧЁТЧИК ОБЯЗАТЕЛЬНЫХ ────────────────────────────────────────────────────
   ...
   `tr.moi` выше), а «закрыл всё обязательное» переехало в свой столбец `td.gt`.
...
/* 🔴 СЧЁТЧИК СТОИТ В СВОЕЙ КОЛОНКЕ ПОСЛЕ ФАМИЛИИ — ПРАВКА ВЛАДЕЛЬЦА 10.09 (H4.4).
   ...
   3. Счётчик говорит РОВНО ОДНО: сколько обязательных сдано из скольких. «Закрыл
      все обязательные» он не выражает ничем — ни цветом числа, ни подложкой: этот
      признак живёт в столбце `td.gt` и больше нигде (O1, канон «нельзя смешивать»).
      Подложка `--accent-soft` на счётчике закрывшего стояла здесь до 10.09 и снята
      вместе с подсветкой строки — это был второй носитель того же самого. */
