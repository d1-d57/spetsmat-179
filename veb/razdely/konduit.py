...
def _schyotchik(schyot) -> tuple:
    """`(подсказка, разметка)` счётчика: сколько обязательных сдано — КОГДА ДОЛГИ ЕСТЬ.
    ...
    🔴 ВЕТКИ «СДАЛ ВСЁ» ЗДЕСЬ БОЛЬШЕ НЕТ, И ЭТО НЕ ПОТЕРЯ — ПРАВКА ВЛАДЕЛЬЦА 11.09.
    «Когда у тебя обязательные задачи все сданы, вместо того чтобы ставить счётчик вида
    N из N, ставить большую красивую галочку».  Значит состояние «сдано всё» до счётчика
    просто не доходит: клетку берёт `_galka`, и решает это `_stolbec_obyaz` ниже — один
    раз, на оба разреза.  Оставить здесь `if schyot.zakryl` значило бы держать вторую
    редакцию того же решения в месте, куда управление не приходит: она была бы вечно
    зелёной и вечно ложной.
    """
    if schyot.vsego == 0:
        return ("в этом листке нет обязательных задач",
                '<i class="ob-sch net">\u00b7</i>')
    return (("обязательных сдано %d из %d, осталось %d"
             % (schyot.sdano, schyot.vsego, schyot.ostalos)),
            '<i class="ob-sch">%d<span class="iz">/%d</span></i>'
            % (schyot.sdano, schyot.vsego))


#: Шапки колонки обязательных — по одной на разрез, и разными их делает ГАЛОЧКА, а не
#: счётчик.  Слово сокращено не для красоты: колонка узкая нарочно, а полное «обязательных»
#: раздвинуло бы её втрое и отобрало ширину у самой решётки.  Что именно сокращено, говорит
#: подсказка, и она же называет оба числа словами — то же устройство, что и у самого счётчика.
#:
...
#: столбца написан словами.  До 11.09 эти две подписи стояли над ОТДЕЛЬНЫМ столбцом `gt`;
#: столбца больше нет, а вопросов по-прежнему два, и подписи переехали сюда целиком.
ZAGOLOVOK_SCH = ('<th class="sch" title="сдано обязательных задач этого листка '
                 'из общего их числа; крупная зелёная галочка вместо числа \u2014 '
                 'школьник закрыл всё обязательное этого листка">\u041e\u0431\u044f\u0437.</th>')
ZAGOLOVOK_SCH_GOD = ('<th class="sch" title="сдано обязательных задач этого разреза '
                     'из общего их числа; крупная зелёная галочка вместо числа \u2014 долгов '
                     'нет, и рядом с ней написано почему: номер листка, закрытого целиком, '
                     'или \u00ab\u0432\u0441\u0451\u00bb, если сданы все обязательные '
                     'задачи разреза">\u041e\u0431\u044f\u0437.</th>')


def _galka(podskazka: str, podpis: str = "") -> tuple:
    """`(подсказка, разметка)` крупной зелёной галочки — того, что стоит ВМЕСТО счётчика.

    🔴 ГАЛОЧКА ЗАНИМАЕТ КЛЕТКУ СЧЁТЧИКА — ПРАВКА ВЛАДЕЛЬЦА 11.09, И ОНА ПЕРЕСМАТРИВАЕТ
    ЕГО ЖЕ РЕШЕНИЕ 10.09 ОБ ОТДЕЛЬНОМ СТОЛБЦЕ.  Дословно: «я галочки предлагал ставить
    в другом месте — там, где написано слово „обязательное“.  Когда у тебя обязательные
    задачи все сданы, вместо того чтобы ставить счётчик вида N из N, ставить большую
    красивую галочку».  Прежний столбец `gt` стоял вплотную к счётчику, и два соседних
    столбца отвечали на ОДИН вопрос — «как у него с обязательными»: у закрывшего горели
    оба, `5/5` и галочка рядом.  Это то же смешивание, от которого 10.09 лечили строку
    (O1), только разложенное по горизонтали.  Клетка теперь одна, и в ней ровно одно:
    или галочка, или счётчик.

    🔴 НЕ СДАЛ — СТОИТ СЧЁТЧИК, А НЕ КРЕСТИК И НЕ СЕРАЯ ГАЛОЧКА.  Пятьдесят три строки,
    и знак у каждой второй перестал бы быть заметным — то самое «достаточно заметной»,
    о котором владелец сказал 10.09.  Отсутствие галочки уже сообщение: на её месте
    стоят числа, которые называют, сколько осталось.

    🔴 ПОДПИСЬ РЯДОМ С ГАЛОЧКОЙ — ЭТО ОДИН ФАКТ ДВУМЯ КЕГЛЯМИ, А НЕ ВТОРОЙ НОСИТЕЛЬ.
    Набрана тем же `<span class="iz">`, что и знаменатель счётчика (`3/5`), и по той же
    причине: она отвечает на вопрос «галочка — про что?», а не заводит новый факт.  Чем
    её заполняет годовой разрез, решает `_obzor`.
    """
    hvost = '<span class="iz">%s</span>' % e(podpis) if podpis else ""
    return (podskazka, '<i class="ob-sch gt-da">\u2713%s</i>' % hvost)


def _stolbec_obyaz(schyot, dolgov_net=False, podskazka="", podpis="") -> str:
    """Клетка «как у него с обязательными» — ОДНА НА ОБА РАЗРЕЗА, и говорит она одно.

    🔴 ВЫБОР СТОИТ ЗДЕСЬ, А НЕ В ДВУХ МЕСТАХ ВЫЗОВА.  Разрезов два (`_obzor` и `_listok`),
    а «когда галочка вытесняет счётчик» — один и тот же вопрос для обоих; разложенный по
    двум `f`-строкам, он разъехался бы ровно так, как разъезжались все прежние пары
    источников в этом файле.  Разрез отвечает только на СВОЙ вопрос — «есть ли долги и
    почему», — а чем это нарисовать, решает эта функция.

    🔴 ПОДСКАЗКА ВИСИТ НА КЛЕТКЕ, А НЕ НА ЗНАКЕ ВНУТРИ НЕЁ.  Та же правка 11.09, что и у
    столбца принимающего (`_stolbec_prin`): владелец наводит на КЛЕТКУ, и подсказка,
    висящая на `<i>` шириной в один символ, не показывается почти никогда.  Галочка —
    один символ и есть, то есть промах здесь был бы ещё вернее, чем на инициалах.
    """
    podskazka, razmetka = (_galka(podskazka, podpis) if dolgov_net
                           else _schyotchik(schyot))
    return '<td class="sch" title="%s">%s</td>' % (e(podskazka), razmetka)


def _klass_stroki(u, chuzhoj) -> str:
    """Класс строки школьника: чей это ребёнок — и БОЛЬШЕ НИЧЕГО.

    🔴 ПОДСВЕТКА СТРОКИ НЕСЁТ РОВНО ОДИН ПРИЗНАК — ПРАВКА ВЛАДЕЛЬЦА 10.09
    (`TZ-DOBOR-10-09.md` O1).  Дословно: «я хочу видеть своих школьников, и я их вижу.
    А теперь ещё вижу почему-то школьников, которые просто всё сдали — это не нужно…
    Ты сейчас выделяешь всю строчку — не надо».  До этой правки строка светилась ДВУМЯ
    смыслами сразу: `moi` («мой ребёнок») красил фон `--accent-soft`, `gotov` («закрыл
    все обязательные») — `--chip`, и прежняя редакция этого места считала их сложение
    достоинством («два признака СКЛАДЫВАЮТСЯ»).  Владелец судит иначе, и это его канон
    «нельзя смешивать» (`Q4`): одна клетка — один смысл, один признак — один носитель.

    🔴 «ЗАКРЫЛ ВСЕ ОБЯЗАТЕЛЬНЫЕ» НЕ ПОТЕРЯН, А ПЕРЕЕХАЛ В СВОЙ СТОЛБЕЦ (`_galka`, O2).
    Снятая подсветка — не потеря факта, а перенос его в носитель, который ничего кроме
    него не значит.
    """
    if not chuzhoj:
        return ""
    return ' class="%s"' % ("chuzh" if u.id in chuzhoj else "moi")
...
def _obzor(na_uchyote, listki, zadachi, sostoyaniya, chuzhoj, prinimayushchie, imya="vse") -> str:
    ...
    for u in na_uchyote:
        ...
        kletki = []
        celikom = []        # номера листков, закрытых ЦЕЛИКОМ, — подпись к галочке (O5)
        for sh in listki:
            zad = zadachi[sh.id]
            vzyato = sum(1 for p in zad if sostoyaniya[(u.id, p.id)].is_credited)
            if zad and vzyato == len(zad):
                celikom.append(sh.number)
            if not zad:
                kletki.append('<td></td>')
            elif vzyato == len(zad):
        ...
        # 🔴 ЧИСЛА СЧЁТЧИКА НЕ ПРОПАДАЮТ ВМЕСТЕ С НИМ, А ПЕРЕЕЗЖАЮТ В ПОДСКАЗКУ.
        # С 11.09 галочка занимает клетку счётчика, и на годовом разрезе она может
        # гореть у школьника, у которого обязательных сдано 17 из 21 (`O5`: закрыт
        # листок целиком — долга нет, хотя число не полное). Если бы `17/21` просто
        # исчезло с экрана, правка «поставить галочку вместо счётчика» отняла бы
        # факт, о потере которого никто не просил. На вкладке ЛИСТКА дописывать
        # нечего: там галочка горит ровно при `N из N`, и подсказка это и говорит.
        #
        # 🔴 ПОДПИСЬ РЯДОМ С ГАЛОЧКОЙ — ТРЕБОВАНИЕ ВЛАДЕЛЬЦА 11.09, И ОНО ПРО ЭТУ
        # ВКЛАДКУ.  Дословно: «Что значит галочка в разделе Весь год?  Я не понимаю.
        # Это значит, что один из листков закрыт?  Тогда должна быть не просто галочка,
        # а дописано, ЧТО ЭТО 16-й листок — что у него нет долгов по 16-му листку,
        # потому что он сдал 16A».  Голая галочка отвечала на вопрос «долги есть?» и
        # молчала о том, ПОЧЕМУ их нет, — а причин здесь ровно две, и они разные.
        #
        # 🔴 «ВСЁ» — ЭТО НЕ ОТКАЗ НАЗВАТЬ НОМЕР, А СОСТОЯНИЕ, В КОТОРОМ НОМЕРА НЕ
        # СУЩЕСТВУЕТ.  Обязательные — подмножество листка, поэтому школьник может сдать
        # ВСЕ обязательные разреза, не закрыв ни одного листка целиком: назвать листок
        # тут нечем, и любой вписанный номер был бы утверждением, которого база не
        # поддерживает.  Замер на живой базе 11.09: галочек на годовом разрезе 14, из
        # них 6 ровно такие.  Когда причина — закрытый листок, стоит его номер, как и
        # просил владелец.  Когда сдано ВСЁ обязательное, это утверждение СИЛЬНЕЕ
        # любого одного листка, и подменять его номером значило бы занижать ответ.
        if schyot.zakryl:
            podskazka = "долгов нет: закрыл все %d обязательных этого разреза" % schyot.vsego
            if celikom:
                podskazka += " (и листки %s — целиком)" % ", ".join(celikom)
            podpis = "всё"
        elif celikom:
            # 🔴 ПОДСКАЗКА НАЗЫВАЕТ ПРИЧИНУ, ПО КОТОРОЙ НЕПОЛНОЕ ЧИСЛО НЕ ЯВЛЯЕТСЯ
            # ДОЛГОМ.  Находка верификатора §3: «долгов нет» рядом с «обязательных сдано
            # 146 из 215» он прочитал как противоречие внутри одной строки — и прочитал
            # бы так же владелец, ради которого эта подсказка и написана.  Противоречия
            # нет, но чинится оно НЕ снятием числа, а тем самым «листки даны на выбор»,
            # которым владелец объяснил O5: «школьник закрыл один и не закрывал второй —
            # это норма, а не долг».  Без этой половины фразы подсказка называет вывод и
            # прячет посылку.
            podskazka = ("долгов нет: листки даны на выбор, и %s %s закрыт%s целиком"
                         % ("листок" if len(celikom) == 1 else "листки",
                            ", ".join(celikom), "" if len(celikom) == 1 else "ы"))
            if schyot.vsego:
                podskazka += ("; обязательных сдано %d из %d"
                              % (schyot.sdano, schyot.vsego))
            # Подпись называет ПЕРВЫЙ из них: колонка узкая нарочно — владелец дважды
            # просил не отбирать ширину у решётки, — и три номера подряд её распёрли бы.
            # Остальные названы в подсказке этой же клетки, а на самой строке они и так
            # видны: их клетки покрашены как закрытые.
            podpis = celikom[0]
        else:
            podskazka = podpis = ""   # галочки не будет, и подписывать нечего
        ...
        stroki.append(
            ...
            f'{_stolbec_obyaz(schyot, dolgov_net, podskazka, podpis)}'
            f'{"".join(kletki)}</tr>')
    return (f'<section class="vid" id="n-{imya}">'
            f'<table class="kond" style="max-width:{25 + len(listki) * 5.5:.1f}em">'
            f'<thead><tr><th>Ученик</th>{ZAGOLOVOK_PRIN}{ZAGOLOVOK_SCH_GOD}'
            f'{shapka}</tr></thead>'
...
def _grobarij(listki, zadachi, na_uchyote, sostoyaniya, daty) -> str:
    ...
    🔴 ПУСТОЙ ГРОБАРИЙ ПУСТ, И ЭТО ПРАВКА ВЛАДЕЛЬЦА 10.09 (`TZ-DOBOR-10-09.md` O4).
    Дословно: «мне не нравится, что сюда попадает куча текста от тебя (задачи листка), а
    гробарий пуст.  Это не ошибка — это нейрослоп.  Убираем это.  **Пустой гробарий пусть
    будет пустым**».  Здесь стояли два абзаца прозы: правило «сюда попадают задачи листка,
    ставшего историческим…» — и объяснение пустоты, перечислявшее розданные листки.  Оба
    сняты целиком.

    🔴 КОРОТКОЙ ВЕРСИИ НА ИХ МЕСТЕ НЕ ЗАВОДИТСЯ, И ЭТО НАЗВАНО ПРАВИЛОМ, А НЕ ВКУСОМ.
    Это второй нейрослоп за сутки — первым была подпись под полосой занятий в кабинете
    (`K1`), и владелец потребовал убрать её тем же словом.  Замена коротким пояснением
    есть тот же нейрослоп, только тише.

    🔴 ЧТО ОТ ПРЕЖНЕГО ДОВОДА ОСТАЛОСЬ ЧЕСТНЫМ.  Прежняя редакция этого места
    защищала прозу тем, что пустая вкладка «неотличима от сломанной».  Довод не выдуман, но
    владелец посмотрел на живой экран и рассудил иначе: вкладка называется «Гробарий»
    заголовком панели, как называется любая другая панель этого раздела, и пустота под
    известным заголовком — это сообщение, а не молчание.
    """
    ...
    if not istoricheskie:
        return '<section class="vid" id="n-grob"><p class="zag2">Гробарий</p></section>'
    ...
    for sh in istoricheskie:
        ...
        if not zapisi:
            # 🔴 ЛИСТОК, С КОТОРОГО НИЧЕГО НЕ ПОШЛО, НЕ ПИШЕТ О СЕБЕ НИЧЕГО (O4).
            # Здесь стояла строка «ни одна задача этого листка не осталась за порогом» под
            # его заголовком — тот же текст «от тебя» в пустом месте, что и снятые абзацы.
            continue
        stroki = "".join(
    ...
    return (f'<section class="vid" id="n-grob">'
            f'<p class="zag2">Гробарий</p>{"".join(bloki)}</section>')
...
/* ── СЧЁТЧИК ОБЯЗАТЕЛЬНЫХ ────────────────────────────────────────────────────
   🔴 СВЕТЯЩЕЙСЯ СТРОКИ ЗДЕСЬ БОЛЬШЕ НЕТ — ПРАВКА ВЛАДЕЛЬЦА 10.09 (O1). Правила
   `tr.gotov td{{background:--chip}}` стояли ровно тут и красили всю строку тому,
   кто закрыл обязательные. Владелец: «ты сейчас выделяешь всю строчку — не надо».
   Подсветка строки осталась ровно у одного признака — «мой школьник» (правила
   `tr.moi` выше), а «закрыл всё обязательное» переехало в знак — с 11.09 он стоит
   в клетке счётчика `td.sch` вместо числа, а не в столбце рядом с ней.
   Снято ДВА носителя одного признака, а не один: фон строки и подложка счётчика
   (`tr.gotov td.sch .ob-sch{{background:--accent-soft}}`, стояла ниже). Оставить
   второй значило бы починить жалобу и сохранить её причину. */
/* 🔴 СЧЁТЧИК СТОИТ В СВОЕЙ КОЛОНКЕ ПОСЛЕ ФАМИЛИИ — ПРАВКА ВЛАДЕЛЬЦА 10.09 (H4.4).
   ...
   3. Клетка говорит РОВНО ОДНО, и с 11.09 это «как у него с обязательными»: или
      сколько сдано из скольких, или — если долгов нет — галочка ВМЕСТО числа.
      Двух ответов сразу в ней не бывает: `_stolbec_obyaz` выбирает один. Подложка
      `--accent-soft` на счётчике закрывшего стояла здесь до 10.09 и снята вместе с
      подсветкой строки — это был второй носитель того же самого. */
...
#s-kond .kond tbody td.sch{{text-align:center;white-space:nowrap;padding:.3rem .4rem;
  min-width:0;border-left:1px solid var(--rule);border-right:1px solid var(--rule);
  border-bottom:1px solid var(--rule);background:var(--bg);cursor:help}}
...
/* ── КРУПНАЯ ЗЕЛЁНАЯ ГАЛОЧКА — В ТОЙ ЖЕ КЛЕТКЕ, ГДЕ СТОЯЛ БЫ СЧЁТЧИК ───────
   🔴 ПРАВКА ВЛАДЕЛЬЦА 11.09: «там, где написано слово „обязательное“… вместо того
   чтобы ставить счётчик вида N из N, ставить большую красивую галочку».  До неё у
   галочки был СВОЙ столбец (`td.gt`, правка 10.09 O2), и у закрывшего горели оба —
   `5/5` и галочка вплотную к нему.  Столбца больше нет; размер и цвет переехали на
   сам знак, потому что клетка теперь общая.
   🔴 КРУПНАЯ — ЭТО `1.15rem` ПРОТИВ `.92rem` У СЧЁТЧИКА, и число сохранено дословно
   с 10.09: «достаточно заметной галочки — мы увидим, что школьник всё сдал».  Она
   обязана читаться раньше числа, потому что отвечает на вопрос «всё?», а не «сколько?».
   ...
   Подпись у галочки красится ТЕМ ЖЕ зелёным, а не `--faint` общего правила `.iz`:
   она про галочку, а не про клетку вокруг неё, и серая читалась бы как отдельный
   факт рядом. */
#s-kond .kond td.sch .ob-sch.gt-da{{font-size:1.15rem;line-height:1;color:var(--zel)}}
#s-kond .kond td.sch .ob-sch.gt-da .iz{{color:var(--zel);font-size:.68rem;
...
@media(max-width:760px){{
  ...
  /* 🔴 НА ТЕЛЕФОНЕ КОЛОНКИ ПРИНИМАЮЩЕГО И СЧЁТЧИКА ЕДУТ ВМЕСТЕ С ФАМИЛИЕЙ, А НЕ
     ...
     уводил. Примёрзших столбцов ровно три, и четвёртого нет: с 11.09 галочка
     живёт в клетке счётчика, а не в столбце рядом, и вопрос «всё?» едет вместе с
     ним — отдельного липкого столбца под него больше не нужно. */
  ...
  /* Галочка остаётся КРУПНЕЕ счётчика и на телефоне: она и на большом экране
     читается раньше числа, и пропорция важнее абсолютного кегля. */
  #s-kond .kond td.sch .ob-sch.gt-da{{font-size:.95rem}}
...
/* ── ГРОБАРИЙ ─────────────────────────────────────────────────────────────
   Ни одного нового приёма: таблица — та же `kond-lich`, что и в личной карточке
   школьника, имена — те же фишки на `--chip`. Отличие ровно одно: у гробария есть
   шапка, потому что три столбца тут не читаются без подписи.
   ⚠ Правило `.grob-pravilo` стояло здесь и оформляло абзац с объяснением, что
   попадает в гробарий. Абзац снят целиком 10.09 (O4, «пустой гробарий пусть будет
   пустым»), и правило снято вместе с ним: селектор без разметки — это мусор,
   который следующий читатель примет за живой. */
