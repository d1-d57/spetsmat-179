...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path.startswith("/static/"):
            ...
            return
        # 🔴 ПРОБА СВЯЗИ — САМЫЙ ДЕШЁВЫЙ ОТВЕТ НА ЭТОМ СЕРВЕРЕ, И ОНА СТОИТ ЗДЕСЬ,
        # ДО ВСЕГО ОСТАЛЬНОГО. Оффлайн-очередь (`veb/razdely/ochered.py`) спрашивает
        # ею ровно одно: «сервер отвечает или нет», — и спрашивает регулярно, пока
        # баннер деградации висит на экране. Поэтому здесь нет ни базы, ни сборки
        # страницы, ни куки: проба обязана стоить меньше, чем то, что она сторожит,
        # иначе она сама станет нагрузкой на занятии, где восемнадцать телефонов.
        #
        # 🔴 `no-store` НЕ УКРАШЕНИЕ. Ответ, положенный в кеш браузера или в кеш
        # промежуточного узла (VPN, школьный прокси), вернулся бы 200 при мёртвой
        # сети — и баннер «связи нет» не появился бы никогда, ровно в тот момент,
        # ради которого он существует.
        #
        # 🔴 И `navigator.onLine` ЭТИМ НЕ ЗАМЕНЯЕТСЯ — наоборот, это его замена:
        # на школьном wi-fi флаг говорит «online», когда наружу не проходит ничего
        # (`NADEZHNOST-zakaz-na-resyorch.md` §2). Признаком связи считается ФАКТ
        # ответа отсюда, а не флаг.
        #
        # Ответ собирается здесь, а не через `_send_json`: заголовок `Cache-Control`
        # нужен ровно этому маршруту, а добавить его в общий отправщик значило бы
        # поменять ответы всех остальных ручек заодно.
        if path == "/api/zhiv":
            telo = b'{"zhiv": true}'
            self.send_response(200)
            self.send_header("Content-Type", "application/json; charset=utf-8")
    ...
    def _post_prepodavateli(self) -> None:
        ...
        if deystvie == "ubrat":
            ...
            # 🔴 «В ЭТОТ ДЕНЬ ПРИХОЖУ» — ГАЛОЧКА, И ЭТО РЕШЕНИЕ ВЛАДЕЛЬЦА 07.09,
            # ПЕРЕСМАТРИВАЮЩЕЕ ПРЕДЫДУЩУЮ РЕДАКЦИЮ ЭТОГО ЖЕ ЗАХОДА. Дословно:
            # «не бывает принимающего, который в разные дни в разных группах,
            # такого никогда не было… можно сделать проще: две кнопки-галочки —
            # „в четверг прихожу“ и „в понедельник прихожу“, и отдельно выбирается
            # группа принимающего… он ходит только в одну группу, за которой он
            # фиксирован, но либо один, либо два дня». До этого здесь стоял выбор
            # `В · Д · Н · —` на каждый день — орган, которым можно записать то,
            # чего в школе не бывает.
            #
            # 🔴 СНЯТАЯ ГАЛОЧКА ЗАКРЫВАЕТ ЕГО СТРОКИ ЭТОГО ДНЯ, И ЭТО НЕ ПОБОЧНЫЙ
            # ЭФФЕКТ, А СОДЕРЖАНИЕ ОТМЕТКИ. Человека в четверг не будет — значит
            # четверговых детей надо раздать, и они обязаны стать видимыми как
            # нераспределённые на странице занятия. Оставить их за отсутствующим
            # значило бы показать в четверг группу, которой никто не ведёт.
            # Понедельник при этом не трогается ни строкой.
            #
            # Поставленная галочка не создаёт ничего, кроме самой памяти дня:
            # детей раздаёт человек, а не эта дверь.
            ...
            if not 1 <= slot <= 7:
                self._send_json(400, {"error": "slot must be 1..7"})
                return
            prihodit = bool(payload.get("prihodit"))
            from infra.prepodavatel_den_repo import otmetit
            otmetit(connection, int(tid), slot, prihodit)
            if prihodit:
                self._send_json(200, {"ok": True, "slot": slot, "prihodit": True})
                return
            _obespechit_gruppu_shkolnika(connection)
            ...
            connection.commit()
            self._send_json(200, {"ok": True, "slot": slot, "prihodit": False,
                                  "snyaty": snyaty})
            return
        else:
            self._send_json(400, {
