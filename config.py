...
#: 🔴 КАНИКУЛЫ — ФАКТ КАЛЕНДАРЯ ШКОЛЫ, А НЕ ДОГАДКА РАСПИСАНИЯ. Даты названы
#: владельцем 11.09 со страницы школы (schc179.mskobr.ru/uchashimsya/raspisanie-kanikuly),
#: и он же предупредил: «к сожалению, бывают изменения по ходу учебного года» —
#: поэтому список лежит ОДНОЙ константой, которую правят в одном месте, а не
#: размазан по экранам. Обе границы ВКЛЮЧИТЕЛЬНЫ. В эти дни занятий нет: они
#: выпадают из решётки журнала, из подсчёта четвертей и из списка ближайших занятий.
KANIKULY = (
...
def v_kanikuly(den: str) -> bool:
    """Попадает ли день в каникулы. Одна дверь для всех, кто спрашивает календарь."""
    return any(ot <= den <= do for ot, do in KANIKULY)
