...
def table_text(
    ...
    # 🔴 БЫЛО «+ принята · сдана, не защищена: - · не сдана: ·» — три пары
    # «знак → смысл», склеенные так, что знак стоит то перед смыслом, то после,
    # и разобрать, чем помечена «сдана, не защищена», нельзя. Читает это
    # преподаватель, сверяя таблицу глазами.
