...
# 🔴 ДНИ ЗАНЯТИЙ ЖИВУТ В ОДНОМ МЕСТЕ, И ЭТО `veb/sobrat_fajl.py`. Понедельник и
# четверг, `date.weekday()` от понедельника с нуля. Здесь только соответствие слоту:
# слот 1 — понедельник, слот 2 — четверг, ровно как в `migrations/003_slot_vmesto_weekday.sql`
# (weekday 1 → slot 1, weekday 4 → slot 2). Раскладка на них одна (см. докстринг),
# различаются даты. Было `{1: 3, 2: 5}` — четверг и суббота; отсюда `valid_from`
# открытых строк уезжал на субботние даты, хотя занятие в этот день не идёт.
SLOT_DEN = {1: 0, 2: 3}
