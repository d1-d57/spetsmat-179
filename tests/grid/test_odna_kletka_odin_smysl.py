...
#: 🔴 СТОЛБЦА `gt` ЗДЕСЬ БОЛЬШЕ НЕТ — ПРАВКА ВЛАДЕЛЬЦА 11.09.  Он стоял вплотную к `sch`,
#: и два соседних столбца отвечали на ОДИН вопрос — «как у него с обязательными»: у
#: закрывшего горели оба, `3/3` и галочка рядом.  Владелец: «когда у тебя обязательные
#: задачи все сданы, вместо того чтобы ставить счётчик вида N из N, ставить большую
#: красивую галочку».  Смысл столбца от слияния не раздвоился, а сузился до одного
#: вопроса с двумя ответами: клетка говорит ЛИБО сколько сдано, ЛИБО что долгов нет, и
#: никогда оба сразу — это и проверяется ниже, по числу знаков в клетке.
SMYSL_STOLBCA = {
    "kto": "кто этот школьник",
    "pr": "кто у него принимает",
    "sch": "как у него с обязательными — сколько сдано или что долгов нет",
}
...
def narusheniya_kletok(html: str) -> list:
    ...
    for _klassy, razmetka in stroki_reshotki(html):
        for klass, soderzhimoe in kletki(razmetka):
            ...
            # 🔴 КЛЕТКА ОБЯЗАТЕЛЬНЫХ НЕСЁТ РОВНО ОДИН ЗНАК — ИЛИ СЧЁТЧИК, ИЛИ ГАЛОЧКУ.
            # С 11.09 они делят одну клетку, и «поставить галочку» соблазнительно
            # исполнить ДОПИСЫВАНИЕМ к числу; владелец просил замену («вместо того
            # чтобы ставить счётчик вида N из N»).  `<span class="iz">` знаком не
            # считается: им набраны знаменатель счётчика и подпись галочки — второй
            # кегль ОДНОГО факта, а не второй факт.
            if imya == "sch" and nositeli.count("i") != 1:
                plohie.append("в клетке обязательных не один знак: %r" % soderzhimoe)
    return plohie


def narusheniya_znaka(html: str) -> list:
    """Знак «закрыл всё» стоит не в своей клетке — или не стоит нигде.

    🔴 АДРЕС ЗНАКА ПОМЕНЯЛСЯ 11.09, А ВОПРОС — НЕТ.  Знак жил в столбце `gt`; теперь он
    живёт в клетке `sch` ВМЕСТО счётчика.  Проверяется по-прежнему одно: он стоит ровно
    в одном месте решётки, и клетки, которые про него не спрашивали, о нём молчат.
    """
    plohie, goryat = [], 0
    for _klassy, razmetka in stroki_reshotki(html):
        for klass, soderzhimoe in kletki(razmetka):
            ...
            elif imya in ("kto", "pr") and "\u2713" in soderzhimoe:
...
def test_samoproverka_a_duplicate_tick_beside_the_counter_goes_red(
    ...
    """Галочка, ДОПИСАННАЯ к счётчику вместо того чтобы его заменить, — то же смешивание.

    🔴 ИМЕННО ТАК ПРАВКА 11.09 ЛОМАЕТСЯ ТИШЕ ВСЕГО.  Владелец просил ЗАМЕНУ: «вместо
    того чтобы ставить счётчик вида N из N, ставить большую красивую галочку».  Знак,
    приписанный к числу, выглядит на экране почти как исполненная просьба и является
    прежним столбцом `gt`, сдвинутым на два пикселя влево."""
    _mir, prepod = vse_priznaki_srazu
    monkeypatch.setattr(
        konduit, "_schyotchik",
        lambda schyot: ("проба", '<i class="ob-sch">%d</i><i class="ob-sch gt-da">'
                        '\u2713</i>' % schyot.sdano))
    html = konduit.razdel(kontekst(connection, prepod_id=prepod))
    plohie = narusheniya_kletok(html)
    assert plohie, "рычаг не увидел галочку, дописанную к счётчику в одной клетке"
    assert "в клетке обязательных не один знак" in plohie[0], plohie[0]
