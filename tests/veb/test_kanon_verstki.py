...
def test_tri_voprosa_kanona(zamer, imya):
    ...
    # 🔴 ОБРЕЗКА В КОНТРОЛЕ ВЫДЕЛЕНА В СВОЙ ТЕСТ НИЖЕ, И ЭТО НЕ СМЯГЧЕНИЕ.  С
    # 11.09 гейт судит и `<select>` — по РЕНДЕРУ выбранного варианта, а не по
    # наличию тега, — и на живом распределении находит там настоящий дефект N1,
    # на который владелец жалуется четвёртый раз. Чинится он ВЁРСТКОЙ
    # распределения (`veb/razdely/**`), а не этим файлом, поэтому стоит рычагом
    # `xfail(strict=True)` рядом, как и клауза [F11]: дефект записан, посчитан в
    # каждом прогоне и покраснеет как XPASS в день, когда поля расширят.
    # Здесь остаётся строгий ноль по РАЗМЕТКЕ — ничего из купленного не потеряно.
    razmetka = [d for d in z["obrezka"]
                if "select" not in d["put"] and "input" not in d["put"]]
    assert not razmetka, (
...
@pytest.mark.xfail(strict=True, reason=(
    "ЗАМЕРЕНО, А НЕ ПРЕДПОЛОЖЕНО: правило владельца «ширина полей, в которые "
    "что-то вводится, должна выбираться по самому широкому тексту, который может "
    "быть введён» сегодня НЕ выполняется. На вкладке «школьникам» у организатора "
    "узки ВСЕ 108 полей из 108: селект принимающего имеет 74 px, а самый широкий "
    "его вариант («Елена Мирошниченко») требует 191. Владелец жалуется на это "
    "место ЧЕТВЁРТЫЙ раз (N1): «Дима Елисе◇», «Наталия Стр◇», «Александр Т◇». "
    "Чинится это вёрсткой распределения (`veb/razdely/**`) — зона соседней "
    "позиции, не этого файла. Здесь стоит РЫЧАГ, а не заплатка: строгий xfail "
    "держит дефект записанным и посчитанным (`xfailed` в каждом прогоне), и в "
    "день, когда поля расширят, тест покраснеет как XPASS. Это красное значит "
    "ровно одно — «сними маркер»."))
def test_polya_shirinoy_po_shirochayshemu_vvodimomu(zamer):
    """Правило владельца, ставшее проверкой, а не абзацем в ТЗ.

    🔴 СУДИТСЯ РЕНДЕР, А НЕ НАЛИЧИЕ ТЕГА. `<select>` исключили из проверки обрезки
    в круге 15 как источник ложно-красных, и исключение сделало слепое пятно ровно
    там, где владелец жалуется четвёртый раз. Гейт мерит КЛОН контрола с одним
    вариантом внутри и отпущенной шириной: браузер сам говорит, сколько места
    нужно этому тексту вместе со стрелкой и отступами.
    """
    z = zamer["stranicy"]["школьникам"]
    assert z, "страница «школьникам» не измерена"
    print(f"\nполя: осмотрено {z['na_pole']}, узких {len(z['polya'])}, "
          f"без известного набора значений {z['poley_bez_nabora']}")
    assert z["na_pole"] > 0, (
        "на вкладке «школьникам» не нашлось ни одного поля ввода — проверка не "
        "испытана, и ноль узких полей значит «не смотрел», а не «чисто»")
    assert not z["polya"], (
        "поле уже́ самого широкого варианта, который в него можно выбрать — " +
        "; ".join(f"{d['put']} «{d['tekst']}» надо {d['nado']} есть {d['est']}"
                  for d in z["polya"][:5]))
