...
#: Список закрытый и проверяется на протухание: `test_teksty.py` краснеет, если
#: слово отсюда не встречается НИ В ОДНОЙ живой строке. Чего он НЕ проверяет и
#: что честнее назвать, чем обещать: что такая команда боту известна и что она
#: открыта тому, кто читает отказ. Второе стоило этому же заходу ошибки — совет
#: «/auditoria» в отказе преподавателю, которому эта команда закрыта.
BELYJ_SPISOK_ZHIVOJ = (
    "Telegram",
    "setka",
    "god",
